...
pid_t GraphmasterPid;
bool IsUnderGraphmaster;
bool IsGraphmasterEnvironment = false;
...
static int	GraphmasterServerLoop(void);
...
bool
GraphSelectConfigFiles(const char *userDoption, const char *progname)
{
	char	   *configdir;
	char	   *fname;
...
MemoryContextReset(CacheMemoryContext);// = NULL;
	...
	if (chdir("..") < 0)
		ereport(FATAL,
				(errcode_for_file_access(),
				 errmsg("could not change directory to ..: %m")));

	GraphmasterPid = MyProcPid;
	
	IsGraphmasterEnvironment = true;
	 ...
	 * blocked at all times except when GraphmasterServerLoop is waiting for something to
	 * happen, and during that window, we want signals to exit the select(2)
	 * wait so that GraphmasterServerLoop can respond if anything interesting happened.
	 ...
	 */
	if (!GraphSelectConfigFiles(userDoption, progname))
		ExitPostmaster(2);
/*
	...
	if (ListenAddresses)
	{
		...
		foreach(l, elemlist)
		{
			...
			if (strcmp(curhost, "*") == 0)
				status = PostGraphStreamServerPort(AF_UNSPEC, NULL,
										  (unsigned short) 5678,
										  NULL,
										  ListenSocket, MAXLISTEN);
			else
				status = PostGraphStreamServerPort(AF_UNSPEC, curhost,
										  (unsigned short) 5678,
										  NULL,
										  ListenSocket, MAXLISTEN);
	...
	if (Unix_socket_directories)
	{
		...
		foreach(l, elemlist)
		{
			...
			status = PostGraphStreamServerPort(AF_UNIX, NULL,
									  (unsigned short) 5678,
									  socketdir,
									  ListenSocket, MAXLISTEN);
	...
	if (!load_hba())
	{
		 ...
		 */
		ereport(FATAL,
				(errmsg("could not load pg_hba.conf %s", HbaFileName)));
	}
//	if (!load_ident())
	{
		/*
...
/*
 * Determine how long should we let GraphmasterServerLoop sleep.
 *
 * In normal conditions we wait at most one minute, to ensure that the other
 * background tasks handled by GraphmasterServerLoop get done even when no requests are
...
/*
 * PostGraphClosePostmasterPorts -- close all the postmaster's open sockets
 *
 ...
 */
void
PostGraphClosePostmasterPorts(bool am_syslogger)
{
	int			i;
 ...
 */
static void
graphmaster_reaper(SIGNAL_ARGS)
{
	int			save_errno = errno;
	...
	if (pmState == PM_WAIT_BACKENDS)
	{
			...
			AutoVacPID == 0)
		{
			if (Shutdown >= ImmediateShutdown || FatalError)
			{
				 ...
				 * change causes GraphmasterServerLoop to stop creating new ones.
				 */
...
BackendInitialize(Port *port)
{
	...
	/* SIGQUIT handler was already set up by InitGraphmasterChild */
	 ...
	 *
	 * Note: because PostGraphMain will call InitializeTimeouts again, the
...
/*
 * BackendRun -- set up the backend's argument list and invoke PostGraphMain()
 *
 * returns:
...
BackendRun(Port *port)
{
	...
	PostGraphMain(ac, av, port->database_name, port->user_name);
}
 ...
 * initially go to PostGraphAuxiliaryProcessMain, which will handle common setup.
 *
...
StartChildProcess(AuxProcType type)
{
	...
	if (pid == 0)				/* child */
	{
		...
		PostGraphAuxiliaryProcessMain(ac, av);	/* does not return */
...
StartAutovacuumWorker(void)
{
	 ...
	 * for GraphmasterServerLoop to do it.  This way we avoid a ping-pong signaling in
 ...
 * As long as StartWorkerNeeded is true, GraphmasterServerLoop will not block and will
...
#define MAX_BGWORKERS_TO_LAUNCH 100
	...
	slist_foreach_modify(iter, &BackgroundWorkerList)
	{
		...
		if (bgworker_should_start_now(rw->rw_worker.bgw_start_time))
		{
			 ...
			 * of GraphmasterServerLoop to try again.  (We don't want to wait, because
			 ...
			 * GraphmasterServerLoop call us again to look for additional ready-to-run
...
#endif
{
	...
	param->GraphmasterPid = GraphmasterPid;
...
restore_backend_variables(BackendParameters *param, Port *port)
{
	...
	GraphmasterPid = param->GraphmasterPid;
...
InitPostmasterDeathWatchHandle(void)
{
	 ...
	 * write fd. That is taken care of in PostGraphClosePostmasterPorts().
	 */
	Assert(MyProcPid == GraphmasterPid);
