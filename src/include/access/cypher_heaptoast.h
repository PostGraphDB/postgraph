 ...
 *-------------------------------------------------------------------------
 */
#ifndef CYPHER_HEAPTOAST_H
#define CYPHER_HEAPTOAST_H
...
/* ----------
 * cypher_heap_toast_insert_or_update -
 *
 *	Called by heap_insert() and cypher_heap_update().
 * ----------
 */
extern HeapTuple cypher_heap_toast_insert_or_update(Relation rel, HeapTuple newtup,
											 HeapTuple oldtup, int options);

/* ----------
 * cypher_heap_toast_delete -
 *
 *	Called by heap_delete().
 * ----------
 */
extern void cypher_heap_toast_delete(Relation rel, HeapTuple oldtup,
							  bool is_speculative);

/* ----------
 * cypher_toast_flatten_tuple -
 *
 *	"Flatten" a tuple to contain no out-of-line toasted fields.
 *	(This does not eliminate compressed or short-header datums.)
 * ----------
 */
extern HeapTuple cypher_toast_flatten_tuple(HeapTuple tup, TupleDesc tupleDesc);

/* ----------
 * cypher_toast_flatten_tuple_to_datum -
 *
 *	"Flatten" a tuple containing out-of-line toasted fields into a Datum.
 * ----------
 */
extern Datum cypher_toast_flatten_tuple_to_datum(HeapTupleHeader tup,
										  uint32 tup_len,
										  TupleDesc tupleDesc);

/* ----------
 * cypher_toast_build_flattened_tuple -
 *
 *	Build a tuple containing no out-of-line toasted fields.
 *	(This does not eliminate compressed or short-header datums.)
 * ----------
 */
extern HeapTuple cypher_toast_build_flattened_tuple(TupleDesc tupleDesc,
											 Datum *values,
											 bool *isnull);

/* ----------
 * cypher_heap_fetch_toast_slice
 *
 *	Fetch a slice from a toast value stored in a heap table.
 * ----------
 */
extern void cypher_heap_fetch_toast_slice(Relation toastrel, Oid valueid,
