...
#include "access/cypher_rewriteheap.h"
						 ...
						 bool *call_again, bool *all_dead)
{
	...
	if (!*call_again)
	{
		...
		if (prev_buf != hscan->xs_cbuf)
			cypher_heap_page_prune_opt(hscan->xs_base.rel, hscan->xs_cbuf);
	}

	/* Obtain share-lock on the buffer so we can examine visibility */
	LockBuffer(hscan->xs_cbuf, BUFFER_LOCK_SHARE);
	got_heap_tuple = cypher_heap_hot_search_buffer(tid,
											hscan->xs_base.rel,
											hscan->xs_cbuf,
											snapshot,
											&bslot->base.tupdata,
						 ...
						 TupleTableSlot *slot)
{
	...
	bslot->base.tupdata.t_self = *tid;
	if (cypher_heap_fetch(relation, snapshot, &bslot->base.tupdata, &buffer))
	{
								...
								Snapshot snapshot)
{
	...
	/*
	 * We need buffer pin and lock to call CypherHeapTupleSatisfiesVisibility.
	 * Caller should be holding pin, but not lock.
	 */
	LockBuffer(bslot->buffer, BUFFER_LOCK_SHARE);
	res = CypherHeapTupleSatisfiesVisibility(bslot->base.tuple, snapshot,
									   bslot->buffer);
					...
					int options, BulkInsertState bistate)
{
	...
	cypher_heap_insert(relation, tuple, cid, options, bistate);
								  ...
								  uint32 specToken, bool succeeded)
{
	...
	if (succeeded)
		cypher_heap_finish_speculative(relation, &slot->tts_tid);
	else
		cypher_heap_abort_speculative(relation, &slot->tts_tid);
					...
					LockTupleMode *lockmode, bool *update_indexes)
{
	...
	result = cypher_heap_update(relation, otid, tuple, cid, crosscheck, wait,
						 tmfd, lockmode);
	ItemPointerCopy(&tuple->t_self, &slot->tts_tid);
	 ...
	 *
	 * Note: cypher_heap_update returns the tid (location) of the new tuple in the
	 * t_self field.
	 *
		...
		if (!ItemPointerEquals(&tmfd->ctid, &tuple->t_self))
		{
			...
			*tid = tmfd->ctid;
			...
			for (;;)
			{
				...
				tuple->t_self = *tid;
				if (cypher_heap_fetch_extended(relation, &SnapshotDirty, tuple,
										&buffer, true))
				{
					/*
					...
					if (TransactionIdIsValid(SnapshotDirty.xmax))
					{
						...
						continue;	/* loop back to repeat cypher_heap_fetch */
					}
								 ...
								 double *tups_recently_dead)
{
	...
	rwstate = cypher_begin_heap_rewrite(OldHeap, NewHeap, OldestXmin, *xid_cutoff,
								 *multi_cutoff);
	 ...
	 * CypherHeapTupleSatisfiesVacuum for the visibility test.
	 */
	...
	for (;;)
	{
		...
		switch (CypherHeapTupleSatisfiesVacuum(tuple, OldestXmin, buf))
		{
			case HEAPTUPLE_DEAD:
				/* Definitely dead */
				isdead = true;
		...
		if (isdead)
		{
			*tups_vacuumed += 1;
			...
			if (cypher_rewrite_heap_dead_tuple(rwstate, tuple))
			{
	...
	cypher_end_heap_rewrite(rwstate);
							   ...
							   TupleTableSlot *slot)
{
	...
	for (; hscan->rs_cindex <= maxoffset; hscan->rs_cindex++)
	{
		 ...
		 * cypher_heap_page_prune() counts things.
		 */
		...
		switch (CypherHeapTupleSatisfiesVacuum(targtuple, OldestXmin,
										 hscan->rs_cbuf))
		{
			case HEAPTUPLE_LIVE:
				sample_it = true;
							  ...
							  TableScanDesc scan)
{
	...
	if (!allow_sync)
		cypher_heap_setscanlimits(scan, start_blockno, numblocks);
	else
	{
	 ...
	 */
	while ((heapTuple = cypher_heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		bool		tupleIsAlive;
		 ...
		 * cypher_heap_get_root_tuples() to build a map of root item offsets on the
		 * page.
		 *
		...
		if (hscan->rs_cblock != root_blkno)
		{
			...
			LockBuffer(hscan->rs_cbuf, BUFFER_LOCK_SHARE);
			cypher_heap_get_root_tuples(page, root_offsets);
	...
	recheck:
			 ...
			 * tuples.
			 */
			switch (CypherHeapTupleSatisfiesVacuum(heapTuple, OldestXmin,
											 hscan->rs_cbuf))
			{
				case HEAPTUPLE_DEAD:
							  ...
							  TBMIterateResult *tbmres)
{
	 ...
	 */
	cypher_heap_page_prune_opt(scan->rs_rd, buffer);
	...
	if (tbmres->ntuples >= 0)
	{
		...
		for (curslot = 0; curslot < tbmres->ntuples; curslot++)
		{
			...
			if (cypher_heap_hot_search_buffer(&tid, scan->rs_rd, buffer, snapshot,
									   &heapTuple, NULL, true))
	...
	else
	{
		...
		for (offnum = FirstOffsetNumber; offnum <= maxoff; offnum = OffsetNumberNext(offnum))
		{
			...
			CypherHeapCheckForSerializableConflictOut(valid, scan->rs_rd, &loctup,
												buffer, snapshot);
		}
	}
...
heapam_scan_sample_next_block(TableScanDesc scan, SampleScanState *scanstate)
{
	...
	cypher_heapgetpage(scan, blockno);
							  ...
							  TupleTableSlot *slot)
{
	...
	for (;;)
	{
		...
		if (OffsetNumberIsValid(tupoffset))
		{
			...
			/* in pagemode, cypher_heapgetpage did this for us */
			if (!pagemode)
				CypherHeapCheckForSerializableConflictOut(visible, scan->rs_rd, tuple,
						 ...
						 Datum *values, bool *isnull, RewriteState rwstate)
{
	...
	cypher_rewrite_heap_tuple(rwstate, tuple, copiedTuple);
					   ...
					   OffsetNumber tupoffset)
{
	...
	{
		/*
		 * In pageatatime mode, cypher_heapgetpage() already did visibility checks,
	...
	else
	{
		...
		return CypherHeapTupleSatisfiesVisibility(tuple, scan->rs_snapshot,
											buffer);
	}
}
...
static const TableAmRoutine heapam_methods = {
	...
	.scan_begin = cypher_heap_beginscan,
	.scan_end = cypher_heap_endscan,
	.scan_rescan = cypher_heap_rescan,
	.scan_getnextslot = cypher_heap_getnextslot,

	.scan_set_tidrange = cypher_heap_set_tidrange,
	.scan_getnextslot_tidrange = cypher_heap_getnextslot_tidrange,
	...
	.multi_insert = cypher_heap_multi_insert,
	.tuple_delete = heapam_tuple_delete,
	.tuple_update = heapam_tuple_update,
	.tuple_lock = heapam_tuple_lock,

	.tuple_fetch_row_version = heapam_fetch_row_version,
	.tuple_get_latest_tid = cypher_heap_get_latest_tid,
	.tuple_tid_valid = heapam_tuple_tid_valid,
	.tuple_satisfies_snapshot = heapam_tuple_satisfies_snapshot,
	.index_delete_tuples = cypher_heap_index_delete_tuples,

	.relation_set_new_filenode = heapam_relation_set_new_filenode,
	.relation_nontransactional_truncate = heapam_relation_nontransactional_truncate,
	.relation_copy_data = heapam_relation_copy_data,
	.relation_copy_for_cluster = heapam_relation_copy_for_cluster,
	.relation_vacuum = cypher_heap_vacuum_rel,
	...
	.relation_fetch_toast_slice = cypher_heap_fetch_toast_slice,
