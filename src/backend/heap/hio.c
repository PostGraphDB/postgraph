...
/*
 * CypherRelationPutHeapTuple - place tuple at specified page
 *
 ...
 */
void
CypherRelationPutHeapTuple(Relation relation,
					 Buffer buffer,
					 HeapTuple tuple,
					 bool token)
{
					 ...
					 Buffer *vmbuffer1, Buffer *vmbuffer2)
{
	...
	while (1)
	{
		...
		need_to_pin_buffer1 = PageIsAllVisible(BufferGetPage(buffer1))
			&& !cypher_visibilitymap_pin_ok(block1, *vmbuffer1);
		need_to_pin_buffer2 = buffer2 != InvalidBuffer
			&& PageIsAllVisible(BufferGetPage(buffer2))
			&& !cypher_visibilitymap_pin_ok(block2, *vmbuffer2);
		...
		if (need_to_pin_buffer1)
			cypher_visibilitymap_pin(relation, block1, vmbuffer1);
		if (need_to_pin_buffer2)
			cypher_visibilitymap_pin(relation, block2, vmbuffer2);
...
RelationAddExtraBlocks(Relation relation, BulkInsertState bistate)
{
	...
	do
	{
		 ...
		 * extension code in CypherRelationGetBufferForTuple, except that we hold
...
/*
 * CypherRelationGetBufferForTuple
 *
 ...
 *	buffer will also be exclusive-locked.  (This case is used by cypher_heap_update;
 *	the otherBuffer contains the tuple being updated.)
 *
 *	The reason for passing otherBuffer is that if two backends are doing
 *	concurrent cypher_heap_update operations, a deadlock could occur if they try
 *	to lock the same two buffers in opposite orders.  To ensure that this
 *	can't happen, we impose the rule that buffers of a relation must be
 *	locked in increasing page number order.  This is most conveniently done
 *	by having CypherRelationGetBufferForTuple lock them both, with suitable care
 *	for ordering.
 *
 *	NOTE: it is unlikely, but not quite impossible, for otherBuffer to be the
 *	same buffer we select for insertion of the new tuple (this could only
 *	happen if space is freed in that page after cypher_heap_update finds there's not
 ...
 *	any committed data of other transactions.  (See cypher_heap_insert's comments
 ...
 */
Buffer
CypherRelationGetBufferForTuple(Relation relation, Size len,
...
loop:
	while (targetBlock != InvalidBlockNumber)
	{
		...
		if (otherBuffer == InvalidBuffer)
		{
			...
			if (PageIsAllVisible(BufferGetPage(buffer)))
				cypher_visibilitymap_pin(relation, targetBlock, vmbuffer);
	...
	if (options & HEAP_INSERT_FROZEN)
	{
		...
		cypher_visibilitymap_pin(relation, BufferGetBlockNumber(buffer), vmbuffer);
	}
