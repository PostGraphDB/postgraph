 ...
 *
 * INTERFACE ROUTINES
 *		cypher_heap_beginscan	- begin relation scan
 *		heap_rescan		- restart a relation scan
 *		cypher_heap_endscan	- end relation scan
 *		heap_getnext	- retrieve next tuple in scan
 *		cypher_heap_fetch		- retrieve tuple with given tid
 *		cypher_heap_insert		- insert tuple into a relation
 *		cypher_heap_multi_insert - insert multiple tuples into a relation
 *		cypher_heap_delete		- delete a tuple from a relation
 *		cypher_heap_update		- replace a tuple in a relation with another tuple
 *
 * NOTES
...
#include "access/cypher_heapam_xlog.h"
#include "access/cypher_heaptoast.h"
#include "access/cypher_hio.h"
...
#include "access/cypher_visibilitymap.h"
...
static CYPHER_TM_Result heap_lock_updated_tuple(Relation rel, HeapTuple tuple,
...
#ifdef USE_PREFETCH
/*
 * cypher_heap_index_delete_tuples and index_delete_prefetch_buffer use this
...
/* cypher_heap_index_delete_tuples bottom-up index deletion costing constants */
#define BOTTOMUP_MAX_NBLOCKS			6
#define BOTTOMUP_TOLERANCE_NBLOCKS		3

/*
 * cypher_heap_index_delete_tuples uses this when determining which heap blocks it
...
/* ----------------
 *		initscan - scan code common to cypher_heap_beginscan and heap_rescan
 * ----------------
 */
static void
...
/*
 * cypher_heap_setscanlimits - restrict range of a heapscan
 *
 * startBlk is the page to start at
 * numBlks is number of pages to scan (InvalidBlockNumber means "all")
 */
void
cypher_heap_setscanlimits(TableScanDesc sscan, BlockNumber startBlk, BlockNumber numBlks)
{
...
/*
 * cypher_heapgetpage - subroutine for heapgettup()
 *
 ...
 */
void
cypher_heapgetpage(TableScanDesc sscan, BlockNumber page)
{
	HeapScanDesc scan = (HeapScanDesc) sscan;
	Buffer		buffer;
	Snapshot	snapshot;
	 ...
	 */
	cypher_heap_page_prune_opt(scan->rs_base.rs_rd, buffer);
		 ...
		 lineoff++, lpp++)
	{
		if (ItemIdIsNormal(lpp))
		{
			...
			if (all_visible)
				...
				valid = CypherHeapTupleSatisfiesVisibility(&loctup, snapshot, buffer);

			CypherHeapCheckForSerializableConflictOut(valid, scan->rs_base.rs_rd,
		   ...
		   ScanKey key)
{
	...
	if (ScanDirectionIsForward(dir))
	{
		if (!scan->rs_inited)
		{
			...
			cypher_heapgetpage((TableScanDesc) scan, page);
	...
	else if (backward)
	{
		...
		if (!scan->rs_inited)
		{
			 ...
			 * rs_numblocks if it's been adjusted by cypher_heap_setscanlimits().
			 */
	...
	for (;;)
	{
		...
		while (linesleft > 0)
		{
			if (ItemIdIsNormal(lpp))
			{
				 ...
				 */
				valid = CypherHeapTupleSatisfiesVisibility(tuple,
													 snapshot,
													 scan->rs_cbuf);
 ...
 * happen inside cypher_heapgetpage), and we iterate through just the tuples listed
...
TableScanDesc
cypher_heap_beginscan(Relation relation, Snapshot snapshot,
			   ...
			   uint32 flags)
{
	...
	/*
	 * we do this here instead of in initscan() because cypher_heap_rescan also calls
...
void
cypher_heap_rescan(TableScanDesc sscan, ScanKey key, bool set_params,
...
void
cypher_heap_endscan(TableScanDesc sscan)
{
	HeapScanDesc scan = (HeapScanDesc) sscan;
...
HeapTuple
cypher_heap_getnext(TableScanDesc sscan, ScanDirection direction)
{
	HeapScanDesc scan = (HeapScanDesc) sscan;
...
bool
cypher_heap_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	HeapScanDesc scan = (HeapScanDesc) sscan;
...
void
cypher_heap_set_tidrange(TableScanDesc sscan, ItemPointer mintid,
				  ItemPointer maxtid)
{
	HeapScanDesc scan = (HeapScanDesc) sscan;
	...
	if (ItemPointerCompare(&highestItem, &lowestItem) < 0)
	{
		...
		cypher_heap_setscanlimits(sscan, 0, 0);
		return;
	}
	...
	cypher_heap_setscanlimits(sscan, startBlk, numBlks);
...
bool
cypher_heap_getnextslot_tidrange(TableScanDesc sscan, ScanDirection direction,
						  TupleTableSlot *slot)
{
	HeapScanDesc scan = (HeapScanDesc) sscan;
	...
	for (;;)
	{
		...
		/*
		 * cypher_heap_set_tidrange will have used cypher_heap_setscanlimits to limit the
 ...
 *
 * cypher_heap_fetch does not follow HOT chains: only the exact TID requested will
 * be fetched.
 *
 ...
 */
bool
cypher_heap_fetch(Relation relation,
		   Snapshot snapshot,
		   HeapTuple tuple,
		   Buffer *userbuf)
{
	return cypher_heap_fetch_extended(relation, snapshot, tuple, userbuf, false);
}

/*
 *	cypher_heap_fetch_extended		- fetch tuple even if it fails snapshot test
 *
 ...
 * If keep_buf is false then this behaves identically to cypher_heap_fetch().
 */
bool
cypher_heap_fetch_extended(Relation relation,
					Snapshot snapshot,
					HeapTuple tuple,
					Buffer *userbuf,
					bool keep_buf)
{
	 ...
	 */
	valid = CypherHeapTupleSatisfiesVisibility(tuple, snapshot, buffer);

	if (valid)
		PredicateLockTID(relation, &(tuple->t_self), snapshot,
						 HeapTupleHeaderGetXmin(tuple->t_data));

	CypherHeapCheckForSerializableConflictOut(valid, relation, tuple, buffer, snapshot);
...
/*
 *	cypher_heap_hot_search_buffer	- search HOT chain for tuple satisfying snapshot
 *
 ...
 *
 * Unlike cypher_heap_fetch, the caller must already have pin and (at least) share
 * lock on the buffer; it is still pinned/locked at exit.
 */
bool
cypher_heap_hot_search_buffer(ItemPointer tid, Relation relation, Buffer buffer,
					   ...
					   bool *all_dead, bool first_call)
{
	...
	for (;;)
	{
		...
		if (!skip)
		{
			...
			valid = CypherHeapTupleSatisfiesVisibility(heapTuple, snapshot, buffer);
			CypherHeapCheckForSerializableConflictOut(valid, relation, heapTuple,
												buffer, snapshot);
		...
		if (all_dead && *all_dead)
		{
			...
			if (!CypherHeapTupleIsSurelyDead(heapTuple, vistest))
				*all_dead = false;
		}
...
/*
 *	cypher_heap_get_latest_tid -  get the latest tid of a specified tuple
 *
 ...
 */
void
cypher_heap_get_latest_tid(TableScanDesc sscan,
					ItemPointer tid)
{
	...
	for (;;)
	{
		 ...
		 * candidate.
		 */
		valid = CypherHeapTupleSatisfiesVisibility(&tp, snapshot, buffer);
		CypherHeapCheckForSerializableConflictOut(valid, relation, &tp, buffer, snapshot);
		if (valid)
			*tid = ctid;
		...
		if ((tp.t_data->t_infomask & HEAP_XMAX_INVALID) ||
			CypherHeapTupleHeaderIsOnlyLocked(tp.t_data) ||
...
UpdateXmaxHintBits(HeapTupleHeader tuple, Buffer buffer, TransactionId xid)
{
	...
	if (!(tuple->t_infomask & (HEAP_XMAX_COMMITTED | HEAP_XMAX_INVALID)))
	{
		if (!HEAP_XMAX_IS_LOCKED_ONLY(tuple->t_infomask) &&
			...
			CypherHeapTupleSetHintBits(tuple, buffer, HEAP_XMAX_COMMITTED,
								 xid);
		else
			CypherHeapTupleSetHintBits(tuple, buffer, HEAP_XMAX_INVALID,
...
/*
 * CypherGetBulkInsertState - prepare status object for a bulk insert
 */
BulkInsertState
CypherGetBulkInsertState(void)
{
...
/*
 * CypherFreeBulkInsertState - clean up after finishing a bulk insert
 */
void
CypherFreeBulkInsertState(BulkInsertState bistate)
{
...
/*
 * CypherReleaseBulkInsertStatePin - release a buffer currently held in bistate
 */
void
CypherReleaseBulkInsertStatePin(BulkInsertState bistate)
{
	if (bistate->current_buf != InvalidBuffer)
		ReleaseBuffer(bistate->current_buf);
	bistate->current_buf = InvalidBuffer;
...
/*
 *	cypher_heap_insert		- insert tuple into a heap
 *
 ...
 */
void
cypher_heap_insert(Relation relation, HeapTuple tup, CommandId cid,
			int options, BulkInsertState bistate)
{
	 ...
	 */
	buffer = CypherRelationGetBufferForTuple(relation, heaptup->t_len,
	...
	CypherRelationPutHeapTuple(relation, buffer, heaptup,
	...
	if (PageIsAllVisible(BufferGetPage(buffer)))
	{
		...
		cypher_visibilitymap_clear(relation,
	...
	if (RelationNeedsWAL(relation))
	{
		...
		uint8		info = CYPHER_XLOG_HEAP_INSERT;
		int			bufflags = 0;
...
/*
 * Subroutine for cypher_heap_insert(). Prepares a tuple for insertion. This sets the
					...
					CommandId cid, int options)
{
		...
		relation->rd_rel->relkind != RELKIND_MATVIEW)
	{
		...
		return cypher_heap_toast_insert_or_update(relation, tup, NULL, options);
	else
		return tup;
}

/*
 *	cypher_heap_multi_insert	- insert multiple tuples into a heap
 *
 * This is like cypher_heap_insert(), but inserts multiple tuples in one operation.
 * That's faster than calling cypher_heap_insert() in a loop, because when multiple
 ...
 */
void
cypher_heap_multi_insert(Relation relation, TupleTableSlot **slots, int ntuples,
				  CommandId cid, int options, BulkInsertState bistate)
{
	...
	/* currently not needed (thus unsupported) for cypher_heap_multi_insert() */
	...
	while (ndone < ntuples)
	{
		 ...
		 */
		buffer = CypherRelationGetBufferForTuple(relation, heaptuples[ndone]->t_len,
										   InvalidBuffer, options, bistate,
										   &vmbuffer, NULL);
		page = BufferGetPage(buffer);
		...
		/*
		 * CypherRelationGetBufferForTuple has ensured that the first tuple fits.
		 * Put that on the page, and then as many other tuples as fit.
		 */
		CypherRelationPutHeapTuple(relation, buffer, heaptuples[ndone], false);
		...
		for (nthispage = 1; ndone + nthispage < ntuples; nthispage++)
		{
			...
			CypherRelationPutHeapTuple(relation, buffer, heaptup, false);
		...
		/*
		 * XXX Should we set PageSetPrunable on this page ? See cypher_heap_insert()
		 */
		...
		if (needwal)
		{
			 ...
			 * emitted by this call to cypher_heap_multi_insert(). Needed for logical
		...
		if (all_frozen_set)
		{
			...
			Assert(cypher_visibilitymap_pin_ok(BufferGetBlockNumber(buffer), vmbuffer));
			 ...
			 */
			cypher_visibilitymap_set(relation, BufferGetBlockNumber(buffer), buffer,
...
/*
 *	cypher_simple_heap_insert - insert a tuple
 *
 * Currently, this routine differs from cypher_heap_insert only in supplying
 * a default command ID and not allowing access to the speedup options.
 *
 * This should be used rather than using cypher_heap_insert directly in most places
 * where we are modifying system catalogs.
 */
void
cypher_simple_heap_insert(Relation relation, HeapTuple tup)
{
	cypher_heap_insert(relation, tup, GetCurrentCommandId(true), 0, NULL);
}
...
/*
 *	cypher_heap_delete - delete a tuple
 *
 ...
 * only for CYPHER_TM_SelfModified, since we cannot obtain cmax from a combo CID
 * generated by another transaction).
 */
CYPHER_TM_Result
cypher_heap_delete(Relation relation, ItemPointer tid,
			...
			TM_FailureData *tmfd, bool changingPart)
{
	...
	if (PageIsAllVisible(page))
		cypher_visibilitymap_pin(relation, block, &vmbuffer);
...
l1:
	...
	result = CypherHeapTupleSatisfiesUpdate(&tp, cid, buffer);



	if (result == CYPHER_TM_Invisible)
	{
		UnlockReleaseBuffer(buffer);
		ereport(ERROR,
	...
	else if (result == CYPHER_TM_BeingModified && wait)
	{
		TransactionId xwait;
		uint16		infomask;
		 ...
		 * priority for the tuple (see cypher_heap_lock_tuple).  LockTuple will
		...
		if ((tp.t_data->t_infomask & HEAP_XMAX_INVALID) ||
			...
			result = CYPHER_TM_Updated;
		else
			result = CYPHER_TM_Deleted;
	}


	/* sanity check the result CypherHeapTupleSatisfiesUpdate() and the logic above */
	if (result != CYPHER_TM_Ok)
	{
		Assert(result == CYPHER_TM_SelfModified ||
			   result == CYPHER_TM_Updated ||
			   result == CYPHER_TM_Deleted ||
			   result == CYPHER_TM_BeingModified);
		Assert(!(tp.t_data->t_infomask & HEAP_XMAX_INVALID));
		Assert(result != CYPHER_TM_Updated ||
			   !ItemPointerEquals(&tp.t_self, &tp.t_data->t_ctid));
	}

	if (crosscheck != InvalidSnapshot && result == CYPHER_TM_Ok)
	{
		/* Perform additional check for transaction-snapshot mode RI updates */
		if (!CypherHeapTupleSatisfiesVisibility(&tp, crosscheck, buffer))
			result = CYPHER_TM_Updated;
	}
	...
	if (PageIsAllVisible(page))
	{
		all_visible_cleared = true;
		PageClearAllVisible(page);
		cypher_visibilitymap_clear(relation, BufferGetBlockNumber(buffer),
							vmbuffer, VISIBILITYMAP_VALID_BITS);
	}
	 ...
	 * XLOG stuff
	 *
	 * NB: cypher_heap_abort_speculative() uses the same xlog record and replay
	 * routines.
	 */
	if (RelationNeedsWAL(relation))
	{
		...
		recptr = XLogInsert(RM_HEAP_ID, CYPHER_XLOG_HEAP_DELETE);
		...
		relation->rd_rel->relkind != RELKIND_MATVIEW)
	{
		...
		cypher_heap_toast_delete(relation, &tp, false);
...
/*
 *	cypher_simple_heap_delete - delete a tuple
 *
 ...
 */
void
cypher_simple_heap_delete(Relation relation, ItemPointer tid)
{
	CYPHER_TM_Result	result;
	TM_FailureData tmfd;

	result = cypher_heap_delete(relation, tid,
	...
	switch (result)
	{
		...
		case CYPHER_TM_Deleted:
			elog(ERROR, "tuple concurrently deleted");
			break;

		default:
			elog(ERROR, "unrecognized cypher_heap_delete status: %u", result);
			break;
	}
}

/*
 *	cypher_heap_update - replace a tuple
 *
 ...
 */
CYPHER_TM_Result
cypher_heap_update(Relation relation, ItemPointer otid, HeapTuple newtup,
			CommandId cid, Snapshot crosscheck, bool wait,
...
l2:
	...
	result = CypherHeapTupleSatisfiesUpdate(&oldtup, cid, buffer);

//BlackPink
    /*if (result == CYPHER_TM_PrevClauseUpdated || result == CYPHER_TM_SelfModified || result == CYPHER_TM_BeingModified ) {
		ereport(ERROR, errmsg("Here %i", cid));
		//ereport(ERROR, errmsg("Here %i %i", HeapTupleHeaderGetCmax(&oldtup), cid));
	}*/

	/* see below about the "no wait" case */
	Assert(result != CYPHER_TM_BeingModified || wait);
	...
	{
		/* Perform additional check for transaction-snapshot mode RI updates */
		if (!CypherHeapTupleSatisfiesVisibility(&oldtup, crosscheck, buffer)) {	
			result = CYPHER_TM_Updated;
		}
	}
	...
	if (need_toast || newtupsize > pagefree)
	{
		 ...
		 */
		if (PageIsAllVisible(page) &&
			cypher_visibilitymap_clear(relation, block, vmbuffer,
		...
		if (RelationNeedsWAL(relation))
		{
			...
			recptr = XLogInsert(RM_HEAP_ID, CYPHER_XLOG_HEAP_LOCK);
			PageSetLSN(page, recptr);
		}
		...
		if (need_toast)
		{
			...
			heaptup = cypher_heap_toast_insert_or_update(relation, newtup, &oldtup, 0);
		 ...
		 * first".  To implement this, we must do CypherRelationGetBufferForTuple
		 ...
		 * call CypherRelationGetBufferForTuple, we rely on it to acquire any such
		...
		for (;;)
		{
			...
			{
				/* It doesn't fit, must use CypherRelationGetBufferForTuple. */
				newbuf = CypherRelationGetBufferForTuple(relation, heaptup->t_len,
												   buffer, 0, NULL,
	...
	// 
	CypherRelationPutHeapTuple(relation, newbuf, heaptup, false); /* insert new tuple */
	...
	if (newbuf != buffer && PageIsAllVisible(BufferGetPage(newbuf)))
	{
		...
		cypher_visibilitymap_clear(relation, BufferGetBlockNumber(newbuf),
 ...
 * since this is invoked at most once in cypher_heap_update.
 */
static Bitmapset *
...
/*
 *	cypher_simple_heap_update - replace a tuple
 *
 ...
 */
void
cypher_simple_heap_update(Relation relation, ItemPointer otid, HeapTuple tup)
{
	CYPHER_TM_Result	result;
	TM_FailureData tmfd;
	LockTupleMode lockmode;

	result = cypher_heap_update(relation, otid, tup,
						 GetCurrentCommandId(true), InvalidSnapshot,
						 true /* wait for commit */ ,
						 &tmfd, &lockmode);
	switch (result)
	{
		...
		default:
			elog(ERROR, "unrecognized cypher_heap_update status: %u", result);
			break;
	}
}
 ...
 *
 * In the failure cases other than CYPHER_TM_Invisible, the routine fills
 * *tmfd with the tuple's t_ctid, t_xmax (resolving a possible MultiXact,
 * if necessary), and t_cmax (the last only for CYPHER_TM_SelfModified,
 ...
 */
CYPHER_TM_Result
cypher_heap_lock_tuple(Relation relation, HeapTuple tuple,
...
l3:
	result = CypherHeapTupleSatisfiesUpdate(tuple, cid, *buffer);
	...
	if (result == CYPHER_TM_Invisible)
	{
		 ...
		 * error.
		 */
		result = CYPHER_TM_Invisible;
		goto out_locked;
	}
	else if (result == CYPHER_TM_BeingModified ||
			 result == CYPHER_TM_Updated ||
			 result == CYPHER_TM_Deleted)
	{
		TransactionId xwait;
		...
		if (first_time)
		{
			...
			if (infomask & HEAP_XMAX_IS_MULTI)
			{
				 ...
				 * been the case, CypherHeapTupleSatisfiesUpdate would have returned
		...
		if (mode == LockTupleKeyShare)
		{
			...
			if (!(infomask2 & HEAP_KEYS_UPDATED))
			{
				 ...
				 * versions.
				 */
				if (!CypherHeapTupleHeaderIsOnlyLocked(tuple->t_data) &&
		 ...
		 */
		if (require_sleep && (result == CYPHER_TM_Updated || result == CYPHER_TM_Deleted))
		{
			LockBuffer(*buffer, BUFFER_LOCK_EXCLUSIVE);
			goto failed;
		}
		...
		if (!require_sleep ||
			...
			CypherHeapTupleHeaderIsOnlyLocked(tuple->t_data))
			result = CYPHER_TM_Ok;
...
failed:
	if (result != CYPHER_TM_Ok)
	{
		Assert(result == CYPHER_TM_SelfModified || result == CYPHER_TM_Updated ||
			   result == CYPHER_TM_Deleted || result == CYPHER_TM_WouldBlock);

		/*
		 * When locking a tuple under LockWaitSkip semantics and we fail with
		 * CYPHER_TM_WouldBlock above, it's possible for concurrent transactions to
		 ...
		 * heap_delete and cypher_heap_update.
		 */
		Assert((result == CYPHER_TM_WouldBlock) ||
			   !(tuple->t_data->t_infomask & HEAP_XMAX_INVALID));
		Assert(result != CYPHER_TM_Updated ||
 ...
 *
 * Most callers will have called CypherHeapTupleSatisfiesUpdate before this function;
 ...
 * CYPHER_TM_SelfModified.  In case of a conflict with another transaction, a
 * different CypherHeapTupleSatisfiesUpdate return code is returned.
 *
							...
							LockTupleMode mode)
{
	...
	for (;;)
	{
		...
		if (!cypher_heap_fetch(rel, SnapshotAny, &mytup, &buf))
		{
			/*
...
l4:
		...
		{
			cypher_visibilitymap_pin(rel, block, &vmbuffer);
		...
		if (PageIsAllVisible(BufferGetPage(buf)) &&
			cypher_visibilitymap_clear(rel, block, vmbuffer,
								VISIBILITYMAP_ALL_FROZEN))
			cleared_all_frozen = true;
...
next:
		...
		if (mytup.t_data->t_infomask & HEAP_XMAX_INVALID ||
			...
			CypherHeapTupleHeaderIsOnlyLocked(mytup.t_data))
		{
			result = CYPHER_TM_Ok;
			goto out_locked;
		}
...
/*
 *	cypher_heap_finish_speculative - mark speculative insertion as successful
 *
 ...
 */
void
cypher_heap_finish_speculative(Relation relation, ItemPointer tid)
{
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	...
	if (RelationNeedsWAL(relation))
	{
		...
		recptr = XLogInsert(RM_HEAP_ID, CYPHER_XLOG_HEAP_CONFIRM);
...
/*
 *	cypher_heap_abort_speculative - kill a speculatively inserted tuple
 *
 ...
 */
void
cypher_heap_abort_speculative(Relation relation, ItemPointer tid)
{
	TransactionId xid = GetCurrentTransactionId();
	ItemId		lp;
	HeapTupleData tp;
	...
	if (HeapTupleHasExternal(&tp))
	{
		...
		cypher_heap_toast_delete(relation, &tp, true);
	}
...
/*
 * cypher_heap_inplace_update - update a tuple "in place" (ie, overwrite it)
 *
 ...
 */
void
cypher_heap_inplace_update(Relation relation, HeapTuple tuple)
{
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
	...
	if (RelationNeedsWAL(relation))
	{
		...
		recptr = XLogInsert(RM_HEAP_ID, CYPHER_XLOG_HEAP_INPLACE);
	...
	*flags = 0;
	...
	for (i = 0; i < nmembers; i++)
	{
		...
		if (ISUPDATE_from_mxstatus(members[i].status))
		{
			 ...
			 * committed, because CypherHeapTupleSatisfiesVacuum would have returned
...
/*
 * cypher_heap_prepare_freeze_tuple
 *
 ...
 * CypherHeapTupleSatisfiesVacuum() and determined that it is not HEAPTUPLE_DEAD
 ...
 */
bool
cypher_heap_prepare_freeze_tuple(HeapTupleHeader tuple,
						  TransactionId relfrozenxid, TransactionId relminmxid,
						  ...
						  xl_heap_freeze_tuple *frz, bool *totally_frozen_p)
{
	 ...
	 *
	 * Make sure to keep cypher_heap_tuple_needs_freeze in sync with this.
	 */
...
/*
 * cypher_heap_execute_freeze_tuple
 ...
 */
void
cypher_heap_execute_freeze_tuple(HeapTupleHeader tuple, xl_heap_freeze_tuple *frz)
{
...
/*
 * cypher_heap_freeze_tuple
 *		Freeze tuple in place, without WAL logging.
 *
 * Useful for callers like CLUSTER that perform their own WAL logging.
 */
bool
cypher_heap_freeze_tuple(HeapTupleHeader tuple,
				  TransactionId relfrozenxid, TransactionId relminmxid,
				  TransactionId cutoff_xid, TransactionId cutoff_multi)
{
	...
	do_freeze = cypher_heap_prepare_freeze_tuple(tuple,
										  relfrozenxid, relminmxid,
	...
	if (do_freeze)
		cypher_heap_execute_freeze_tuple(tuple, &frz);
	return do_freeze;
}
...
/*
 * cypher_heap_tuple_needs_eventual_freeze
 *
 * Check to see whether any of the XID fields of a tuple (xmin, xmax, xvac)
 * will eventually require freezing.  Similar to cypher_heap_tuple_needs_freeze,
 * but there's no cutoff, since we're trying to figure out whether freezing
 * will ever be needed, not whether it's needed now.
 */
bool
cypher_heap_tuple_needs_eventual_freeze(HeapTupleHeader tuple)
{
	TransactionId xid;
...
/*
 * cypher_heap_tuple_needs_freeze
 *
 * Check to see whether any of the XID fields of a tuple (xmin, xmax, xvac)
 ...
 */
bool
cypher_heap_tuple_needs_freeze(HeapTupleHeader tuple, TransactionId cutoff_xid,
						MultiXactId cutoff_multi, Buffer buf)
{
	 ...
	 * cypher_heap_prepare_freeze_tuple for justifications.  This routine had better
 ...
 */
void
CypherHeapTupleHeaderAdvanceLatestRemovedXid(HeapTupleHeader tuple,
...
#ifdef USE_PREFETCH
/*
 * Helper function for cypher_heap_index_delete_tuples.  Issues prefetch requests for
 ...
 */
TransactionId
cypher_heap_index_delete_tuples(Relation rel, TM_IndexDeleteOp *delstate)
{
	...
	for (int i = 0; i < delstate->ndeltids; i++)
	{
		...
		else
		{
			...
			if (cypher_heap_hot_search_buffer(&tmp, rel, buf, &SnapshotNonVacuumable,
									   &heapTuple, NULL, true))
		...
		for (;;)
		{
			...
			CypherHeapTupleHeaderAdvanceLatestRemovedXid(htup, &latestRemovedXid);
 ...
 * processing by cypher_heap_index_delete_tuples().
 *
 ...
 * cypher_heap_index_delete_tuples() will expect this.
 *
 * Caller passes blockgroups, a description of the final order that deltids
 * will be sorted in for cypher_heap_index_delete_tuples() bottom-up index deletion
 ...
 * cypher_heap_update() call) will have the same heap TID in each and every index.
...
/*
 * cypher_heap_index_delete_tuples() helper function for bottom-up deletion callers.
 *
 ...
 */
XLogRecPtr
cypher_log_heap_freeze(Relation reln, Buffer buffer, TransactionId cutoff_xid,
 ...
 */
XLogRecPtr
cypher_log_heap_visible(RelFileNode rnode, Buffer heap_buffer, Buffer vm_buffer,
				...
				bool all_visible_cleared, bool new_all_visible_cleared)
{
	...
	if (HeapTupleIsHeapOnly(newtup))
		info = CYPHER_XLOG_HEAP_HOT_UPDATE;
	else
		info = CYPHER_XLOG_HEAP_UPDATE;
					   ...
					   bool *copy)
{
	...
	*copy = false;
	...
	if (replident == REPLICA_IDENTITY_FULL)
	{
		...
		{
			*copy = true;
			tp = cypher_toast_flatten_tuple(tp, desc);
		}
		return tp;
	}
	 ...
	 * (This case should not be reachable from cypher_heap_update, since that should
	...
	*copy = true;
	 ...
	 * duplicate cypher_toast_flatten_tuple()s functionality in the above loop over
	...
	if (HeapTupleHasExternal(key_tuple))
	{
		...
		key_tuple = cypher_toast_flatten_tuple(oldtup, desc);
...
heap_xlog_prune(XLogReaderState *record)
{
	...
	if (action == BLK_NEEDS_REDO)
	{
		...
		cypher_heap_page_prune_execute(buffer,
...
heap_xlog_visible(XLogReaderState *record)
{
									  ...
									  &vmbuffer) == BLK_NEEDS_REDO)
	{
		 ...
		 * cypher_visibilitymap_set will handle locking itself.
		 */
		LockBuffer(vmbuffer, BUFFER_LOCK_UNLOCK);

		reln = CreateFakeRelcacheEntry(rnode);
		cypher_visibilitymap_pin(reln, blkno, &vmbuffer);
		...
		if (lsn > PageGetLSN(vmpage))
			cypher_visibilitymap_set(reln, blkno, InvalidBuffer, lsn, vmbuffer,
...
heap_xlog_freeze_page(XLogReaderState *record)
{
	...
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		for (ntup = 0; ntup < xlrec->ntuples; ntup++)
		{
			...
			cypher_heap_execute_freeze_tuple(tuple, xlrec_tp);
		}
...
heap_xlog_delete(XLogReaderState *record)
{
	...
	if (xlrec->flags & XLH_DELETE_ALL_VISIBLE_CLEARED)
	{
		...
		cypher_visibilitymap_pin(reln, blkno, &vmbuffer);
		cypher_visibilitymap_clear(reln, blkno, vmbuffer, VISIBILITYMAP_VALID_BITS);
		ReleaseBuffer(vmbuffer);
		FreeFakeRelcacheEntry(reln);
	}
...
heap_xlog_update(XLogReaderState *record, bool hot_update)
{
	...
	if (xlrec->flags & XLH_UPDATE_OLD_ALL_VISIBLE_CLEARED)
	{
		...
		cypher_visibilitymap_pin(reln, oldblk, &vmbuffer);
		cypher_visibilitymap_clear(reln, oldblk, vmbuffer, VISIBILITYMAP_VALID_BITS);
		ReleaseBuffer(vmbuffer);
		FreeFakeRelcacheEntry(reln);
	}
	...
	if (xlrec->flags & XLH_UPDATE_NEW_ALL_VISIBLE_CLEARED)
	{
		...
		cypher_visibilitymap_pin(reln, newblk, &vmbuffer);
		cypher_visibilitymap_clear(reln, newblk, vmbuffer, VISIBILITYMAP_VALID_BITS);
		ReleaseBuffer(vmbuffer);
		FreeFakeRelcacheEntry(reln);
	}
...
heap_xlog_lock(XLogReaderState *record)
{
	...
	if (xlrec->flags & XLH_LOCK_ALL_FROZEN_CLEARED)
	{
		...
		cypher_visibilitymap_pin(reln, block, &vmbuffer);
		cypher_visibilitymap_clear(reln, block, vmbuffer, VISIBILITYMAP_ALL_FROZEN);
...
void
cypher_heap_redo(XLogReaderState *record)
{
	...
	{
		case CYPHER_XLOG_HEAP_INSERT:
			heap_xlog_insert(record);
			break;
		case CYPHER_XLOG_HEAP_DELETE:
			heap_xlog_delete(record);
			break;
		case CYPHER_XLOG_HEAP_UPDATE:
			heap_xlog_update(record, false);
			break;
		case CYPHER_XLOG_HEAP_TRUNCATE:
			 ...
			 */
			break;
		case CYPHER_XLOG_HEAP_HOT_UPDATE:
			heap_xlog_update(record, true);
			break;
		case CYPHER_XLOG_HEAP_CONFIRM:
			heap_xlog_confirm(record);
			break;
		case CYPHER_XLOG_HEAP_LOCK:
			heap_xlog_lock(record);
			break;
		case CYPHER_XLOG_HEAP_INPLACE:
...
void
cypher_heap2_redo(XLogReaderState *record)
{
	uint8		info = XLogRecGetInfo(record) & ~XLR_INFO_MASK;
	...
	switch (info & XLOG_HEAP_OPMASK)
	{
		...
		case XLOG_HEAP2_REWRITE:
			cypher_heap_xlog_logical_rewrite(record);
			break;
		default:
			elog(PANIC, "cypher_heap2_redo: unknown op code %u", info);
	}
}
 ...
 */
void
cypher_heap_mask(char *pagedata, BlockNumber blkno)
{
	...
	for (off = 1; off <= PageGetMaxOffsetNumber(page); off++)
	{
		...
		if (ItemIdIsNormal(iid))
		{
			...
			/*
			 * For a speculative tuple, cypher_heap_insert() does not set ctid in the
...
/*
 * CypherHeapCheckForSerializableConflictOut
 ...
 */
void
CypherHeapCheckForSerializableConflictOut(bool visible, Relation relation,
									...
									Snapshot snapshot)
{
	 ...
	 * tuple is visible to us, while CypherHeapTupleSatisfiesVacuum checks what else
	 * is going on with it.
	 *
	 ...
	 */
	htsvResult = CypherHeapTupleSatisfiesVacuum(tuple, TransactionXmin, buffer);
	switch (htsvResult)
	{
		case HEAPTUPLE_LIVE:
			if (visible)
		...
		default:
			 ...
			 */
			elog(ERROR, "unrecognized return value from CypherHeapTupleSatisfiesVacuum: %u", htsvResult);
