...
/*
 *	cypher_heap_vacuum_rel() -- perform VACUUM for one heap relation
 *
 ...
 */
void
cypher_heap_vacuum_rel(Relation rel, VacuumParams *params,
				BufferAccessStrategy bstrategy)
{
	LVRelState *vacrel;
	PGRUsage	ru0;
	...
	cypher_visibilitymap_count(rel, &new_rel_allvisible, NULL);
 ...
 *		page, and set commit status bits (see cypher_heap_page_prune).  It also builds
...
lazy_scan_heap(LVRelState *vacrel, VacuumParams *params, bool aggressive)
{
	 ...
	 * this stage.
	 *
	 * Note: The value returned by cypher_visibilitymap_get_status could be slightly
	...
	if ((params->options & VACOPT_DISABLE_PAGE_SKIPPING) == 0)
	{
		while (next_unskippable_block < nblocks)
		{
			...
			vmstatus = cypher_visibilitymap_get_status(vacrel->rel,
												next_unskippable_block,
												&vmbuffer);
			if (aggressive)
			{
...
#define FORCE_CHECK_PAGE() \
		...
		if (blkno == next_unskippable_block)
		{
			...
			if ((params->options & VACOPT_DISABLE_PAGE_SKIPPING) == 0)
			{
				while (next_unskippable_block < nblocks)
				{
					...
					vmskipflags = cypher_visibilitymap_get_status(vacrel->rel,
														   next_unskippable_block,
														   &vmbuffer);
					if (aggressive)
					{
		 ...
		 */
		cypher_visibilitymap_pin(vacrel->rel, blkno, &vmbuffer);
		...
		if (PageIsEmpty(page))
		{
			...
			if (!PageIsAllVisible(page))
			{
				...
				cypher_visibilitymap_set(vacrel->rel, blkno, buf, InvalidXLogRecPtr,
		...
		if (!all_visible_according_to_vm && prunestate.all_visible)
		{
			 ...
			 * dirtied before passing it to cypher_visibilitymap_set(), because it
				 ...
				 && VM_ALL_VISIBLE(vacrel->rel, blkno, &vmbuffer))
		{
			...
			cypher_visibilitymap_clear(vacrel->rel, blkno, vmbuffer,
	 ...
	 *
	 * The autovacuum specific logging in cypher_heap_vacuum_rel summarizes an entire
 ...
 *
 * Prior to PostgreSQL 14 there were very rare cases where cypher_heap_page_prune()
 ...
 * inserting transaction concurrently aborted (after our cypher_heap_page_prune()
 ...
 * detected.  This allows cypher_heap_page_prune() to prune the tuples inserted by
...
retry:
	 ...
	 */
	tuples_deleted = cypher_heap_page_prune(rel, buf, vistest,
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		 ...
		 * cypher_heap_page_prune(), but it's possible that the tuple state changed
		 * since cypher_heap_page_prune() looked.  Handle that here by restarting.
		 * (See comments at the top of function for a full explanation.)
		 */
		res = CypherHeapTupleSatisfiesVacuum(&tuple, vacrel->OldestXmin, buf);
		...
		if (cypher_heap_prepare_freeze_tuple(tuple.t_data,
									  vacrel->relfrozenxid,
									  vacrel->relminmxid,
									  vacrel->FreezeLimit,
	...
	if (nfrozen > 0)
	{
		...
		for (int i = 0; i < nfrozen; i++)
		{
			...
			cypher_heap_execute_freeze_tuple(htup, &frozen[i]);
		}
		...
		if (RelationNeedsWAL(vacrel->rel))
		{
			...
			recptr = cypher_log_heap_freeze(vacrel->rel, buf, vacrel->FreezeLimit,
									 frozen, nfrozen);
					  ...
					  int tupindex, Buffer *vmbuffer)
{
	...
	{
		uint8		flags = 0;
		uint8		vm_status = cypher_visibilitymap_get_status(vacrel->rel,
														 blkno, vmbuffer);
		...
		if (flags != 0)
			cypher_visibilitymap_set(vacrel->rel, blkno, buffer, InvalidXLogRecPtr,
...
lazy_check_needs_freeze(Buffer buf, bool *hastup, LVRelState *vacrel)
{
	...
	*hastup = false;
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		if (cypher_heap_tuple_needs_freeze(tupleheader, vacrel->FreezeLimit,
						 ...
						 bool *all_frozen)
{
	...
	*all_frozen = true;
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		switch (CypherHeapTupleSatisfiesVacuum(&tuple, vacrel->OldestXmin, buf))
		{
			case HEAPTUPLE_LIVE:
				{
					TransactionId xmin;
					...
					if (all_visible && *all_frozen &&
						cypher_heap_tuple_needs_eventual_freeze(tuple.t_data))
						*all_frozen = false;
				}
				break;
					  ...
					  int nrequested)
{
	...
	EnterParallelMode();
	pcxt = CreateParallelContext("postgres", "cypher_parallel_vacuum_main",
								 parallel_workers);
 ...
 */
void
cypher_parallel_vacuum_main(dsm_segment *seg, shm_toc *toc)
{
	Relation	rel;
	Relation   *indrels;
	LVShared   *lvshared;
