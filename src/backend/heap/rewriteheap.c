 ...
 * To use the facility:
 *
 * cypher_begin_heap_rewrite
 * while (fetch next tuple)
 * {
 *	   if (tuple is dead)
 *		   cypher_rewrite_heap_dead_tuple
 *	   else
 *	   {
 *		   // do any transformations here if required
 *		   cypher_rewrite_heap_tuple
 *	   }
 * }
 * cypher_end_heap_rewrite
 *
 * The contents of the new relation shouldn't be relied on until after
 * cypher_end_heap_rewrite is called.
 *
 *
 * IMPLEMENTATION
 *
 ...
 */
RewriteState
cypher_begin_heap_rewrite(Relation old_heap, Relation new_heap, TransactionId oldest_xmin,
 ...
 */
void
cypher_end_heap_rewrite(RewriteState state)
{
 ...
 *
 * state		opaque state as returned by cypher_begin_heap_rewrite
 * old_tuple	original tuple in the old heap
 * new_tuple	new, rewritten tuple to be inserted to new heap
 */
void
cypher_rewrite_heap_tuple(RewriteState state,
				   HeapTuple old_tuple, HeapTuple new_tuple)
{
	 ...
	 */
	cypher_heap_freeze_tuple(new_tuple->t_data,
	...
	if (!((old_tuple->t_data->t_infomask & HEAP_XMAX_INVALID) ||
		  CypherHeapTupleHeaderIsOnlyLocked(old_tuple->t_data)) &&
 ...
 */
bool
cypher_rewrite_heap_dead_tuple(RewriteState state, HeapTuple old_tuple)
{
	/*
...
raw_heap_insert(RewriteState state, HeapTuple tup)
{
	...
	else if (HeapTupleHasExternal(tup) || tup->t_len > TOAST_TUPLE_THRESHOLD)
	{
		...
		heaptup = cypher_heap_toast_insert_or_update(state->rs_new_rel, tup, NULL,
											  options);
	}
	else
		heaptup = tup;
	...
	if (state->rs_buffer_valid)
	{
		...
		if (len + saveFreeSpace > pageFreeSpace)
		{
			 ...
			 * contains a tuple.  Hence, unlike CypherRelationGetBufferForTuple(),
			 ...
			 * ourselves in cypher_end_heap_rewrite.
			 */
 ...
 * in memory or when cypher_end_heap_rewrite() has been called.
 *
 ...
 * CypherCheckPointLogicalRewriteHeap()) has flushed the (partial) mapping file to
...
/*
 * Logical remapping part of cypher_end_heap_rewrite().
 */
static void
 ...
 * new_tuple->t_self by cypher_rewrite_heap_tuple() if necessary for the tuple.
 */
static void
 ...
 */
void
cypher_heap_xlog_logical_rewrite(XLogReaderState *r)
{
	char		path[MAXPGPATH];
	int			fd;
	xl_heap_rewrite_mapping *xlrec;
 ...
 */
void
CypherCheckPointLogicalRewriteHeap(void)
{
	XLogRecPtr	cutoff;
	XLogRecPtr	redo;
	DIR		   *mappings_dir;
