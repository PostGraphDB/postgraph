 ...
 *
 * INTERFACE ROUTINES
 *		cypher_visibilitymap_clear  - clear bits for one page in the visibility map
 *		cypher_visibilitymap_pin	 - pin a map page for setting a bit
 *		cypher_visibilitymap_pin_ok - check whether correct map page is already pinned
 *		cypher_visibilitymap_set	 - set a bit in a previously pinned page
 *		cypher_visibilitymap_get_status - get status of bits
 *		cypher_visibilitymap_count  - count number of bits set in visibility map
 *		cypher_visibilitymap_prepare_truncate -
...
/*
 *	cypher_visibilitymap_clear - clear specified bits for one page in visibility map
 *
 * You must pass a buffer containing the correct map page to this function.
 * Call cypher_visibilitymap_pin first to pin the right one. This function doesn't do
 * any I/O.  Returns true if any bits have been cleared and false otherwise.
 */
bool
cypher_visibilitymap_clear(Relation rel, BlockNumber heapBlk, Buffer buf, uint8 flags)
{
	...
	if (!BufferIsValid(buf) || BufferGetBlockNumber(buf) != mapBlock)
		elog(ERROR, "wrong buffer passed to cypher_visibilitymap_clear");
 ...
 * cypher_visibilitymap_pin, to pin the visibility map page containing the bit for
 ...
 * cypher_visibilitymap_set to actually set the bit.
 *
 * On entry, *buf should be InvalidBuffer or a valid buffer returned by
 * an earlier call to cypher_visibilitymap_pin or cypher_visibilitymap_get_status on the same
 ...
 */
void
cypher_visibilitymap_pin(Relation rel, BlockNumber heapBlk, Buffer *buf)
{
	BlockNumber mapBlock = HEAPBLK_TO_MAPBLOCK(heapBlk);
...
/*
 *	cypher_visibilitymap_pin_ok - do we already have the correct page pinned?
 *
 * On entry, buf should be InvalidBuffer or a valid buffer returned by
 * an earlier call to cypher_visibilitymap_pin or cypher_visibilitymap_get_status on the same
 ...
 */
bool
cypher_visibilitymap_pin_ok(BlockNumber heapBlk, Buffer buf)
{
	BlockNumber mapBlock = HEAPBLK_TO_MAPBLOCK(heapBlk);
...
/*
 *	cypher_visibilitymap_set - set bit(s) on a previously pinned page
 *
 ...
 */
void
cypher_visibilitymap_set(Relation rel, BlockNumber heapBlk, Buffer heapBuf,
				  ...
				  uint8 flags)
{
	...
	if (BufferIsValid(heapBuf) && BufferGetBlockNumber(heapBuf) != heapBlk)
		elog(ERROR, "wrong heap buffer passed to cypher_visibilitymap_set");

	/* Check that we have the right VM page pinned */
	if (!BufferIsValid(vmBuf) || BufferGetBlockNumber(vmBuf) != mapBlock)
		elog(ERROR, "wrong VM buffer passed to cypher_visibilitymap_set");
	...
	if (flags != (map[mapByte] >> mapOffset & VISIBILITYMAP_VALID_BITS))
	{
		...
		if (RelationNeedsWAL(rel))
		{
			...
			{
				Assert(!InRecovery);
				recptr = cypher_log_heap_visible(rel->rd_node, heapBuf, vmBuf,
										  cutoff_xid, flags);
 ...
 * earlier call to cypher_visibilitymap_pin or cypher_visibilitymap_get_status on the same
 * relation. On return, *buf is a valid buffer with the map page containing
 ...
 */
uint8
cypher_visibilitymap_get_status(Relation rel, BlockNumber heapBlk, Buffer *buf)
{
	BlockNumber mapBlock = HEAPBLK_TO_MAPBLOCK(heapBlk);
	uint32		mapByte = HEAPBLK_TO_MAPBYTE(heapBlk);
	uint8		mapOffset = HEAPBLK_TO_OFFSET(heapBlk);
 ...
 */
void
cypher_visibilitymap_count(Relation rel, BlockNumber *all_visible, BlockNumber *all_frozen)
{
 ...
 */
BlockNumber
cypher_visibilitymap_prepare_truncate(Relation rel, BlockNumber nheapblocks)
{
