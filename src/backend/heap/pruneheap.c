...
/* Working data for cypher_heap_page_prune and subroutines */
typedef struct
{
	Relation	rel;
	 ...
	 * and efficiency reasons; see comment in cypher_heap_page_prune() for
 ...
 */
void
cypher_heap_page_prune_opt(Relation relation, Buffer buffer)
{
	...
	if (PageIsFull(page) || PageGetHeapFreeSpace(page) < minfree)
	{
		...
		{
			/* OK to prune */
			(void) cypher_heap_page_prune(relation, buffer, vistest,
								   limited_xmin, limited_ts,
								   true, NULL);
		}
 ...
 */
int
cypher_heap_page_prune(Relation relation, Buffer buffer,
				GlobalVisState *vistest,
				TransactionId old_snap_xmin,
				TimestampTz old_snap_ts,
				bool report_stats,
...
heap_prune_satisfies_vacuum(PruneState *prstate, HeapTuple tup, Buffer buffer)
{
	...
	res = CypherHeapTupleSatisfiesVacuumHorizon(tup, buffer, &dead_after);
...
heap_prune_chain(Buffer buffer, OffsetNumber rootoffnum, PruneState *prstate)
{
	...
	if (ItemIdIsNormal(rootlp))
	{
		...
		if (HeapTupleHeaderIsHeapOnly(htup))
		{
				...
				!HeapTupleHeaderIsHotUpdated(htup))
			{
				...
				CypherHeapTupleHeaderAdvanceLatestRemovedXid(htup,
	...
	else if (nchain < 2 && ItemIdIsRedirected(rootlp))
	{
		 ...
		 * item.  This can happen if the loop in cypher_heap_page_prune caused us to
...
/*
 * Perform the actual page changes needed by cypher_heap_page_prune.
 * It is expected that the caller has a super-exclusive lock on the
 * buffer.
 */
void
cypher_heap_page_prune_execute(Buffer buffer,
 ...
 */
void
cypher_heap_get_root_tuples(Page page, OffsetNumber *root_offsets)
{
	OffsetNumber offnum,
				maxoff;
