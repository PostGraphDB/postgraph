 ...
 *	 CypherHeapTupleSatisfiesUpdate()
...
/*
 * CypherHeapTupleSetHintBits --- exported version of SetHintBits()
 *
 * This must be separate because of C99's brain-dead notions about how to
 * implement inline functions.
 */
void
CypherHeapTupleSetHintBits(HeapTupleHeader tuple, Buffer buffer,
					 uint16 infomask, TransactionId xid)
{
 ...
 */
HTSV_Result
CypherHeapTupleSatisfiesVacuum(HeapTuple htup, TransactionId OldestXmin,
						 Buffer buffer)
{
	TransactionId dead_after = InvalidTransactionId;
	HTSV_Result res;

	res = CypherHeapTupleSatisfiesVacuumHorizon(htup, buffer, &dead_after);
 ...
 */
HTSV_Result
CypherHeapTupleSatisfiesVacuumHorizon(HeapTuple htup, Buffer buffer, TransactionId *dead_after)
{
	HeapTupleHeader tuple = htup->t_data;
	...
	*dead_after = InvalidTransactionId;
	...
	if (!HeapTupleHeaderXminCommitted(tuple))
	{
		...
		else if (TransactionIdIsCurrentTransactionId(HeapTupleHeaderGetRawXmin(tuple)))
		{
			...
			if (HEAP_XMAX_IS_LOCKED_ONLY(tuple->t_infomask) ||
				CypherHeapTupleHeaderIsOnlyLocked(tuple))
				return HEAPTUPLE_INSERT_IN_PROGRESS;
 ...
 */
bool
CypherHeapTupleIsSurelyDead(HeapTuple htup, GlobalVisState *vistest)
{
	HeapTupleHeader tuple = htup->t_data;
 ...
 */
bool
CypherHeapTupleHeaderIsOnlyLocked(HeapTupleHeader tuple)
{
	TransactionId xmax;
...
/*
 * CypherHeapTupleSatisfiesVisibility
 ...
 */
bool
CypherHeapTupleSatisfiesVisibility(HeapTuple tup, Snapshot snapshot, Buffer buffer)
{
