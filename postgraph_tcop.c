...
/* --------------------------------
 *		signal handler routines used in PostGraphMain()
 * --------------------------------
 */
...
stack_is_too_deep(void)
{
	 ...
	 * "BSP" pointer from PostGraphMain to here.  Logic is just as above,
 ...
 *	   Parse command line arguments for PostGraphMain
 *
...
*/
	 ...
	 * it inside InitPostGraph() instead.  In particular, anything that
...
ereport(LOG, errmsg("ReadCommand(\"%s\")", input_message.data));
		...
		ereport(LOG, errmsg("msg: %c", firstchar));
		switch (firstchar)
		{
			case 'Q':			/* simple query */
				{
