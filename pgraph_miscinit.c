...
{
	IsUnderGraphmaster = true;	/* we are a postmaster subprocess now */
...
{
	Assert(!IsGraphmasterEnvironment);
...
InitializeSessionUserIdStandalone(void)
{
	 ...
	 */
	AssertState(!IsUnderGraphmaster || IsAutoVacuumWorkerProcess() || IsBackgroundWorker);
...
UnlinkLockFiles(int status, Datum arg)
{
	 ...
	 */
	ereport(IsGraphmasterEnvironment ? LOG : NOTICE,
			   ...
			   bool isDDLock, const char *refName)
{
	...
	lock_files = lcons(pstrdup(filename), lock_files);
        //ereport(FATAL, errmsg("here: %s %s %s", filename, socketDir, refName));
}
