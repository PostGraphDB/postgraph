 ...
 *
 * setup/teardown:
 *		PostGraphStreamServerPort	- Open postmaster's server port
...
/*
 * PostGraphStreamServerPort -- open a "listening" port to accept connections.
 *
...
Lock_AF_UNIX(const char *unixSocketDir, const char *unixSocketPath)
{
	...
	//ereport(FATAL, errmsg("here %s %s", unixSocketPath, unixSocketDir));
	/*
	 ...
	 */
	PostGraphCreateSocketLockFile(unixSocketPath, true, unixSocketDir);
...
/*
 * PostGraphRemoveSocketFiles -- unlink socket files at postmaster shutdown
 */
void
