...
void insert_label(const char *label_name, Oid graph_oid, int32 label_id,
  ...
  /*
     ...
     CStringGetDatum(label_name)); nulls[5] = false; } else if (label_kind ==
     'v') { values[5] = DirectFunctionCall2(ltree_addltree,
                          DirectFunctionCall1(ltree_in,
     CStringGetDatum("_ag_label_vertex")), DirectFunctionCall1(ltree_in,
     CStringGetDatum(label_name))); nulls[5] = false;

      } else {
          values[5] = DirectFunctionCall2(ltree_addltree,
                          DirectFunctionCall1(ltree_in,
     CStringGetDatum("_ag_label_edge")), DirectFunctionCall1(ltree_in,
     CStringGetDatum(label_name))); nulls[5] = false;
...
List *get_all_edge_labels_per_graph(EState *estate, Oid graph_oid) {
  ...
  resultRelInfo =
      create_entity_result_rel_info(estate, CATALOG_SCHEMA, "ag_label");
