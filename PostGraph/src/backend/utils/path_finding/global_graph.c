...
static bool insert_edge(graph_context *ggctx, graphid id, Datum properties,
                        graphid start_id, graphid end_id, Oid oid);
static void insert_vertex(graph_context *ggctx, graphid start_id,
                          graphid end_id, graphid id);
static void insert_vertex_entry(graph_context *ggctx, graphid id, Oid oid,
                                Datum properties);

// XXX: To Be Removed
bool is_ggctx_invalid(graph_context *ggctx) {
  Snapshot snap = GetActiveSnapshot();
  return (ggctx->xmin != snap->xmin || ggctx->xmax != snap->xmax ||
...
static void create_hashtables(graph_context *ggctx) {
  ...
  ggctx->vertex_hashtable =
      hash_create(VERTEX_HTAB_NAME, VERTEX_HTAB_INITIAL_SIZE, &vertex_ctl,
  ...
  ggctx->edge_hashtable = hash_create(EDGE_HTAB_NAME, EDGE_HTAB_INITIAL_SIZE,
                                      &edge_ctl, HASH_ELEM | HASH_FUNCTION);
}
...
static bool insert_edge(graph_context *ggctx, graphid id, Datum properties,
  ...
  // search for the edge
  value = (edge_entry *)hash_search(ggctx->edge_hashtable, (void *)&id,
                                    HASH_ENTER, &found);
  /*
...
static void insert_vertex_entry(graph_context *ggctx, graphid id, Oid oid,
  ...
  ve = (vertex_entry *)hash_search(ggctx->vertex_hashtable, (void *)&id,
                                   HASH_ENTER, &found);
...
static void insert_vertex(graph_context *ggctx, graphid start_id,
  ...
  value = (vertex_entry *)hash_search(ggctx->vertex_hashtable, (void *)&end_id,
                                      HASH_FIND, &found);
  // vertices were preloaded so it must be there
  Assert(found);
...
static void load_edge_hashtable(graph_context *ggctx) {
  ...
  foreach (lc, labels) {
    ...
    TableScanDesc scan_desc =
        table_beginscan(graph_edge_label, snapshot, 0, NULL);
...
static void free_graph_context(graph_context *ggctx) {
  ...
  while (curr_vertex) {
    ...
    value = (vertex_entry *)hash_search(ggctx->vertex_hashtable, (void *)&id,
                                        HASH_FIND, &found);
    Assert(found);
...
edge_entry *get_edge_entry(graph_context *ggctx, graphid id) {
  ...
  ee = (edge_entry *)hash_search(ggctx->edge_hashtable, (void *)&id, HASH_FIND,
                                 &found);
...
Queue *get_graph_vertices(graph_context *ggctx) { return ggctx->vertices; }

// vertex_entry accessor functions
graphid get_vertex_entry_id(vertex_entry *ve) { return ve->id; }

Queue *get_vertex_entry_edges_in(vertex_entry *ve) { return ve->in; }

Queue *get_vertex_entry_edges_out(vertex_entry *ve) { return ve->out; }

Queue *get_vertex_entry_edges_self(vertex_entry *ve) { return ve->loop; }

Oid get_vertex_entry_label_table_oid(vertex_entry *ve) { return ve->oid; }

Datum get_vertex_entry_properties(vertex_entry *ve) { return ve->properties; }

// edge_entry accessor functions
graphid get_edge_entry_id(edge_entry *ee) { return ee->id; }

Oid get_edge_entry_label_table_oid(edge_entry *ee) { return ee->oid; }

Datum get_edge_entry_properties(edge_entry *ee) { return ee->properties; }

graphid get_start_id(edge_entry *ee) { return ee->start_id; }

graphid get_end_id(edge_entry *ee) { return ee->end_id; }
