...
// gtype functions
static bool check_edge_constraints(path_finding_context *path_ctx,
                                   edge_entry *ee);
// VLE local context functions
static path_finding_context *build_vle_context(FunctionCallInfo fcinfo,
                                               FuncCallContext *funcctx);
static void create_hashtable(path_finding_context *path_ctx);
// VLE graph traversal functions
static edge_state_entry *get_edge_state(path_finding_context *path_ctx,
                                        graphid edge_id);
...
static void create_hashtable(path_finding_context *path_ctx) {
  ...
  path_ctx->edge_state_hashtable =
      hash_create(EDGE_STATE_HTAB_NAME, EDGE_STATE_HTAB_INITIAL_SIZE,
                  &edge_state_ctl, HASH_ELEM | HASH_FUNCTION);
  ...
  path_ctx->exists_hash =
      hash_create(EXISTS_HTAB_NAME, 20, &exists_ctl, HASH_ELEM | HASH_FUNCTION);
}
...
static bool check_edge_constraints(path_finding_context *path_ctx,
  ...
  if (path_ctx->label_name != NULL &&
      strcmp(path_ctx->label_name, label_name) != 0)
    return false;
...
static bool do_vsid_and_veid_exist(path_finding_context *path_ctx) {
  ...
  return ((get_vertex_entry(path_ctx->ggctx, path_ctx->vsid) != NULL) &&
          (get_vertex_entry(path_ctx->ggctx, path_ctx->veid) != NULL));
}
...
static path_finding_context *build_vle_context(FunctionCallInfo fcinfo,
  ...
  gtype_value *agtv_temp =
      get_gtype_value("age_vle", AG_GET_ARG_GTYPE_P(0), AGTV_STRING, true);
  char *graph_name =
      pnstrdup(agtv_temp->val.string.val, agtv_temp->val.string.len);
  ...
  if (PG_ARGISNULL(3) || is_gtype_null(AG_GET_ARG_GTYPE_P(3))) {
    ...
    agtv_temp =
        get_gtype_value("age_vle", AG_GET_ARG_GTYPE_P(3), AGTV_INTEGER, true);
  ...
  if (PG_ARGISNULL(4) || is_gtype_null(AG_GET_ARG_GTYPE_P(4))) {
    ...
    agtv_temp =
        get_gtype_value("age_vle", AG_GET_ARG_GTYPE_P(4), AGTV_INTEGER, true);
    path_ctx->uidx = agtv_temp->val.int_value;
    path_ctx->uidx_infinite = false;
  }
  // get edge direction
  agtv_temp =
      get_gtype_value("age_vle", AG_GET_ARG_GTYPE_P(5), AGTV_INTEGER, true);
  ...
  if (PG_ARGISNULL(6) || is_gtype_null(AG_GET_ARG_GTYPE_P(6))) {
    ...
    agtv_temp =
        get_gtype_value("age_vle", AG_GET_ARG_GTYPE_P(6), AGTV_STRING, true);
...
static edge_state_entry *get_edge_state(path_finding_context *path_ctx,
  ...
  ese = (edge_state_entry *)hash_search(path_ctx->edge_state_hashtable,
...
static bool dfs_find_a_path_between(path_finding_context *path_ctx) {
  ...
  while (!IS_GRAPHID_STACK_EMPTY(edge_queue)) {
    ...
    if (ese->visited) {
      ...
      // If the ids are the same, we're backing up. So, remove it from the path
      // queue and reset visited.
    ...
    if (next_vertex_id == end_vertex_id &&
        queue_size(path_queue) >= path_ctx->lidx &&
     ...
     */
    if (next_vertex_id == end_vertex_id && !path_ctx->uidx_infinite &&
        queue_size(path_queue) > path_ctx->uidx)
      continue;
...
static void add_edges(path_finding_context *path_ctx, graphid vertex_id) {
  ...
  while (edge_out || edge_in || edge_self) {
    ...
    // Don't add any edges that we have already seen because they will cause a
    // loop to form.
...
static path_container *build_path_container(path_finding_context *path_ctx) {
  ...
  for (index = 1; index < vpc->graphid_array_size - 1; index += 2) {
    ...
    graphid_array[index + 1] =
        (vid == get_start_id(ee)) ? get_end_id(ee) : get_start_id(ee);
  }
...
Datum gtype_vle(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    //   PointerGetDatum(build_path_container(funcctx->user_fctx)));
    //   SRF_RETURN_NEXT(funcctx,
    //   PointerGetDatum(gtype_value_to_gtype(agtv_materialize_vle_path(build_path_container(funcctx->user_fctx)))));
  }
  ...
  if (found_a_path) {
    ...
    hash_destroy(
        ((path_finding_context *)funcctx->user_fctx)->edge_state_hashtable);
...
VariableEdge *create_variable_edge(path_container *vpc) {
  ...
  for (int index = 0; index < graphid_array_size; index += 2) {
    ...
    Datum d =
        EDGE_GET_DATUM(create_edge(id, startid, endid, vpc->graph_oid, prop));
...
Datum _ag_enforce_edge_uniqueness(PG_FUNCTION_ARGS) {
  ...
  HTAB *exists_hash = hash_create(EXISTS_HTAB_NAME, nargs, &exists_ctl,
  ...
  for (int i = 0; i < nargs; i++) {
    ...
    if (types[i] == GRAPHIDOID) {
      ...
      bool found;
      int64 *value = (int64 *)hash_search(exists_hash, (void *)&edge_id,
                                          HASH_ENTER, &found);
