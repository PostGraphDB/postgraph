...
bool ag_serialize_extended_type(StringInfo buffer, gtentry *gtentry,
  ...
  case AGTV_TIMETZ:
    ...
    *((int32 *)(buffer->data + offset + sizeof(TimeADT))) =
        scalar_val->val.timetz.zone;
  ...
  case AGTV_INTERVAL:
    ...
    *((int32 *)(buffer->data + offset + sizeof(TimeOffset))) =
        scalar_val->val.interval.day;
    *((int32 *)(buffer->data + offset + sizeof(TimeOffset) + sizeof(int32))) =
        scalar_val->val.interval.month;
  ...
  case AGTV_POINT:
    padlen = ag_serialize_header(buffer, GT_HEADER_POINT);
  ...
  case AGTV_LSEG:
    ...
    *gtentry = GTENTRY_IS_GTYPE | (padlen + numlen + GT_HEADER_SIZE);
    break;
  case AGTV_LINE:
    padlen = ag_serialize_header(buffer, GT_HEADER_LINE);
    ...
    *gtentry = GTENTRY_IS_GTYPE | (padlen + numlen + GT_HEADER_SIZE);
    break;
  case AGTV_PATH:
    padlen = ag_serialize_header(buffer, GT_HEADER_PATH);

    numlen = scalar_val->val.path->vl_len_ / 4;
    offset = reserve_from_buffer(buffer, numlen);
    memcpy(buffer->data + offset, scalar_val->val.path,
           scalar_val->val.path->vl_len_ / 4);
  ...
  case AGTV_POLYGON:
    ...
    offset = reserve_from_buffer(buffer, numlen);
    memcpy(buffer->data + offset, scalar_val->val.polygon,
           scalar_val->val.polygon->vl_len_ / 4);

    *gtentry = GTENTRY_IS_GTYPE | (padlen + numlen + GT_HEADER_SIZE);
    break;
  case AGTV_CIRCLE:
    padlen = ag_serialize_header(buffer, GT_HEADER_CIRCLE);
    ...
    /*
       case AGTV_BOX2D:
        ...
        case AGTV_SPHEROID:
            ...
            offset = reserve_from_buffer(buffer, numlen);
            memcpy(buffer->data + offset, &scalar_val->val.spheroid,
       sizeof(SPHEROID));
        ...
        case AGTV_GSERIALIZED:
            ...
            offset = reserve_from_buffer(buffer, numlen);
            memcpy(buffer->data + offset, scalar_val->val.gserialized,
       ((GSERIALIZED *)scalar_val->val.gserialized)->size / 4);
  ...
  case AGTV_BYTEA:
    ...
    offset = reserve_from_buffer(buffer, numlen);
    memcpy(buffer->data + offset, scalar_val->val.bytea,
           *((int *)(scalar_val->val.bytea)->vl_len_) / 4);
  ...
  case AGTV_TSVECTOR:
    ...
    offset = reserve_from_buffer(buffer, numlen);
    memcpy(buffer->data + offset, scalar_val->val.tsvector,
           (scalar_val->val.tsvector)->vl_len_ / 4);
  ...
  case AGTV_TSQUERY:
    ...
    offset = reserve_from_buffer(buffer, numlen);
    memcpy(buffer->data + offset, scalar_val->val.tsquery,
           (scalar_val->val.tsquery)->vl_len_ / 4);
  ...
  case AGTV_RANGE_INT:
    ...
    offset = reserve_from_buffer(buffer, numlen);
    memcpy(buffer->data + offset, scalar_val->val.range,
           (scalar_val->val.range)->vl_len_ / 4);
  ...
  case AGTV_RANGE_INT_MULTI:
    ...
    offset = reserve_from_buffer(buffer, numlen);
    memcpy(buffer->data + offset, scalar_val->val.multirange,
           (scalar_val->val.multirange)->vl_len_ / 4);
...
void ag_deserialize_extended_type(char *base_addr, uint32 offset,
  ...
  case GT_HEADER_TIMETZ:
    ...
    result->val.timetz.zone =
        *((int32 *)(base + GT_HEADER_SIZE + sizeof(TimeADT)));
    break;
  case GT_HEADER_INTERVAL:
    result->type = AGTV_INTERVAL;
    result->val.interval.time = *((TimeOffset *)(base + GT_HEADER_SIZE));
    result->val.interval.day =
        *((int32 *)(base + GT_HEADER_SIZE + sizeof(TimeOffset)));
    result->val.interval.month = *(
        (int32 *)(base + GT_HEADER_SIZE + sizeof(TimeOffset) + sizeof(int32)));
    break;
  case GT_HEADER_INET:
  ...
  case GT_HEADER_LSEG:
    ...
    break;
  case GT_HEADER_LINE:
    result->type = AGTV_LINE;
    result->val.line = (base + GT_HEADER_SIZE);
    break;
  case GT_HEADER_PATH:
  ...
  case GT_HEADER_POLYGON:
    ...
    break;
  case GT_HEADER_CIRCLE:
    result->type = AGTV_CIRCLE;
    result->val.circle = (base + GT_HEADER_SIZE);
    break;
  case GT_HEADER_BOX:
    result->type = AGTV_BOX;
    result->val.box = (base + GT_HEADER_SIZE);
    break;
    /*
