...
Datum build_variable_edge(PG_FUNCTION_ARGS) {
 ...
 */
static int32 compare_variable_edge_orderability(VariableEdge *lhs,
                                                VariableEdge *rhs) {
...
Datum edge_contained_in_variable_edge(PG_FUNCTION_ARGS) {
  ...
  for (int i = 0; i < variable_edge->children[0] - 1;
       i++, ptr = ptr + VARSIZE(ptr)) {
    if (i % 2 == 0) {
...
PG_FUNCTION_INFO_V1(variable_edge_contains_edge);
Datum variable_edge_contains_edge(PG_FUNCTION_ARGS) {
  edge *e = AG_GET_ARG_EDGE(1);
  VariableEdge *variable_edge = AG_GET_ARG_VARIABLE_EDGE(0);
...
Datum variable_edge_edges_overlap(PG_FUNCTION_ARGS) {
  ...
  char *rhs_ptr = &rhs->children[1];
  for (int i = 0; i < rhs->children[0] - 1;
       i++, rhs_ptr = rhs_ptr + VARSIZE(rhs_ptr)) {
    if (i % 2 == 0) {
      char *lhs_ptr = &lhs->children[1];
      edge *right_edge = (edge *)rhs_ptr;
      for (int j = 0; j < lhs->children[0];
           j++, lhs_ptr = lhs_ptr + VARSIZE(lhs_ptr)) {
        if (j % 2 == 0) {
...
Datum variable_edge_edges(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_ARRAYTYPE_P(
      construct_array(array_value, size, EDGEOID, -1, false, TYPALIGN_INT));
}
...
Datum variable_edge_nodes(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_ARRAYTYPE_P(
      construct_array(array_value, size, VERTEXOID, -1, false, TYPALIGN_INT));
}
...
Datum variable_edge_length(PG_FUNCTION_ARGS) {
  ...
  gtype_value gtv = {.type = AGTV_INTEGER,
                     .val = {.int_value = (v->children[0] + 1) / 2}};
