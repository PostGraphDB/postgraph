...
static void fill_gtype_value(gtype_container *container, int index,
                             ...
                             gtype_value *result);
static bool equals_gtype_scalar_value(const gtype_value *a,
                                      const gtype_value *b);
...
static void convert_gtype_array(StringInfo buffer, gtentry *pheader,
                                gtype_value *val, int level);
static void convert_gtype_vector(StringInfo buffer, gtentry *pheader,
                                 gtype_value *val, int level);
static void convert_gtype_object(StringInfo buffer, gtentry *pheader,
                                 gtype_value *val, int level);
static void convert_gtype_scalar(StringInfo buffer, gtentry *entry,
    ...
    if (ra == rb) {
      ...
      if ((va.type == vb.type) ||
          ((va.type == AGTV_INTEGER || va.type == AGTV_FLOAT ||
            va.type == AGTV_NUMERIC || va.type == AGTV_TIMESTAMP ||
            va.type == AGTV_DATE || va.type == AGTV_TIMESTAMPTZ ||
            va.type == AGTV_TIMETZ || va.type == AGTV_TIME ||
            va.type == AGTV_DATE || va.type == AGTV_INET ||
            va.type == AGTV_CIDR || va.type == AGTV_RANGE_INT ||
            va.type == AGTV_RANGE_NUM || va.type == AGTV_RANGE_TS ||
            va.type == AGTV_RANGE_TSTZ || va.type == AGTV_RANGE_DATE ||
            va.type == AGTV_TSQUERY) &&
           (vb.type == AGTV_INTEGER || vb.type == AGTV_FLOAT ||
            vb.type == AGTV_NUMERIC || vb.type == AGTV_TIMESTAMP ||
            vb.type == AGTV_DATE || vb.type == AGTV_TIMESTAMPTZ ||
            vb.type == AGTV_TIMETZ || vb.type == AGTV_TIME ||
            vb.type == AGTV_DATE || vb.type == AGTV_INET ||
            va.type == AGTV_CIDR || vb.type == AGTV_RANGE_INT ||
            vb.type == AGTV_RANGE_NUM || vb.type == AGTV_RANGE_TS ||
            vb.type == AGTV_RANGE_TSTZ || vb.type == AGTV_RANGE_DATE ||
            vb.type == AGTV_TSQUERY))) {
        switch (va.type) {
        case AGTV_STRING:
        case AGTV_NULL:
        case AGTV_NUMERIC:
        ...
        // Type-defined order
        res =
            (get_type_sort_priority(va.type) < get_type_sort_priority(vb.type))
                ? -1
                : 1;
      }
    } else {
      ...
      // Type-defined order
      res = (get_type_sort_priority(va.type) < get_type_sort_priority(vb.type))
                ? -1
                : 1;
    }
  } while (res == 0);
...
gtype_value *find_gtype_value_from_container(gtype_container *container,
  ...
  if ((flags & GT_FARRAY) && GTYPE_CONTAINER_IS_ARRAY(container)) {
  ...
  } else if ((flags & GT_FOBJECT) && GTYPE_CONTAINER_IS_OBJECT(container)) {
    ...
    while (stop_low < stop_high) {
      ...
      candidate.val.string.len = get_gtype_length(container, stop_middle);
...
static void fill_gtype_value(gtype_container *container, int index,
  ...
  } else if (GTE_IS_VECTOR(container)) {
    ereport(ERROR, (errmsg("here")));
  } else {
    Assert(GTE_IS_CONTAINER(entry));
    result->type = AGTV_BINARY;
    // Remove alignment padding from data pointer and length
    result->val.binary.data = (gtype_container *)(base_addr + INTALIGN(offset));
    result->val.binary.len =
        get_gtype_length(container, index) - (INTALIGN(offset) - offset);
  }
}
...
gtype_value *push_gtype_value(gtype_parse_state **pstate,
  ...
  while ((tok = gtype_iterator_next(&it, &v, false)) != WGT_DONE) {
    res =
        push_gtype_value_scalar(pstate, tok, tok < WGT_BEGIN_ARRAY ? &v : NULL);
  }
...
static gtype_value *push_gtype_value_scalar(gtype_parse_state **pstate,
  ...
  case WGT_END_ARRAY:
    ...
    *pstate = (*pstate)->next;
    ...
    break;
  default:
    ereport(ERROR, (errmsg("unrecognized gtype sequential processing token")));
  }
...
static void append_key(gtype_parse_state *pstate, gtype_value *string) {
  ...
  if (object->val.object.num_pairs >= pstate->size) {
    ...
    object->val.object.pairs =
        repalloc(object->val.object.pairs, sizeof(gtype_pair) * pstate->size);
  }
...
static void append_element(gtype_parse_state *pstate, gtype_value *scalar_val) {
  ...
  if (array->val.array.num_elems >= pstate->size) {
    ...
    array->val.array.elems =
        repalloc(array->val.array.elems, sizeof(gtype_value) * pstate->size);
  }
...
recurse:
  ...
  case GTI_ARRAY_ELEM:
    ...
    fill_gtype_value((*it)->container, (*it)->curr_index, (*it)->data_proper,
  ...
  case GTI_OBJECT_START:
    ...
    (*it)->curr_value_offset =
        get_gtype_offset((*it)->container, (*it)->num_elems);
    // Set state for next call
    ...
    if ((*it)->curr_index >= (*it)->num_elems) {
      ...
      *it = free_and_get_parent(*it);
      ...
      if (val->type != AGTV_STRING)
        ereport(ERROR,
                (errmsg("unexpected gtype type as object key %d", val->type)));
  ...
  case GTI_OBJECT_VALUE:
    ...
    fill_gtype_value((*it)->container, (*it)->curr_index + (*it)->num_elems,
                     (*it)->data_proper, (*it)->curr_value_offset, val);

    GTE_ADVANCE_OFFSET((*it)->curr_data_offset,
                       (*it)->children[(*it)->curr_index]);
    GTE_ADVANCE_OFFSET((*it)->curr_value_offset,
  ...
  case GTI_VECTOR_START:
    ...
    (*it)->curr_value_offset = 0; // not actually used
    // Set state for next call
    (*it)->state = GTI_VECTOR_VALUE;
    return WGT_BEGIN_VECTOR;

  case GTI_VECTOR_VALUE:
    if ((*it)->curr_index >= (*it)->num_elems) {
      /*
       * All elements within array already processed.  Report this
       * to caller, and give it back original parent iterator (which
       ...
       */
      *it = free_and_get_parent(*it);
      return WGT_END_VECTOR;
    }

    val->type = AGTV_FLOAT;
    val->val.float_value =
        *(float8 *)((*it)->data_proper + (*it)->curr_data_offset);

    (*it)->curr_data_offset = (*it)->curr_data_offset + sizeof(float8);
    (*it)->curr_index++;

    return WGT_VECTOR_VALUE;
  }
...
static gtype_iterator *iterator_from_container(gtype_container *container,
  ...
  switch (container->header &
          (GT_FARRAY | GT_FOBJECT | GT_FEXTENDED_COMPOSITE)) {
  case GT_FARRAY:
    it->data_proper = (char *)it->children + it->num_elems * sizeof(gtentry);
  ...
  case GT_FOBJECT:
    it->data_proper =
        (char *)it->children + it->num_elems * sizeof(gtentry) * 2;
    it->state = GTI_OBJECT_START;
    break;

  case GT_FEXTENDED_COMPOSITE: {
    if (container->children[0] == GT_HEADER_VECTOR) {
      it->data_proper = (char *)it->children + sizeof(gtentry);
      it->state = GTI_VECTOR_START;
      break;
    }
  }
  default:
    ereport(ERROR, (errmsg("unknown type of gtype container %d",
                           container->header & (GT_FARRAY | GT_FOBJECT |
                                                GT_FEXTENDED_COMPOSITE))));
  }
...
bool gtype_deep_contains(gtype_iterator **val, gtype_iterator **m_contained) {
  ...
  if (rval != rcont) {
    ...
    for (;;) {
      ...
      lhs_val = find_gtype_value_from_container((*val)->container, GT_FOBJECT,
                                                &vcontained);
      ...
      if (lhs_val->type != vcontained.type) {
        ...
        nest_contained = gtype_iterator_init(vcontained.val.binary.data);
    ...
    for (;;) {
      ...
      if (IS_A_GTYPE_SCALAR(&vcontained)) {
        if (!find_gtype_value_from_container((*val)->container, GT_FARRAY,
                                             &vcontained))
          return false;
      } else {
        uint32 i;
        ...
        for (i = 0; i < num_lhs_elems; i++) {
          ...
          nestval = gtype_iterator_init(lhs_conts[i].val.binary.data);
          nest_contained = gtype_iterator_init(vcontained.val.binary.data);
...
static bool equals_gtype_scalar_value(const gtype_value *a,
  ...
  if (a->type == b->type) {
    ...
    default:
      ereport(ERROR,
              (errmsg("invalid gtype scalar type %d for equals", a->type)));
    }
  }
...
static int silly_cmp_tsvector(const TSVector a, const TSVector b) {
  if (VARSIZE(a) < VARSIZE(b))
    ...
    for (i = 0; i < a->size; i++) {
      ...
      } else if ((res = tsCompareString(STRPTR(a) + aptr->pos, aptr->len,
                                        STRPTR(b) + bptr->pos, bptr->len,
                                        false)) != 0) {
        return res;
      } else if (aptr->haspos) {
...
static int compare_range_internal(gtype_value *a, gtype_value *b) {
  ...
  TypeCacheEntry *typcache =
      lookup_type_cache(RangeTypeGetOid(r1), TYPECACHE_RANGE_INFO);
...
int compare_gtype_scalar_values(gtype_value *a, gtype_value *b) {
  if (a->type == b->type) {
    ...
    case AGTV_NULL:
      return 0;
    case AGTV_STRING:
      return varstr_cmp(a->val.string.val, a->val.string.len, b->val.string.val,
                        b->val.string.len, DEFAULT_COLLATION_OID);
    case AGTV_NUMERIC:
      return DatumGetInt32(
          DirectFunctionCall2(numeric_cmp, PointerGetDatum(a->val.numeric),
                              PointerGetDatum(b->val.numeric)));
    case AGTV_BOOL:
    ...
    case AGTV_FLOAT:
      return compare_two_floats_orderability(a->val.float_value,
                                             b->val.float_value);
      /*        case AGTV_GSERIALIZED:
                  return gserialized_cmp(a->val.gserialized,
         b->val.gserialized);*/
    default:
      ereport(ERROR,
              (errmsg("invalid gtype scalar type %d for compare", a->type)));
    }
  }

  // timestamp and timestamp with timezone
  if (a->type == AGTV_TIMESTAMP && b->type == AGTV_TIMESTAMPTZ)
    return timestamp_cmp_timestamptz_internal(a->val.int_value,
                                              b->val.int_value);

  if (a->type == AGTV_TIMESTAMPTZ && b->type == AGTV_TIMESTAMP)
    return -1 * timestamp_cmp_timestamptz_internal(b->val.int_value,
                                                   a->val.int_value);
  ...
  if (a->type == AGTV_TIME && b->type == AGTV_TIMETZ) {
    int64 b_time = DatumGetTimeADT(
        DirectFunctionCall1(timetz_time, TimeTzADTPGetDatum(&b->val.timetz)));
  ...
  if (a->type == AGTV_TIMETZ && b->type == AGTV_TIME) {
    int64 a_time = DatumGetTimeADT(
        DirectFunctionCall1(timetz_time, TimeTzADTPGetDatum(&a->val.timetz)));
...
static void convert_gtype_value(StringInfo buffer, gtentry *header,
  ...
  if (IS_A_GTYPE_SCALAR(val))
    ...
    ereport(ERROR, (errmsg("unknown gtype type %d to convert", val->type)));
}
  ...
  for (i = 0; i < num_elems; i++) {
    ...
    if (totallen > GTENTRY_OFFLENMASK)
      ereport(
          ...
          (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
           errmsg(
               "total size of vector elements exceeds the maximum of %u bytes",
               GTENTRY_OFFLENMASK)));
  }
  ...
  for (i = 0; i < num_elems; i++) {
    ...
    if (totallen > GTENTRY_OFFLENMASK) {
      ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                      errmsg("total size of gtype array elements exceeds the "
                             "maximum of %u bytes",
                             GTENTRY_OFFLENMASK)));
    }
    ...
    copy_to_buffer(buffer, gtentry_offset, (char *)&meta, sizeof(gtentry));
...
static void convert_gtype_object(StringInfo buffer, gtentry *pheader,
  ...
  gtentry_offset = reserve_from_buffer(buffer, sizeof(gtentry) * num_pairs * 2);
  ...
  for (i = 0; i < num_pairs; i++) {
    ...
    if (totallen > GTENTRY_OFFLENMASK) {
      ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                      errmsg("total size of gtype object elements exceeds the "
                             "maximum of %u bytes",
                             GTENTRY_OFFLENMASK)));
    }
...
static void convert_gtype_scalar(StringInfo buffer, gtentry *entry,
  ...
  case AGTV_BOOL:
    *entry = (scalar_val->val.boolean) ? GTENTRY_IS_BOOL_TRUE
                                       : GTENTRY_IS_BOOL_FALSE;
    break;
...
void uniqueify_gtype_object(gtype_value *object) {
  ...
  if (object->val.object.num_pairs > 1)
    qsort_arg(object->val.object.pairs, object->val.object.num_pairs,
              sizeof(gtype_pair), length_compare_gtype_pair, &has_non_uniq);
