...
#define GTYPETRIGFUNC(name)                                                    \
  ...
  Datum gtype_##name(PG_FUNCTION_ARGS) {                                       \
    ...
    gtype_value gtv = {.type = AGTV_FLOAT,                                     \
                       .val.float_value = DatumGetFloat8(DirectFunctionCall1(  \
                           d##name, GT_ARG_TO_FLOAT8_DATUM(0)))};              \
...
Datum gtype_atan2(PG_FUNCTION_ARGS) {
  Datum x = convert_to_scalar(gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(0),
                              "float");
  Datum y = convert_to_scalar(gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(1),
                              "float");

  gtype_value gtv_result;
  gtv_result.type = AGTV_FLOAT;
  gtv_result.val.float_value =
      DatumGetFloat8(DirectFunctionCall2(datan2, y, x));
...
Datum gtype_degrees(PG_FUNCTION_ARGS) {
  ...
  gtype_value gtv_result;
  gtv_result.type = AGTV_FLOAT;
  gtv_result.val.float_value = DatumGetFloat8(DirectFunctionCall1(
      degrees, convert_to_scalar(gtype_to_float8_internal, gt, "float")));
...
Datum gtype_radians(PG_FUNCTION_ARGS) {
  ...
  gtv_result.type = AGTV_FLOAT;
  gtv_result.val.float_value = DatumGetFloat8(DirectFunctionCall1(
      radians, convert_to_scalar(gtype_to_float8_internal, gt, "float")));

  AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv_result));
}

PG_FUNCTION_INFO_V1(gtype_gcd);
Datum gtype_gcd(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);

  if (GTYPE_IS_FLOAT(lhs) || GTE_IS_NUMERIC(lhs->root.children[0]) ||
      GTYPE_IS_FLOAT(rhs) || GTE_IS_NUMERIC(rhs->root.children[0])) {
    ...
    gtv_result.val.numeric =
        DatumGetNumeric(DirectFunctionCall2(numeric_gcd, y, x));
    ...
    gtv_result.val.int_value =
        DatumGetInt64(DirectFunctionCall2(int8gcd, y, x));
...
Datum gtype_lcm(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);
  ...
  if (GTYPE_IS_FLOAT(lhs) || GTE_IS_NUMERIC(lhs->root.children[0]) ||
    ...
    gtv_result.type = AGTV_NUMERIC;
    gtv_result.val.numeric =
        DatumGetNumeric(DirectFunctionCall2(numeric_lcm, y, x));
    ...
    gtv_result.type = AGTV_INTEGER;
    gtv_result.val.int_value =
        DatumGetInt64(DirectFunctionCall2(int8lcm, y, x));
...
Datum gtype_round(PG_FUNCTION_ARGS) {
  ...
  if (is_gtype_numeric(gt)) {
    ...
    gtv.val.numeric = DatumGetNumeric(
        DirectFunctionCall2(numeric_round, x, Int32GetDatum(0)));
...
Datum gtype_e(PG_FUNCTION_ARGS) {
  gtype_value agtv_result = {.type = AGTV_FLOAT,
                             .val.float_value = DatumGetFloat8(
                                 DirectFunctionCall1(dexp, Float8GetDatum(1)))};
...
Datum gtype_pi(PG_FUNCTION_ARGS) {
  gtype_value agtv_result = {.type = AGTV_FLOAT, .val.float_value = M_PI};
...
Datum gtype_rand(PG_FUNCTION_ARGS) {
  gtype_value agtv_result = {
      .type = AGTV_FLOAT,
      .val.float_value =
          DatumGetFloat8(DirectFunctionCall1(random, Float8GetDatum(1)))};
...
Datum gtype_cbrt(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);

  gtype_value gtv_result;
  gtv_result.type = AGTV_FLOAT;
  gtv_result.val.float_value = DatumGetFloat8(DirectFunctionCall1(
      dcbrt, convert_to_scalar(gtype_to_float8_internal, gt, "float")));

  AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv_result));
}

PG_FUNCTION_INFO_V1(gtype_factorial);
Datum gtype_factorial(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);

  gtype_value gtv_result;
  gtv_result.type = AGTV_NUMERIC;
  gtv_result.val.numeric = DatumGetNumeric(DirectFunctionCall1(
      numeric_fac, convert_to_scalar(gtype_to_int8_internal, gt, "int")));
