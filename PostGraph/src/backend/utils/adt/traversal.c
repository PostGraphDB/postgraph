...
Datum build_traversal(PG_FUNCTION_ARGS) {
  ...
  for (int i = 0; i < nargs; i++) {
    if (i % 2 == 0) {
      ...
      cnt++;
      append_to_buffer(&buffer, DATUM_GET_VERTEX(args[i]),
                       VARSIZE_ANY(args[i]));
    } else {
      ...
      if (types[i] == EDGEOID) {
        append_to_buffer(&buffer, DATUM_GET_EDGE(args[i]),
                         VARSIZE_ANY(args[i]));
        cnt++;
      } else {
