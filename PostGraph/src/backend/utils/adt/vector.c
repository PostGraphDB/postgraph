...
gtype_value *InitVectorGType(int dim) {
  //  int size = VECTOR_SIZE(dim) + sizeof(enum gtype_value_type) +
  //  sizeof(float8);
...
Datum l2_distance(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_TSQUERY(lhs) || GT_IS_TSQUERY(rhs)) {
    TSQuery tsquery = DatumGetPointer(
        DirectFunctionCall3(tsquery_phrase_distance, GT_TO_TSQUERY_DATUM(lhs),
                            GT_TO_TSQUERY_DATUM(rhs), Int32GetDatum(1)));
  ...
  if (AGT_ROOT_COUNT(lhs) != AGT_ROOT_COUNT(rhs))
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("different vector dimensions %i and %i",
                           AGT_ROOT_COUNT(lhs), AGT_ROOT_COUNT(rhs))));
  ...
  float8 distance = 0.0;
  while ((type = gtype_iterator_next(&lhs_it, &lgtv, false)) !=
         WGT_END_VECTOR) {
    gtype_iterator_next(&rhs_it, &rgtv, true);
  ...
  gtype_value gtv = {.type = AGTV_FLOAT, .val.float_value = sqrt(distance)};
...
Datum gtype_negative_inner_product(PG_FUNCTION_ARGS) {
  ...
  gtype_value gtv = {.type = AGTV_FLOAT, .val.float_value = distance * -1};
...
Datum gtype_cosine_distance(PG_FUNCTION_ARGS) {
  ...
  gtype_value gtv = {.type = AGTV_FLOAT, .val.float_value = 1.0 - similarity};
...
Datum gtype_norm(PG_FUNCTION_ARGS) {
  ...
  gtype_value gtv = {.type = AGTV_FLOAT, .val.float_value = sqrt(norm)};
