...
Datum edge_out(PG_FUNCTION_ARGS) {
  ...
  appendStringInfoString(str, DatumGetCString(DirectFunctionCall1(
                                  int8out, Int64GetDatum(startid))));

  // end_id
  appendStringInfoString(str, ", \"end_id\": ");
  graphid endid = *((int64 *)(&v->children[4]));
  appendStringInfoString(
      str, DatumGetCString(DirectFunctionCall1(int8out, Int64GetDatum(endid))));
...
void append_edge_to_string(StringInfoData *str, edge *v) {
  ...
  appendStringInfoString(
      str, DatumGetCString(DirectFunctionCall1(
               int8out, Int64GetDatum(*((int64 *)(&v->children[0]))))));

  // start_id
  appendStringInfoString(str, ", \"start_id\": ");
  appendStringInfoString(
      str, DatumGetCString(DirectFunctionCall1(
               int8out, Int64GetDatum(*((int64 *)(&v->children[2]))))));

  // end_id
  appendStringInfoString(str, ", \"end_id\": ");
  appendStringInfoString(
      str, DatumGetCString(DirectFunctionCall1(
               int8out, Int64GetDatum(*((int64 *)(&v->children[4]))))));
...
edge *create_edge(graphid id, graphid start_id, graphid end_id, Oid graph_oid,
                  gtype *properties) {
...
Datum edge_property_access_gtype(PG_FUNCTION_ARGS) {
  edge *v = AG_GET_ARG_EDGE(0);
  gtype *agt = extract_edge_properties(v);
...
Datum edge_eq(PG_FUNCTION_ARGS) {
...
Datum edge_ne(PG_FUNCTION_ARGS) {
  edge *lhs = AG_GET_ARG_EDGE(0);
  edge *rhs = AG_GET_ARG_EDGE(1);
...
Datum edge_lt(PG_FUNCTION_ARGS) {
  edge *lhs = AG_GET_ARG_EDGE(0);
  edge *rhs = AG_GET_ARG_EDGE(1);
...
Datum edge_le(PG_FUNCTION_ARGS) {
  edge *lhs = AG_GET_ARG_EDGE(0);
  edge *rhs = AG_GET_ARG_EDGE(1);
...
Datum edge_gt(PG_FUNCTION_ARGS) {
  edge *lhs = AG_GET_ARG_EDGE(0);
  edge *rhs = AG_GET_ARG_EDGE(1);
...
Datum edge_ge(PG_FUNCTION_ARGS) {
  edge *lhs = AG_GET_ARG_EDGE(0);
  edge *rhs = AG_GET_ARG_EDGE(1);
...
Datum edge_id(PG_FUNCTION_ARGS) {
  edge *v = AG_GET_ARG_EDGE(0);
...
Datum edge_start_id(PG_FUNCTION_ARGS) {
  edge *v = AG_GET_ARG_EDGE(0);

  gtype_value gtv = {.type = AGTV_INTEGER,
                     .val = {.int_value = *((int64 *)(&v->children[2]))}};

  AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv));
}

PG_FUNCTION_INFO_V1(edge_end_id);
Datum edge_end_id(PG_FUNCTION_ARGS) {
  edge *v = AG_GET_ARG_EDGE(0);

  gtype_value gtv = {.type = AGTV_INTEGER,
                     .val = {.int_value = *((int64 *)(&v->children[4]))}};

  AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv));
}

PG_FUNCTION_INFO_V1(edge_label);
Datum edge_label(PG_FUNCTION_ARGS) {
  edge *v = AG_GET_ARG_EDGE(0);
...
Datum edge_properties(PG_FUNCTION_ARGS) {
...
static Datum get_vertex(Oid graph_oid, int64 graphid) {
  ...
  if (!HeapTupleIsValid(tuple))
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_TABLE),
                    errmsg("id %lu does not exist", graphid)));

  tupdesc = RelationGetDescr(graph_vertex_label);

  properties =
      column_get_datum(tupdesc, tuple, 1, "properties", GTYPEOID, true);

  table_endscan(scan_desc);
  table_close(graph_vertex_label, ShareLock);

  return VERTEX_GET_DATUM(
      create_vertex(graphid, graph_oid, DATUM_GET_GTYPE_P(properties)));
}
...
Datum edge_collect_transfn(PG_FUNCTION_ARGS) {
...
Datum edge_collect_finalfn(PG_FUNCTION_ARGS) {
  Datum result;
  ArrayBuildState *state;
  int dims[1];
  int lbs[1];
...
Datum edge_collect_transfn_w_limit(PG_FUNCTION_ARGS) {
  Oid arg1_typeid = EDGEOID;
  MemoryContext aggcontext;
  ArrayBuildState *state;
  Datum elem;
...
char *extract_edge_label(edge *e) {
