...
Datum gtype_tsquery_or(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_TSQUERY(lhs) || GT_IS_TSQUERY(rhs)) {
    TSQuery tsquery = DatumGetPointer(DirectFunctionCall2(
        tsquery_or, GT_ARG_TO_TSQUERY_DATUM(0), GT_ARG_TO_TSQUERY_DATUM(1)));
    ...
    TSVector tsvector = DatumGetPointer(
        DirectFunctionCall2(tsvector_concat, GT_ARG_TO_TSVECTOR_DATUM(0),
...
Datum gtype_tsquery_not(PG_FUNCTION_ARGS) {
  TSQuery tsquery = DatumGetPointer(
      DirectFunctionCall1(tsquery_not, GT_ARG_TO_TSQUERY_DATUM(0)));
...
Datum gtype_ts_delete(PG_FUNCTION_ARGS) {
  TSVector tsvector = DatumGetPointer(
      DirectFunctionCall2(tsvector_delete_str, GT_ARG_TO_TSVECTOR_DATUM(0),
...
Datum gtype_ts_strip(PG_FUNCTION_ARGS) {
  TSVector tsvector = DatumGetPointer(
      DirectFunctionCall1(tsvector_strip, GT_ARG_TO_TSVECTOR_DATUM(0)));
...
Datum gtype_tsquery_phrase(PG_FUNCTION_ARGS) {
  TSQuery tsquery = DatumGetPointer(
      DirectFunctionCall3(tsquery_phrase_distance, GT_ARG_TO_TSQUERY_DATUM(0),
                          GT_ARG_TO_TSQUERY_DATUM(1), Int32GetDatum(1)));
...
Datum gtype_tsquery_phrase_distance(PG_FUNCTION_ARGS) {
  TSQuery tsquery = DatumGetPointer(
      DirectFunctionCall3(tsquery_phrase_distance, GT_ARG_TO_TSQUERY_DATUM(0),
                          GT_ARG_TO_TSQUERY_DATUM(1), GT_ARG_TO_INT4_DATUM(2)));
...
Datum gtype_plainto_tsquery(PG_FUNCTION_ARGS) {
  TSQuery tsquery = DatumGetPointer(
      DirectFunctionCall1(plainto_tsquery, GT_ARG_TO_TEXT_DATUM(0)));
...
Datum gtype_phraseto_tsquery(PG_FUNCTION_ARGS) {
  TSQuery tsquery = DatumGetPointer(
      DirectFunctionCall1(phraseto_tsquery, GT_ARG_TO_TEXT_DATUM(0)));
...
Datum gtype_websearch_to_tsquery(PG_FUNCTION_ARGS) {
  TSQuery tsquery = DatumGetPointer(
      DirectFunctionCall1(websearch_to_tsquery, GT_ARG_TO_TEXT_DATUM(0)));
