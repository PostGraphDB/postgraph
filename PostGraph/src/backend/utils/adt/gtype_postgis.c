...
Datum PostGraphDirectFunctionCall1(PGFunction func, Oid collation,
                                   bool *is_null, Datum arg1) {
...
Datum PostGraphDirectFunctionCall3(PGFunction func, Oid collation,
...
Datum PostGraphDirectFunctionCall4(PGFunction func, Oid collation,
                                   bool *is_null, Datum arg1, Datum arg2,
...
Datum PostGraphDirectFunctionCall5(PGFunction func, Oid collation,
                                   bool *is_null, Datum arg1, Datum arg2,
...
Datum gtype_asEWKT(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);

  Datum d = DirectFunctionCall1(
      LWGEOM_asEWKT,
      convert_to_scalar(gtype_to_geometry_internal, gt, "geometry"));

  gtype_value gtv = {.type = AGTV_STRING,
                     .val.string = {VARSIZE_ANY_EXHDR(d), VARDATA_ANY(d)}};
...
Datum gtype_addBBOX(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
  ...
  gtype_value gtv = {.type = AGTV_GSERIALIZED,
                     .val.gserialized = DatumGetPointer(d)};
...
Datum gtype_dropBBOX(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_x_point(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt, "geometry"));
  POINT4D pt;
...
Datum gtype_y_point(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt, "geometry"));
  POINT4D pt;
...
Datum gtype_z_point(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt, "geometry"));
  POINT4D pt;
  ...
  if (!gserialized_has_z(geom) ||
      (gserialized_peek_first_point(geom, &pt) == LW_FAILURE))
    PG_RETURN_NULL();
...
Datum gtype_m_point(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt, "geometry"));
  POINT4D pt;
...
Datum gtype_xmin(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);

  Datum d = DirectFunctionCall1(
      BOX3D_xmin, convert_to_scalar(gtype_to_box3d_internal, gt_1, "box3d"));
...
Datum gtype_xmax(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);

  Datum d = DirectFunctionCall1(
      BOX3D_xmax, convert_to_scalar(gtype_to_box3d_internal, gt_1, "box3d"));
...
Datum gtype_ymin(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);

  Datum d = DirectFunctionCall1(
      BOX3D_ymin, convert_to_scalar(gtype_to_box3d_internal, gt_1, "box3d"));
...
Datum gtype_zmin(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);

  Datum d = DirectFunctionCall1(
      BOX3D_zmin, convert_to_scalar(gtype_to_box3d_internal, gt_1, "box3d"));
...
Datum gtype_ymax(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);

  Datum d = DirectFunctionCall1(
      BOX3D_ymax, convert_to_scalar(gtype_to_box3d_internal, gt_1, "box3d"));
...
Datum gtype_zmax(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);

  Datum d = DirectFunctionCall1(
      BOX3D_zmax, convert_to_scalar(gtype_to_box3d_internal, gt_1, "box3d"));
...
Datum gtype_within_2d(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
...
#define GEOMETRIC2DOPERATOR(type, postgis_type)                                \
  ...
  Datum gtype_##postgis_type(PG_FUNCTION_ARGS) {                               \
    gtype *lhs = AG_GET_ARG_GTYPE_P(0);                                        \
    gtype *rhs = AG_GET_ARG_GTYPE_P(1);                                        \
                                                                               \
    if (GT_IS_BOX(lhs) && GT_IS_BOX(rhs))                                      \
      PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(                         \
          box_##type, GT_TO_BOX_DATUM(lhs), GT_TO_BOX_DATUM(rhs))));           \
    else if (GT_IS_POLYGON(lhs) && GT_IS_POLYGON(rhs))                         \
      PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(                         \
          poly_##type, GT_TO_POLYGON_DATUM(lhs), GT_TO_POLYGON_DATUM(rhs))));  \
    else if (GT_IS_CIRCLE(lhs) && GT_IS_CIRCLE(rhs))                           \
      PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(                         \
          circle_##type, GT_TO_CIRCLE_DATUM(lhs), GT_TO_CIRCLE_DATUM(rhs))));  \
                                                                               \
...
Datum gtype_gserialized_gist_consistent_2d(PG_FUNCTION_ARGS) {
  Datum d1 = PG_GETARG_DATUM(0);
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(1), "geometry");
  Datum d3 = PG_GETARG_DATUM(2);
  Datum d4 = PG_GETARG_DATUM(3);
  Datum d5 = PG_GETARG_DATUM(4);
  bool is_null;

  Datum d = PostGraphDirectFunctionCall5(gserialized_gist_consistent_2d, 100,
                                         &is_null, d1, d2, d3, d4, d5);
...
Datum gtype_gserialized_gist_distance_2d(PG_FUNCTION_ARGS) {
  Datum d1 = PG_GETARG_DATUM(0);
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(1), "geometry");
  Datum d3 = PG_GETARG_DATUM(2);
  ...
  Datum d = PostGraphDirectFunctionCall4(gserialized_gist_distance_2d, 100,
                                         &is_null, d1, d2, d3, d4);
...
Datum gtype_gserialized_gist_compress_2d(PG_FUNCTION_ARGS) {
  // Datum d1 = PG_GETARG_DATUM(0);
  GISTENTRY *entry_in = (GISTENTRY *)PG_GETARG_POINTER(0);
  entry_in->key =
      convert_to_scalar(gtype_to_geometry_internal, entry_in->key, "geometry");
  // Datum d1 = convert_to_scalar(gtype_to_geometry_internal,
  // AG_GET_ARG_GTYPE_P(0), "geometry");

  bool is_null;

  Datum d = PostGraphDirectFunctionCall1(gserialized_gist_compress_2d, 100,
                                         &is_null, PointerGetDatum(entry_in));
...
Datum gtype_gserialized_gist_penalty_2d(PG_FUNCTION_ARGS) {
  Datum d1 = PG_GETARG_DATUM(0);
  ...
  Datum d = PostGraphDirectFunctionCall3(gserialized_gist_penalty_2d, 100,
                                         &is_null, d1, d2, d3);
...
Datum gtype_gserialized_gist_union_2d(PG_FUNCTION_ARGS) {
  Datum d1 = PG_GETARG_DATUM(0);
  Datum d2 = PG_GETARG_DATUM(1);

  bool is_null;

  Datum d = PostGraphDirectFunctionCall2(gserialized_gist_union_2d, 100,
                                         &is_null, d1, d2);
...
Datum gtype_gserialized_gist_same_2d(PG_FUNCTION_ARGS) {
  Datum d1 = PG_GETARG_DATUM(0);
  Datum d2 = PG_GETARG_DATUM(1);
  Datum d3 = PG_GETARG_DATUM(2);
  ...
  Datum d = PostGraphDirectFunctionCall3(gserialized_gist_same_2d, 100,
                                         &is_null, d1, d2, d3);
...
Datum gtype_gserialized_gist_sortsupport_2d(PG_FUNCTION_ARGS) {
  Datum d1 = PG_GETARG_DATUM(0);

  bool is_null;

  PostGraphDirectFunctionCall1(gserialized_gist_sortsupport_2d, 100, &is_null,
                               d1);
...
Datum gtype_gserialized_gist_picksplit_2d(PG_FUNCTION_ARGS) {
  Datum d1 = PG_GETARG_DATUM(0);
  Datum d2 = PG_GETARG_DATUM(1);

  bool is_null;

  Datum d = PostGraphDirectFunctionCall2(gserialized_gist_picksplit_2d, 100,
                                         &is_null, d1, d2);
...
Datum gtype_gserialized_overlaps(PG_FUNCTION_ARGS) {
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(0), "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(1), "geometry");
  ...
  Datum d =
      PostGraphDirectFunctionCall2(gserialized_overlaps, 100, &is_null, d1, d2);
...
Datum gtype_gserialized_contains(PG_FUNCTION_ARGS) {
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(0), "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(1), "geometry");
  ...
  Datum d =
      PostGraphDirectFunctionCall2(gserialized_contains, 100, &is_null, d1, d2);
...
Datum gtype_gserialized_within(PG_FUNCTION_ARGS) {
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(0), "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(1), "geometry");
  ...
  Datum d =
      PostGraphDirectFunctionCall2(gserialized_within, 100, &is_null, d1, d2);
...
Datum gtype_gserialized_same(PG_FUNCTION_ARGS) {
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(0), "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(1), "geometry");
  ...
  Datum d =
      PostGraphDirectFunctionCall2(gserialized_same, 100, &is_null, d1, d2);
...
Datum gtype_st_intersection(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
...
Datum gtype_ST_ClosestPointOfApproach(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);

  GSERIALIZED *d1 =
      convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry");
  GSERIALIZED *d2 =
      convert_to_scalar(gtype_to_geometry_internal, gt_2, "geometry");
...
Datum gtype_ST_DistanceCPA(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
...
Datum gtype_length_linestring(PG_FUNCTION_ARGS) {
  ...
  Datum d =
      PostGraphDirectFunctionCall1(LWGEOM_length_linestring, 100, &is_null, d1);
...
Datum gtype_length2d_linestring(PG_FUNCTION_ARGS) {
  ...
  Datum d = PostGraphDirectFunctionCall1(LWGEOM_length2d_linestring, 100,
                                         &is_null, d1);
...
Datum gtype_length_ellipsoid_linestring(PG_FUNCTION_ARGS) {
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal,
                               AG_GET_ARG_GTYPE_P(0), "geometry");
  Datum d2 = convert_to_scalar(gtype_to_spheroid_internal,
                               AG_GET_ARG_GTYPE_P(1), "sphereoid");
  bool is_null;

  Datum d = PostGraphDirectFunctionCall2(LWGEOM_length2d_linestring, 100,
                                         &is_null, d1, d2);
...
Datum gtype_length2d_ellipsoid(PG_FUNCTION_ARGS) {
  ...
  Datum d = PostGraphDirectFunctionCall2(LWGEOM_length2d_ellipsoid, 100,
                                         &is_null, d1, d2);
...
Datum gtype_st_isvalidtrajectory(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_ST_CPAWithin(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  gtype *gt_3 = AG_GET_ARG_GTYPE_P(2);
...
Datum gtype_st_generatepoints(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  GSERIALIZED *gser_input = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry"));
  GSERIALIZED *gser_result;
  LWGEOM *lwgeom_input;
  LWGEOM *lwgeom_result;
  int32 npoints =
      DatumGetInt32(convert_to_scalar(gtype_to_int4_internal, gt_2, "int4"));
  int32 seed = 0;
  ...
  if (PG_NARGS() > 2 && !PG_ARGISNULL(2)) {
    gtype *gt_3 = AG_GET_ARG_GTYPE_P(2);
    seed =
        DatumGetInt32(convert_to_scalar(gtype_to_int4_internal, gt_3, "int4"));
    if (seed < 1)
      ereport(ERROR,
  ...
  gtype_value gtv = {.type = AGTV_GSERIALIZED,
                     .val.gserialized = geometry_serialize(lwgeom_result)};
...
Datum gtype_ST_Simplify(PG_FUNCTION_ARGS) {
  ...
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry"));
  double dist = DatumGetFloat8(
      convert_to_scalar(gtype_to_float8_internal, gt_2, "float"));
  GSERIALIZED *result;
  ...
  if (PG_NARGS() > 2 && !PG_ARGISNULL(2))
    preserve_collapsed = DatumGetBool(convert_to_scalar(
        gtype_to_boolean_internal, AG_GET_ARG_GTYPE_P(2), "boolean"));
  ...
  gtype_value gtv = {.type = AGTV_GSERIALIZED,
                     .val.gserialized = geometry_serialize(in)};
...
Datum gtype_affine(PG_FUNCTION_ARGS) {
  ...
  affine.afac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(1), "float"));
  affine.bfac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(2), "float"));
  affine.cfac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(3), "float"));
  affine.dfac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(4), "float"));
  affine.efac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(5), "float"));
  affine.ffac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(6), "float"));
  affine.gfac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(7), "float"));
  affine.hfac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(8), "float"));
  affine.ifac = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(9), "float"));
  affine.xoff = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(10), "float"));
  affine.yoff = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(11), "float"));
  affine.zoff = DatumGetFloat8(convert_to_scalar(
      gtype_to_float8_internal, AG_GET_ARG_GTYPE_P(12), "float"));
  ...
  gtype_value gtv = {.type = AGTV_GSERIALIZED,
                     .val.gserialized = geometry_serialize(lwgeom)};
...
Datum gtype_st_scale_2_args(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
...
Datum gtype_st_scale(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  gtype *gt_3 = AG_GET_ARG_GTYPE_P(2);
...
Datum gtype_makepoint(PG_FUNCTION_ARGS) {
  ...
  x = DatumGetFloat8(convert_to_scalar(gtype_to_float8_internal,
                                       AG_GET_ARG_GTYPE_P(0), "float"));
  y = DatumGetFloat8(convert_to_scalar(gtype_to_float8_internal,
                                       AG_GET_ARG_GTYPE_P(1), "float"));

  if (PG_NARGS() == 2)
    point = lwpoint_make2d(SRID_UNKNOWN, x, y);
  else if (PG_NARGS() == 3) {
    z = DatumGetFloat8(convert_to_scalar(gtype_to_float8_internal,
                                         AG_GET_ARG_GTYPE_P(2), "float"));
    point = lwpoint_make3dz(SRID_UNKNOWN, x, y, z);
  } else if (PG_NARGS() == 4) {
    z = DatumGetFloat8(convert_to_scalar(gtype_to_float8_internal,
                                         AG_GET_ARG_GTYPE_P(2), "float"));
    m = DatumGetFloat8(convert_to_scalar(gtype_to_float8_internal,
                                         AG_GET_ARG_GTYPE_P(3), "float"));
  ...
  gtype_value gtv = {.type = AGTV_GSERIALIZED,
                     .val.gserialized = geometry_serialize((LWGEOM *)point)};
...
Datum gtype_snaptogrid_pointoff(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  gtype *gt_3 = AG_GET_ARG_GTYPE_P(2);
  ...
  Datum d =
      DirectFunctionCall6(LWGEOM_snaptogrid_pointoff, d1, d2, d3, d4, d5, d6);

  gtype_value gtv = {.type = AGTV_GSERIALIZED,
                     .val.boolean = DatumGetPointer(d)};
...
Datum gtype_snaptogrid(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  gtype *gt_3 = AG_GET_ARG_GTYPE_P(2);
  gtype *gt_4 = AG_GET_ARG_GTYPE_P(3);
...
Datum gtype_ST_IsPolygonCW(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry"));
  LWGEOM *input;
...
Datum gtype_ST_IsPolygonCCW(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry"));
  LWGEOM *input;
...
Datum gtype_azimuth(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  GSERIALIZED *geom = DatumGetPointer(
      convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry"));
  LWPOINT *lwpoint;
  ...
  geom = DatumGetPointer(convert_to_scalar(gtype_to_geometry_internal,
                                           AG_GET_ARG_GTYPE_P(1), "geometry"));
  lwpoint = lwgeom_as_lwpoint(lwgeom_from_gserialized(geom));
  if (!lwpoint)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
Datum gtype_distance_ellipsoid(PG_FUNCTION_ARGS) {
  ...
  if (PG_NARGS() == 2) {
    ...
    Datum d3 = convert_to_scalar(gtype_to_spheroid_internal,
                                 AG_GET_ARG_GTYPE_P(2), "geometry");
...
Datum gtype_force_2d(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_force_3dz(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry");
  Datum d2 = convert_to_scalar(gtype_to_float8_internal, gt_2, "float");
...
Datum gtype_force_3dm(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry");
  Datum d2 = convert_to_scalar(gtype_to_float8_internal, gt_2, "float");
...
Datum gtype_force_4d(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  gtype *gt_3 = AG_GET_ARG_GTYPE_P(2);
...
Datum gtype_force_collection(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_force_multi(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_convexhull(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_st_symdifference(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal, gt_2, "geometry");
...
/**
 *  @brief Compute the Hausdorff distance thanks to the corresponding GEOS
 * function
 *  @example ST_HausdorffDistance {@link #hausdorffdistance} - SELECT
 * ST_HausdorffDistance( 'POLYGON((0 0, 0 2, 1 2, 2 2, 2 0, 0 0))'::geometry,
 *      'POLYGON((0.5 0.5, 0.5 2.5, 1.5 2.5, 2.5 2.5, 2.5 0.5, 0.5
 * 0.5))'::geometry);
 */
PG_FUNCTION_INFO_V1(hausdorffdistance);
PG_FUNCTION_INFO_V1(gtype_hausdorffdistance);
Datum gtype_hausdorffdistance(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal, gt_2, "geometry");
  ...
  Datum d =
      PostGraphDirectFunctionCall2(hausdorffdistance, 100, &is_null, d1, d2);
...
/**
 *  @brief Compute the Hausdorff distance with densification thanks to the
 * corresponding GEOS function
 *  @example hausdorffdistancedensify {@link #hausdorffdistancedensify} - SELECT
 * ST_HausdorffDistance( 'POLYGON((0 0, 0 2, 1 2, 2 2, 2 0, 0 0))'::geometry,
 *      'POLYGON((0.5 0.5, 0.5 2.5, 1.5 2.5, 2.5 2.5, 2.5 0.5, 0.5
 * 0.5))'::geometry, 0.5);
 */
...
Datum gtype_hausdorffdistancedensify(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal, gt_2, "geometry");
  ...
  Datum d = PostGraphDirectFunctionCall3(hausdorffdistancedensify, 100,
                                         &is_null, d1, d2, d3);
...
/**
 *  @brief Compute the Frechet distance with optional densification thanks to
 * the corresponding GEOS function
 *  @example ST_FrechetDistance {@link #frechetdistance} - SELECT
 * ST_FrechetDistance( 'LINESTRING (0 0, 50 200, 100 0, 150 200, 200
 * 0)'::geometry, 'LINESTRING (0 200, 200 150, 0 100, 200 50, 0 0)'::geometry,
 * 0.5);
 */
PG_FUNCTION_INFO_V1(ST_FrechetDistance);
PG_FUNCTION_INFO_V1(gtype_st_frechet_distance);
Datum gtype_st_frechet_distance(PG_FUNCTION_ARGS) {
  gtype *gt_1 = AG_GET_ARG_GTYPE_P(0);
  gtype *gt_2 = AG_GET_ARG_GTYPE_P(1);
  Datum d1 = convert_to_scalar(gtype_to_geometry_internal, gt_1, "geometry");
  Datum d2 = convert_to_scalar(gtype_to_geometry_internal, gt_2, "geometry");
  ...
  Datum d = PostGraphDirectFunctionCall3(ST_FrechetDistance, 100, &is_null, d1,
                                         d2, d3);
