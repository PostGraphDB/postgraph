...
Datum gtype_age_today(PG_FUNCTION_ARGS) {
  ...
  ts = DatumGetTimestamp(DirectFunctionCall2(
      timestamp_trunc, cstring_to_text_with_len("day", 3), ts));

  if (agtv1->type != AGTV_TIMESTAMP)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("age(gtype) only supports timestamps")));

  i = DatumGetIntervalP(DirectFunctionCall2(
      timestamp_mi, TimestampGetDatum(agtv1->val.int_value), ts));
...
Datum gtype_age_w2args(PG_FUNCTION_ARGS) {
  ...
  if (lhs_value->type == AGTV_TIMESTAMP && rhs_value->type == AGTV_TIMESTAMP)
    i = DatumGetIntervalP(DirectFunctionCall2(
        timestamp_age, TimestampGetDatum(lhs_value->val.int_value),
        TimestampGetDatum(rhs_value->val.int_value)));
  else if (lhs_value->type == AGTV_TIMESTAMPTZ &&
           rhs_value->type == AGTV_TIMESTAMPTZ)
    i = DatumGetIntervalP(DirectFunctionCall2(
        timestamptz_age, TimestampTzGetDatum(lhs_value->val.int_value),
...
Datum gtype_extract(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);
  ...
  if (rhs_value->type == AGTV_TIMESTAMP) {
    gtv.val.numeric = DatumGetNumeric(
        DirectFunctionCall2(extract_timestamp, CStringGetTextDatum(field),
                            TimestampGetDatum(rhs_value->val.int_value)));
  } else if (rhs_value->type == AGTV_TIMESTAMPTZ) {
    gtv.val.numeric = DatumGetNumeric(
        DirectFunctionCall2(extract_timestamptz, CStringGetTextDatum(field),
                            TimestampTzGetDatum(rhs_value->val.int_value)));
  } else if (rhs_value->type == AGTV_DATE) {
    gtv.val.numeric = DatumGetNumeric(
        DirectFunctionCall2(extract_date, CStringGetTextDatum(field),
                            DateADTGetDatum(rhs_value->val.date)));
  } else if (rhs_value->type == AGTV_TIME) {
    gtv.val.numeric = DatumGetNumeric(
        DirectFunctionCall2(extract_time, CStringGetTextDatum(field),
                            TimeADTGetDatum(rhs_value->val.int_value)));
  } else if (rhs_value->type == AGTV_TIMETZ) {
    gtv.val.numeric = DatumGetNumeric(
        DirectFunctionCall2(extract_timetz, CStringGetTextDatum(field),
                            TimeTzADTPGetDatum(&rhs_value->val.timetz)));
  } else if (rhs_value->type == AGTV_INTERVAL) {
    gtv.val.numeric = DatumGetNumeric(
        DirectFunctionCall2(extract_interval, CStringGetTextDatum(field),
...
Datum gtype_date_part(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);
  ...
  gtype_value gtv;
  if (rhs_value->type == AGTV_TIMESTAMP) {
    gtv.val.float_value = DatumGetFloat8(
        DirectFunctionCall2(timestamp_part, CStringGetTextDatum(field),
                            TimestampGetDatum(rhs_value->val.int_value)));
  } else if (rhs_value->type == AGTV_TIMESTAMPTZ) {
    gtv.val.float_value = DatumGetFloat8(
        DirectFunctionCall2(timestamptz_part, CStringGetTextDatum(field),
                            TimestampTzGetDatum(rhs_value->val.int_value)));
  } else if (rhs_value->type == AGTV_DATE) {
    Datum ts = gtype_to_timestamptz_internal(rhs_value);
    gtv.val.float_value = DatumGetFloat8(
        DirectFunctionCall2(timestamptz_part, CStringGetTextDatum(field), ts));
  } else if (rhs_value->type == AGTV_TIME) {
    gtv.val.float_value = DatumGetFloat8(
        DirectFunctionCall2(time_part, CStringGetTextDatum(field),
                            TimeADTGetDatum(rhs_value->val.int_value)));
  } else if (rhs_value->type == AGTV_TIMETZ) {
    gtv.val.float_value = DatumGetFloat8(
        DirectFunctionCall2(timetz_part, CStringGetTextDatum(field),
                            TimeTzADTPGetDatum(&rhs_value->val.timetz)));
  } else if (rhs_value->type == AGTV_INTERVAL) {
    gtv.val.float_value = DatumGetFloat8(
        DirectFunctionCall2(interval_part, CStringGetTextDatum(field),
                            IntervalPGetDatum(&rhs_value->val.interval)));
  } else {
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
Datum gtype_make_date(PG_FUNCTION_ARGS) {
  ...
  if (is_gtype_null(agt_year) || is_gtype_null(agt_month) ||
      is_gtype_null(agt_day))
    PG_RETURN_NULL();
  ...
  if (agtv_year->type != AGTV_INTEGER || agtv_month->type != AGTV_INTEGER ||
      agtv_day->type != AGTV_INTEGER)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("make_date(gtype, gtype, gtype) only supports "
                           "integer arugments")));

  agtv.type = AGTV_DATE;
  agtv.val.date = DatumGetDateADT(DirectFunctionCall3(
      make_date, Int32GetDatum((int32)agtv_year->val.int_value),
...
Datum gtype_make_time(PG_FUNCTION_ARGS) {
  ...
  if (is_gtype_null(agt_hour) || is_gtype_null(agt_minute) ||
      is_gtype_null(agt_second))
    PG_RETURN_NULL();
  ...
  if (agtv_hour->type != AGTV_INTEGER || agtv_minute->type != AGTV_INTEGER ||
      agtv_second->type != AGTV_FLOAT)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("make_time(gtype, gtype, gtype) only supports "
                           "integer arugments")));

  agtv.type = AGTV_TIME;
  agtv.val.int_value = DatumGetTimeADT(DirectFunctionCall3(
      make_time, Int32GetDatum((int32)agtv_hour->val.int_value),
...
Datum gtype_make_timestamp(PG_FUNCTION_ARGS) {
  ...
  if (is_gtype_null(agt_year) || is_gtype_null(agt_month) ||
      is_gtype_null(agt_day) || is_gtype_null(agt_hour) ||
      is_gtype_null(agt_minute) || is_gtype_null(agt_second))
    PG_RETURN_NULL();
  ...
  if (agtv_year->type != AGTV_INTEGER || agtv_month->type != AGTV_INTEGER ||
      agtv_day->type != AGTV_INTEGER || agtv_hour->type != AGTV_INTEGER ||
      agtv_minute->type != AGTV_INTEGER || agtv_second->type != AGTV_FLOAT)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("make_time(gtype, gtype, gtype) only supports "
                           "integer arugments")));

  agtv.type = AGTV_TIMESTAMP;
  agtv.val.int_value = DatumGetTimestamp(DirectFunctionCall6(
      make_timestamp, Int32GetDatum((int32)agtv_year->val.int_value),
      Int32GetDatum((int32)agtv_month->val.int_value),
      Int32GetDatum((int32)agtv_day->val.int_value),
...
Datum gtype_make_timestamptz(PG_FUNCTION_ARGS) {
  ...
  agtv.val.int_value = DatumGetTimestampTz(DirectFunctionCall6(
      make_timestamp, Int32GetDatum((int32)agtv_year->val.int_value),
      Int32GetDatum((int32)agtv_month->val.int_value),
      Int32GetDatum((int32)agtv_day->val.int_value),
      Int32GetDatum((int32)agtv_hour->val.int_value),
...
Datum gtype_make_timestamptz_wtimezone(PG_FUNCTION_ARGS) {
  ...
  agtv.type = AGTV_TIMESTAMPTZ;
  agtv.val.int_value = DatumGetTimestampTz(DirectFunctionCall7(
      ...
      PointerGetDatum(cstring_to_text_with_len(
          agtv_timezone->val.string.val, agtv_timezone->val.string.len))));
...
Datum gtype_isfinite(PG_FUNCTION_ARGS) {
  ...
  if (agtv->type == AGTV_DATE)
    result = DatumGetBool(
        DirectFunctionCall1(date_finite, DateADTGetDatum(agtv->val.date)));
  else if (agtv->type == AGTV_INTERVAL)
    result = true;
  else if (agtv->type == AGTV_TIMESTAMP || agtv->type == AGTV_TIMESTAMPTZ) {
    result = DatumGetBool(DirectFunctionCall1(
        timestamp_finite, TimestampGetDatum(agtv->val.int_value)));
  } else
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
Datum gtype_justify_days(PG_FUNCTION_ARGS) {
  ...
  Interval *i = DatumGetIntervalP(DirectFunctionCall1(
      interval_justify_days, IntervalPGetDatum(&gt_value->val.interval)));
  gtv.type = AGTV_INTERVAL;
  gtv.val.interval.time = i->time;
  gtv.val.interval.day = i->day;
  gtv.val.interval.month = i->month;
...
Datum gtype_justify_hours(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
  ...
  Interval *i = DatumGetIntervalP(DirectFunctionCall1(
      interval_justify_hours, IntervalPGetDatum(&gt_value->val.interval)));
  gtv.type = AGTV_INTERVAL;
  gtv.val.interval.time = i->time;
  gtv.val.interval.day = i->day;
  gtv.val.interval.month = i->month;
...
Datum gtype_justify_interval(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
  ...
  Interval *i = DatumGetIntervalP(DirectFunctionCall1(
      interval_justify_interval, IntervalPGetDatum(&gt_value->val.interval)));
  gtv.type = AGTV_INTERVAL;
  gtv.val.interval.time = i->time;
  gtv.val.interval.day = i->day;
  gtv.val.interval.month = i->month;
...
Datum gtype_date_trunc(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);
  ...
  gtype_value gtv;
  if (rhs_value->type == AGTV_TIMESTAMP) {
    gtv.val.int_value = DatumGetTimestamp(
        DirectFunctionCall2(timestamp_trunc, CStringGetTextDatum(field),
                            TimestampGetDatum(rhs_value->val.int_value)));
    gtv.type = AGTV_TIMESTAMP;
    AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv));
  } else if (rhs_value->type == AGTV_TIMESTAMPTZ) {
    gtv.val.int_value = DatumGetTimestampTz(
        DirectFunctionCall2(timestamptz_trunc, CStringGetTextDatum(field),
                            TimestampTzGetDatum(rhs_value->val.int_value)));
    gtv.type = AGTV_TIMESTAMPTZ;
    AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv));
  } else if (rhs_value->type == AGTV_INTERVAL) {
    Interval *i = DatumGetIntervalP(
        DirectFunctionCall2(interval_trunc, CStringGetTextDatum(field),
                            IntervalPGetDatum(&rhs_value->val.interval)));
    gtv.type = AGTV_INTERVAL;
    gtv.val.interval.time = i->time;
    gtv.val.interval.day = i->day;
...
Datum gtype_date_trunc_zone(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);
  ...
  if (rhs_value->type == AGTV_TIMESTAMPTZ) {
    gtv.val.int_value = DatumGetTimestampTz(
        DirectFunctionCall3(timestamptz_trunc_zone, CStringGetTextDatum(field),
                            TimestampTzGetDatum(rhs_value->val.int_value),
...
Datum gtype_date_bin(PG_FUNCTION_ARGS) {
  ...
  gtype_value *stride_value =
      get_ith_gtype_value_from_container(&stride->root, 0);
  gtype_value *source_value =
      get_ith_gtype_value_from_container(&source->root, 0);
  gtype_value *origin_value =
      get_ith_gtype_value_from_container(&origin->root, 0);

  if ((source_value->type == AGTV_TIMESTAMP ||
       source_value->type == AGTV_DATE) &&
      (origin_value->type == AGTV_TIMESTAMP ||
       origin_value->type == AGTV_DATE)) {
    ...
    Timestamp ts = DatumGetTimestamp(DirectFunctionCall3(
        timestamp_bin, IntervalPGetDatum(&stride_value->val.interval),
        source_ts, origin_ts));
    ...
    AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv));
  } else if (source_value->type == AGTV_TIMESTAMPTZ &&
             origin_value->type == AGTV_TIMESTAMPTZ) {
    TimestampTz ts = DatumGetTimestampTz(DirectFunctionCall3(
        timestamptz_bin, IntervalPGetDatum(&stride_value->val.interval),
...
enum overlap_type { ovt_timestamp, ovt_time, ovt_timetz } overlap_type;
...
Datum gtype_overlaps(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_INTERVAL(arg2)) {
    if (type == ovt_timestamp)
      d2 = DirectFunctionCall2(timestamp_pl_interval, d1,
                               GT_TO_INTERVAL_DATUM(arg2));
    else if (type == ovt_time)
      d2 =
          DirectFunctionCall2(time_pl_interval, d1, GT_TO_INTERVAL_DATUM(arg2));
    else
      d2 = DirectFunctionCall2(timetz_pl_interval, d1,
                               GT_TO_INTERVAL_DATUM(arg2));
  } else {
    if (type == ovt_timestamp)
  ...
  if (GT_IS_INTERVAL(arg4)) {
    if (type == ovt_timestamp)
      d4 = DirectFunctionCall2(timestamp_pl_interval, d3,
                               GT_TO_INTERVAL_DATUM(arg4));
    else if (type == ovt_time)
      d4 =
          DirectFunctionCall2(time_pl_interval, d3, GT_TO_INTERVAL_DATUM(arg4));
    else
      d4 = DirectFunctionCall2(timetz_pl_interval, d3,
                               GT_TO_INTERVAL_DATUM(arg4));
  } else {
    if (type == ovt_timestamp)
  ...
  if (type == ovt_timestamp)
    result =
        DatumGetBool(DirectFunctionCall4(overlaps_timestamp, d1, d2, d3, d4));
  else if (type == ovt_time)
