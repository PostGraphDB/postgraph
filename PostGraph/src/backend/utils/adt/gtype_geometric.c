...
Datum gtype_intersection_point(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_LSEG(lhs) || GT_IS_LSEG(rhs)) {
    d = PostGraphDirectFunctionCall2(lseg_interpt, 100, &is_null,
                                     GT_TO_LSEG_DATUM(lhs),
                                     GT_TO_LSEG_DATUM(rhs));
    ...
    d = PostGraphDirectFunctionCall2(line_interpt, 100, &is_null,
                                     GT_TO_LINE_DATUM(lhs),
                                     GT_TO_LINE_DATUM(rhs));
    ...
    d = PostGraphDirectFunctionCall2(box_intersect, 100, &is_null,
                                     GT_TO_BOX_DATUM(lhs),
                                     GT_TO_BOX_DATUM(rhs));
...
Datum gtype_closest_point(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);
  ...
  if (GT_IS_POINT(lhs) && GT_IS_BOX(rhs)) {
    d = PostGraphDirectFunctionCall2(
        close_pb, 100, &is_null, GT_TO_POINT_DATUM(lhs), GT_TO_BOX_DATUM(rhs));

    if (is_null)
      PG_RETURN_NULL();
  } else if (GT_IS_POINT(lhs) && GT_IS_LSEG(rhs)) {
    d = PostGraphDirectFunctionCall2(
        close_ps, 100, &is_null, GT_TO_POINT_DATUM(lhs), GT_TO_LSEG_DATUM(rhs));

    if (is_null)
      PG_RETURN_NULL();
  } else if (GT_IS_POINT(lhs) && GT_IS_LINE(rhs)) {
    d = PostGraphDirectFunctionCall2(
        close_pl, 100, &is_null, GT_TO_POINT_DATUM(lhs), GT_TO_LINE_DATUM(rhs));

    if (is_null)
      PG_RETURN_NULL();
  } else if (GT_IS_LSEG(lhs) && GT_IS_BOX(rhs)) {
    d = PostGraphDirectFunctionCall2(
        close_sb, 100, &is_null, GT_TO_LSEG_DATUM(lhs), GT_TO_BOX_DATUM(rhs));

    if (is_null)
      PG_RETURN_NULL();
  } else if (GT_IS_LSEG(lhs) && GT_IS_LSEG(rhs)) {
    d = PostGraphDirectFunctionCall2(close_lseg, 100, &is_null,
                                     GT_TO_LSEG_DATUM(lhs),
                                     GT_TO_LSEG_DATUM(rhs));
    ...
    d = PostGraphDirectFunctionCall2(
        close_ls, 100, &is_null, GT_TO_LINE_DATUM(lhs), GT_TO_LSEG_DATUM(rhs));
...
Datum gtype_center(PG_FUNCTION_ARGS) {
...
Datum gtype_distance(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_vertical(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_horizontal(PG_FUNCTION_ARGS) {
  gtype *gt = AG_GET_ARG_GTYPE_P(0);
...
Datum gtype_perp(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);

  Datum d;
  if (GT_IS_LSEG(lhs) || GT_IS_LSEG(rhs))
    d = DirectFunctionCall2(lseg_perp, GT_TO_LSEG_DATUM(lhs),
                            GT_TO_LSEG_DATUM(rhs));
  else
    d = DirectFunctionCall2(line_perp, GT_TO_LINE_DATUM(lhs),
                            GT_TO_LINE_DATUM(rhs));
...
Datum gtype_parallel(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);

  Datum d;
  if (GT_IS_LSEG(lhs) || GT_IS_LSEG(rhs))
    d = DirectFunctionCall2(lseg_parallel, GT_TO_LSEG_DATUM(lhs),
                            GT_TO_LSEG_DATUM(rhs));
  else
    d = DirectFunctionCall2(line_parallel, GT_TO_LINE_DATUM(lhs),
                            GT_TO_LINE_DATUM(rhs));
...
Datum gtype_bound_box(PG_FUNCTION_ARGS) {
  gtype *lhs = AG_GET_ARG_GTYPE_P(0);
  gtype *rhs = AG_GET_ARG_GTYPE_P(1);

  bool is_null;
  Datum d =
      PostGraphDirectFunctionCall2(boxes_bound_box, 100, &is_null,
                                   GT_TO_BOX_DATUM(lhs), GT_TO_BOX_DATUM(rhs));
...
Datum gtype_height(PG_FUNCTION_ARGS) {
  Datum d =
      DirectFunctionCall1(box_height, GT_TO_BOX_DATUM(AG_GET_ARG_GTYPE_P(0)));
...
Datum gtype_width(PG_FUNCTION_ARGS) {
  Datum d =
      DirectFunctionCall1(box_width, GT_TO_BOX_DATUM(AG_GET_ARG_GTYPE_P(0)));
