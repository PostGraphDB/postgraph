...
static gtype_value *iterator_concat(gtype_iterator **it1, gtype_iterator **it2,
...
static char *get_string_from_gtype_value(gtype_value *agtv, int *length) {
  ...
  switch (agtv->type) {
  case AGTV_INTEGER:
    number = DirectFunctionCall1(int8out, Int8GetDatum(agtv->val.int_value));
    string = DatumGetCString(number);
    *length = strlen(string);
    return string;
  case AGTV_FLOAT:
    number =
        DirectFunctionCall1(float8out, Float8GetDatum(agtv->val.float_value));
    string = DatumGetCString(number);
    *length = strlen(string);
  ...
  case AGTV_NUMERIC:
    string = DatumGetCString(
        DirectFunctionCall1(numeric_out, PointerGetDatum(agtv->val.numeric)));
    *length = strlen(string);
    return string;
...
Datum get_numeric_datum_from_gtype_value(gtype_value *agtv) {
  switch (agtv->type) {
  case AGTV_INTEGER:
    return DirectFunctionCall1(int8_numeric, Int8GetDatum(agtv->val.int_value));
  case AGTV_FLOAT:
...
bool is_gtv_number(gtype_value *gt) {
  if (gt->type == AGTV_NUMERIC || gt->type == AGTV_INTEGER ||
      gt->type == AGTV_FLOAT)
    return true;
  return false;
}
...
Datum gtype_add(PG_FUNCTION_ARGS) {
  ...
  if ((agtv_lhs->type == AGTV_STRING &&
       (is_gtv_number(agtv_rhs) || agtv_rhs->type == AGTV_STRING)) ||
      (agtv_rhs->type == AGTV_STRING &&
       (is_gtv_number(agtv_lhs) || agtv_lhs->type == AGTV_STRING))) {
    ...
    agtv_result.val.numeric = DatumGetNumeric(numd);
             ...
             agtv_rhs->type == AGTV_INTERVAL) {
    agtv_result.type = AGTV_TIMESTAMP;
    agtv_result.val.int_value = DatumGetTimestamp(DirectFunctionCall2(
        timestamp_pl_interval, TimestampGetDatum(agtv_lhs->val.int_value),
    ...
    if (agtv_lhs->type == AGTV_TIMESTAMPTZ) {
      agtv_result.val.int_value = DatumGetTimestampTz(
          DirectFunctionCall2(timestamptz_pl_interval,
                              TimestampTzGetDatum(agtv_lhs->val.int_value), i));
    } else if (agtv_lhs->type == AGTV_DATE) {
      agtv_result.type = AGTV_TIMESTAMPTZ;
      agtv_result.val.int_value = DatumGetTimestampTz(DirectFunctionCall2(
          date_pl_interval, DateADTGetDatum(agtv_lhs->val.date), i));
    } else if (agtv_lhs->type == AGTV_TIME) {
      agtv_result.val.int_value = DatumGetTimeADT(DirectFunctionCall2(
          time_pl_interval, TimeADTGetDatum(agtv_lhs->val.int_value), i));
    } else if (agtv_lhs->type == AGTV_TIMETZ) {
      TimeTzADT *time = DatumGetTimeTzADTP(DirectFunctionCall2(
          timetz_pl_interval, TimeTzADTPGetDatum(&agtv_lhs->val.timetz), i));

      agtv_result.val.timetz.time = time->time;
      agtv_result.val.timetz.zone = time->zone;
    } else if (agtv_lhs->type == AGTV_INTERVAL) {
      Interval *interval = DatumGetIntervalP(DirectFunctionCall2(
          interval_pl, IntervalPGetDatum(&agtv_lhs->val.interval), i));
    ...
    if (agtv_lhs->type == AGTV_POINT) {
      agtv_result.val.point = DatumGetPointP(DirectFunctionCall2(
          point_add, PointPGetDatum(agtv_lhs->val.point), point));
    } else if (agtv_lhs->type == AGTV_BOX) {
      agtv_result.val.box = DatumGetBoxP(
          DirectFunctionCall2(box_add, BoxPGetDatum(agtv_lhs->val.box), point));
    } else if (agtv_lhs->type == AGTV_PATH) {
      agtv_result.val.path = DatumGetPathP(DirectFunctionCall2(
          path_add_pt, PathPGetDatum(agtv_lhs->val.path), point));
    } else if (agtv_lhs->type == AGTV_CIRCLE) {
      agtv_result.val.circle = DatumGetCircleP(DirectFunctionCall2(
          circle_add_pt, CirclePGetDatum(agtv_lhs->val.circle), point));
    } else {
      ereport_op_str("+", lhs, rhs);
    }
  } else if (agtv_lhs->type == AGTV_INET && agtv_rhs->type == AGTV_INTEGER) {
    agtv_result.type = AGTV_INET;
    inet *i = DatumGetInetPP(
        DirectFunctionCall2(inetpl, InetPGetDatum(&agtv_lhs->val.inet),
    ...
    agtv_result.type = AGTV_INET;
    inet *i = DatumGetInetPP(
        DirectFunctionCall2(inetpl, InetPGetDatum(&agtv_rhs->val.inet),
...
Datum gtype_sub(PG_FUNCTION_ARGS) {
  ...
  if (agtv_lhs->type == AGTV_INTEGER && agtv_rhs->type == AGTV_INTEGER) {
    ...
    if (agtv_lhs->type == AGTV_TIMESTAMPTZ) {
      ...
      agtv_result.type = AGTV_TIMESTAMPTZ;
      agtv_result.val.int_value = DatumGetTimestampTz(DirectFunctionCall2(
          date_mi_interval, DateADTGetDatum(agtv_lhs->val.date), i));
    } else if (agtv_lhs->type == AGTV_TIME) {
      agtv_result.val.int_value = DatumGetTimeADT(DirectFunctionCall2(
          time_mi_interval, TimeADTGetDatum(agtv_lhs->val.int_value), i));
    } else if (agtv_lhs->type == AGTV_TIMETZ) {
      TimeTzADT *time = DatumGetTimeTzADTP(DirectFunctionCall2(
          timetz_mi_interval, TimeTzADTPGetDatum(&agtv_lhs->val.timetz), i));

      agtv_result.val.timetz.time = time->time;
      agtv_result.val.timetz.zone = time->zone;
    } else if (agtv_lhs->type == AGTV_INTERVAL) {
      Interval *interval = DatumGetIntervalP(DirectFunctionCall2(
          interval_mi, IntervalPGetDatum(&agtv_lhs->val.interval), i));
    ...
    if (agtv_lhs->type == AGTV_POINT) {
      agtv_result.val.point = DatumGetPointP(DirectFunctionCall2(
          point_sub, PointPGetDatum(agtv_lhs->val.point), point));
    } else if (agtv_lhs->type == AGTV_BOX) {
      agtv_result.val.box = DatumGetBoxP(
          DirectFunctionCall2(box_sub, BoxPGetDatum(agtv_lhs->val.box), point));
    } else if (agtv_lhs->type == AGTV_PATH) {
      agtv_result.val.path = DatumGetPathP(DirectFunctionCall2(
          path_sub_pt, PathPGetDatum(agtv_lhs->val.path), point));
    } else if (agtv_lhs->type == AGTV_CIRCLE) {
      agtv_result.val.circle = DatumGetCircleP(DirectFunctionCall2(
          circle_sub_pt, CirclePGetDatum(agtv_lhs->val.circle), point));
    } else {
      ereport_op_str("+", lhs, rhs);
    }
  } else if (agtv_lhs->type == AGTV_INET && agtv_rhs->type == AGTV_INTEGER) {
    agtv_result.type = AGTV_INET;
    inet *i = DatumGetInetPP(
        DirectFunctionCall2(inetmi_int8, InetPGetDatum(&agtv_lhs->val.inet),
                            Int64GetDatum(agtv_rhs->val.int_value)));
    ...
    agtv_result.val.int_value = DatumGetInt64(
        DirectFunctionCall2(inetmi, InetPGetDatum(&agtv_lhs->val.inet),
...
Datum gtype_neg(PG_FUNCTION_ARGS) {
  ...
  if (agtv_value->type == AGTV_INTEGER) {
    ...
    Interval *interval = DatumGetIntervalP(DirectFunctionCall1(
        interval_um, IntervalPGetDatum(&agtv_value->val.interval)));
...
Datum gtype_mul(PG_FUNCTION_ARGS) {
  ...
  if (agtv_lhs->type == AGTV_INTEGER && agtv_rhs->type == AGTV_INTEGER) {
    ...
    agtv_result.type = AGTV_INTERVAL;
    Interval *interval = DatumGetIntervalP(DirectFunctionCall2(
        interval_mul, IntervalPGetDatum(&agtv_lhs->val.interval),
    ...
    if (agtv_lhs->type == AGTV_POINT) {
      agtv_result.val.point = DatumGetPointP(DirectFunctionCall2(
          point_mul, PointPGetDatum(agtv_lhs->val.point), point));
    } else if (agtv_lhs->type == AGTV_BOX) {
      agtv_result.val.box = DatumGetBoxP(
          DirectFunctionCall2(box_mul, BoxPGetDatum(agtv_lhs->val.box), point));
    } else if (agtv_lhs->type == AGTV_PATH) {
      agtv_result.val.path = DatumGetPathP(DirectFunctionCall2(
          path_mul_pt, PathPGetDatum(agtv_lhs->val.path), point));
    } else if (agtv_lhs->type == AGTV_CIRCLE) {
      agtv_result.val.circle = DatumGetCircleP(DirectFunctionCall2(
          circle_mul_pt, CirclePGetDatum(agtv_lhs->val.circle), point));
    } else {
      ereport_op_str("*", lhs, rhs);
    }
  } else if (agtv_lhs->type == AGTV_INTERVAL &&
    ...
    agtv_result.type = AGTV_INTERVAL;
    Interval *interval = DatumGetIntervalP(DirectFunctionCall2(
        interval_mul, IntervalPGetDatum(&agtv_rhs->val.interval),
...
Datum gtype_div(PG_FUNCTION_ARGS) {
  ...
  if (agtv_lhs->type == AGTV_INTEGER && agtv_rhs->type == AGTV_INTEGER) {
    ...
    if (agtv_lhs->type == AGTV_POINT) {
      agtv_result.val.point = DatumGetPointP(DirectFunctionCall2(
          point_div, PointPGetDatum(agtv_lhs->val.point), point));
    } else if (agtv_lhs->type == AGTV_BOX) {
      agtv_result.val.box = DatumGetBoxP(
          DirectFunctionCall2(box_div, BoxPGetDatum(agtv_lhs->val.box), point));
    } else if (agtv_lhs->type == AGTV_PATH) {
      agtv_result.val.path = DatumGetPathP(DirectFunctionCall2(
          path_div_pt, PathPGetDatum(agtv_lhs->val.path), point));
    } else if (agtv_lhs->type == AGTV_CIRCLE) {
      agtv_result.val.circle = DatumGetCircleP(DirectFunctionCall2(
          circle_div_pt, CirclePGetDatum(agtv_lhs->val.circle), point));
    } else {
      ereport_op_str("/", lhs, rhs);
    }
  } else if (agtv_lhs->type == AGTV_INTERVAL && agtv_rhs->type == AGTV_FLOAT) {
    agtv_result.type = AGTV_INTERVAL;
    Interval *interval = DatumGetIntervalP(DirectFunctionCall2(
        interval_div, IntervalPGetDatum(&agtv_lhs->val.interval),
        Float8GetDatum(agtv_rhs->val.float_value)));
...
Datum gtype_bitwise_and(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_TSQUERY(lhs) || GT_IS_TSQUERY(rhs)) {
    Datum d = DirectFunctionCall2(tsquery_and, GT_ARG_TO_TSQUERY_DATUM(0),
    ...
    Datum d = DirectFunctionCall2(inetand, GT_ARG_TO_INET_DATUM(0),
                                  GT_ARG_TO_INET_DATUM(1));
...
Datum gtype_bitwise_or(PG_FUNCTION_ARGS) {
  Datum d = DirectFunctionCall2(inetor, GT_ARG_TO_INET_DATUM(0),
                                GT_ARG_TO_INET_DATUM(1));
...
Datum gtype_inet_subnet_strict_contains(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_BOX(lhs) && GT_IS_BOX(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        box_left, GT_TO_BOX_DATUM(lhs), GT_TO_BOX_DATUM(rhs))));
  else if (GT_IS_POLYGON(lhs) && GT_IS_POLYGON(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        poly_left, GT_TO_POLYGON_DATUM(lhs), GT_TO_POLYGON_DATUM(rhs))));
  else if (GT_IS_CIRCLE(lhs) && GT_IS_CIRCLE(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        circle_left, GT_TO_CIRCLE_DATUM(lhs), GT_TO_CIRCLE_DATUM(rhs))));
  /*   else if (GT_IS_GEOMETRY(lhs) || GT_IS_GEOMETRY(rhs))
         PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(gserialized_left_2d,
     GT_TO_GEOMETRY_DATUM(lhs), GT_TO_GEOMETRY_DATUM(rhs))));
 */
  PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
      network_sub, GT_ARG_TO_INET_DATUM(0), GT_ARG_TO_INET_DATUM(1))));
}

PG_FUNCTION_INFO_V1(gtype_inet_subnet_contains);
Datum gtype_inet_subnet_contains(PG_FUNCTION_ARGS) {
  PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
      network_subeq, GT_ARG_TO_INET_DATUM(0), GT_ARG_TO_INET_DATUM(1))));
}
...
Datum gtype_inet_subnet_strict_contained_by(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_BOX(lhs) && GT_IS_BOX(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        box_right, GT_TO_BOX_DATUM(lhs), GT_TO_BOX_DATUM(rhs))));
  else if (GT_IS_POLYGON(lhs) && GT_IS_POLYGON(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        poly_right, GT_TO_POLYGON_DATUM(lhs), GT_TO_POLYGON_DATUM(rhs))));
  else if (GT_IS_CIRCLE(lhs) && GT_IS_CIRCLE(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        circle_right, GT_TO_CIRCLE_DATUM(lhs), GT_TO_CIRCLE_DATUM(rhs))));
  /*  else if (GT_IS_GEOMETRY(lhs) || GT_IS_GEOMETRY(rhs))
        PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(gserialized_right_2d,
     GT_TO_GEOMETRY_DATUM(lhs), GT_TO_GEOMETRY_DATUM(rhs))));
*/
  PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
      network_sup, GT_ARG_TO_INET_DATUM(0), GT_ARG_TO_INET_DATUM(1))));
}

PG_FUNCTION_INFO_V1(gtype_inet_subnet_contained_by);
Datum gtype_inet_subnet_contained_by(PG_FUNCTION_ARGS) {
  PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
      network_supeq, GT_ARG_TO_INET_DATUM(0), GT_ARG_TO_INET_DATUM(1))));
}
...
Datum gtype_inet_subnet_contain_both(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_BOX(lhs) && GT_IS_BOX(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        box_overlap, GT_TO_BOX_DATUM(lhs), GT_TO_BOX_DATUM(rhs))));
  else if (GT_IS_POLYGON(lhs) && GT_IS_POLYGON(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        poly_overlap, GT_TO_POLYGON_DATUM(lhs), GT_TO_POLYGON_DATUM(rhs))));
  else if (GT_IS_CIRCLE(lhs) && GT_IS_CIRCLE(rhs))
    PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
        circle_overlap, GT_TO_CIRCLE_DATUM(lhs), GT_TO_CIRCLE_DATUM(rhs))));
  /*    else if (GT_IS_GEOMETRY(lhs) || GT_IS_GEOMETRY(rhs))
          PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(gserialized_overlaps_2d,
     GT_TO_GEOMETRY_DATUM(lhs), GT_TO_GEOMETRY_DATUM(rhs))));
  */

  PG_RETURN_BOOL(DatumGetBool(DirectFunctionCall2(
      network_overlap, GT_TO_INET_DATUM(lhs), GT_TO_INET_DATUM(rhs))));
}

#define GTYPECMPFUNC(type, action)                                             \
  PG_FUNCTION_INFO_V1(gtype_##type);                                           \
  Datum gtype_##type(PG_FUNCTION_ARGS) {                                       \
    gtype *lhs = AG_GET_ARG_GTYPE_P(0);                                        \
    gtype *rhs = AG_GET_ARG_GTYPE_P(1);                                        \
    int result = (compare_gtype_containers_orderability(&lhs->root,            \
                                                        &rhs->root) action 0); \
...
Datum gtype_contains(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_TSQUERY(lhs) || GT_IS_TSQUERY(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        tsq_mcontains, GT_TO_TSQUERY_DATUM(lhs), GT_TO_TSQUERY_DATUM(rhs)));

    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_BOX(lhs) && GT_IS_POINT(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        box_contain_pt, GT_TO_BOX_DATUM(lhs), GT_TO_POINT_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_BOX(lhs) && GT_IS_BOX(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        box_contain, GT_TO_BOX_DATUM(lhs), GT_TO_BOX_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_PATH(lhs) && GT_IS_POINT(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        on_ppath, GT_TO_POINT_DATUM(rhs), GT_TO_PATH_DATUM(lhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_POLYGON(lhs) && GT_IS_POINT(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        poly_contain_pt, GT_TO_POLYGON_DATUM(lhs), GT_TO_POINT_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_CIRCLE(lhs) && GT_IS_POINT(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        circle_contain_pt, GT_TO_CIRCLE_DATUM(lhs), GT_TO_POINT_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_CIRCLE(lhs) && GT_IS_CIRCLE(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        circle_contained, GT_TO_CIRCLE_DATUM(lhs), GT_TO_CIRCLE_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  }
...
Datum gtype_contained_by(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_TSQUERY(lhs) || GT_IS_TSQUERY(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        tsq_mcontains, GT_TO_TSQUERY_DATUM(rhs), GT_TO_TSQUERY_DATUM(lhs)));

    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_BOX(rhs) && GT_IS_POINT(lhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        on_pb, GT_TO_POINT_DATUM(lhs), GT_TO_BOX_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_BOX(lhs) && GT_IS_BOX(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        box_contained, GT_TO_BOX_DATUM(lhs), GT_TO_BOX_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_PATH(rhs) && GT_IS_POINT(lhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        on_ppath, GT_TO_POINT_DATUM(lhs), GT_TO_PATH_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_POLYGON(rhs) && GT_IS_POINT(lhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        pt_contained_poly, GT_TO_POINT_DATUM(lhs), GT_TO_POLYGON_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_CIRCLE(rhs) && GT_IS_POINT(lhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        pt_contained_circle, GT_TO_POINT_DATUM(lhs), GT_TO_CIRCLE_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  } else if (GT_IS_CIRCLE(lhs) && GT_IS_CIRCLE(rhs)) {
    bool boolean = DatumGetBool(DirectFunctionCall2(
        circle_contain, GT_TO_CIRCLE_DATUM(lhs), GT_TO_CIRCLE_DATUM(rhs)));
    PG_RETURN_BOOL(boolean);
  }
...
Datum gtype_exists(PG_FUNCTION_ARGS) {
  ...
  if (!(AGT_ROOT_IS_SCALAR(key)))
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("gtype ? gtype arg 2 must be a string")));
...
Datum gtype_exists_any(PG_FUNCTION_ARGS) {
  ...
  if (GT_IS_POINT(agt) || GT_IS_POINT(agt)) {
    Datum d = DirectFunctionCall2(point_vert, GT_TO_POINT_DATUM(agt),
                                  GT_TO_POINT_DATUM(keys));
  ...
  while (gtype_iterator_next(&it_array, &agtv_elem, false) != WGT_END_ARRAY) {
    ...
    if (find_gtype_value_from_container(&agt->root, GT_FOBJECT | GT_FARRAY,
                                        &agtv_elem) != NULL)
      PG_RETURN_BOOL(true);
  }
...
static gtype_value *iterator_concat(gtype_iterator **it1, gtype_iterator **it2,
  ...
  else if (((rk1 == WGT_BEGIN_ARRAY && !(*it1)->is_scalar) &&
            rk2 == WGT_BEGIN_OBJECT) ||
           (rk1 == WGT_BEGIN_OBJECT &&
            (rk2 == WGT_BEGIN_ARRAY && !(*it2)->is_scalar))) {
