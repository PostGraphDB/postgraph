...
static void gtype_in_scalar(void *pstate, char *token,
                            gtype_token_type tokentype, char *annotation);
static void gtype_categorize_type(Oid typoid, agt_type_category *tcategory,
                                  Oid *outfuncoid);
...
static void array_dim_to_gtype(gtype_in_state *result, int dim, int ndims,
                               ...
                               Oid outfuncoid);
static void datum_to_gtype(Datum val, bool is_null, gtype_in_state *result,
                           agt_type_category tcategory, Oid outfuncoid,
                           bool key_scalar);
static char *gtype_to_cstring_worker(StringInfo out, gtype_container *in,
                                     int estimated_len, bool indent);
static void add_indent(StringInfo out, bool indent, int level);
static gtype_value *execute_array_access_operator_internal(gtype *array,
                                                           int64 array_index);
static gtype_iterator *get_next_object_key(gtype_iterator *it,
                                           gtype_container *agtc,
                                           gtype_value *key);
static Datum process_access_operator_result(FunctionCallInfo fcinfo,
                                            gtype_value *agtv, bool as_text);
...
bool is_gtype_integer(gtype *agt) {
  return AGT_ROOT_IS_SCALAR(agt) && GTE_IS_GTYPE(agt->root.children[0]) &&
         GT_IS_INTEGER(agt->root.children[1]);
}

bool is_gtype_float(gtype *agt) {
  return AGT_ROOT_IS_SCALAR(agt) && GTE_IS_GTYPE(agt->root.children[0]) &&
         GT_IS_FLOAT(agt->root.children[1]);
}
...
size_t check_string_length(size_t len) {
  if (len > GTENTRY_OFFLENMASK)
    ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                    errmsg("string too long to represent as gtype string"),
                    errdetail("Due to an implementation restriction, gtype "
...
static void gtype_in_object_field_start(void *pstate, char *fname,
                                        bool isnull) {
  gtype_in_state *_state = (gtype_in_state *)pstate;
  gtype_value v;
...
Datum PostGraphDirectFunctionCall1Coll(PGFunction func, Oid collation,
                                       Datum arg1) {
...
void gtype_put_escaped_value(StringInfo out, gtype_value *scalar_val) {
  ...
  case AGTV_STRING:
    escape_gtype(
        out, pnstrdup(scalar_val->val.string.val, scalar_val->val.string.len));
    break;
  case AGTV_NUMERIC:
  ...
  case AGTV_TIMESTAMPTZ:
    ...
    break;
  case AGTV_DATE:
    numstr = DatumGetCString(
        DirectFunctionCall1(date_out, DateADTGetDatum(scalar_val->val.date)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_TIME:
    numstr = DatumGetCString(DirectFunctionCall1(
  ...
  case AGTV_INET:
    numstr = DatumGetCString(
        DirectFunctionCall1(inet_out, InetPGetDatum(&scalar_val->val.inet)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_CIDR:
    numstr = DatumGetCString(
        DirectFunctionCall1(cidr_out, InetPGetDatum(&scalar_val->val.inet)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_MAC:
    numstr = DatumGetCString(DirectFunctionCall1(
        macaddr_out, MacaddrPGetDatum(&scalar_val->val.mac)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_MAC8:
    numstr = DatumGetCString(DirectFunctionCall1(
        macaddr8_out, MacaddrPGetDatum(&scalar_val->val.mac)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_POINT:
    numstr = DatumGetCString(
        DirectFunctionCall1(point_out, PointerGetDatum(scalar_val->val.point)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_LSEG:
    numstr = DatumGetCString(
        DirectFunctionCall1(lseg_out, PointerGetDatum(scalar_val->val.lseg)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_LINE:
    numstr = DatumGetCString(
        DirectFunctionCall1(line_out, PointerGetDatum(scalar_val->val.line)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_PATH:
    numstr = DatumGetCString(
        DirectFunctionCall1(path_out, PointerGetDatum(scalar_val->val.path)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_POLYGON:
    numstr = DatumGetCString(DirectFunctionCall1(
        poly_out, PointerGetDatum(scalar_val->val.polygon)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_CIRCLE:
    numstr = DatumGetCString(DirectFunctionCall1(
        circle_out, PointerGetDatum(scalar_val->val.circle)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_BOX:
    numstr = DatumGetCString(
        DirectFunctionCall1(box_out, PointerGetDatum(scalar_val->val.box)));
    appendStringInfoString(out, numstr);
    break;
    /*case AGTV_BOX2D:
            numstr = DatumGetCString(DirectFunctionCall1(BOX2D_out,
       PointerGetDatum(&scalar_val->val.gbox))); appendStringInfoString(out,
       numstr); break; case AGTV_BOX3D: numstr =
       DatumGetCString(DirectFunctionCall1(BOX3D_out,
       PointerGetDatum(&scalar_val->val.box3d))); appendStringInfoString(out,
       numstr); break; case AGTV_SPHEROID: numstr =
       DatumGetCString(DirectFunctionCall1(ellipsoid_out,
       PointerGetDatum(&scalar_val->val.spheroid))); appendStringInfoString(out,
       numstr); break; case AGTV_GSERIALIZED: numstr =
       DatumGetCString(DirectFunctionCall1(LWGEOM_out,
       PointerGetDatum(scalar_val->val.gserialized)));
            appendStringInfoString(out, numstr);
            break;*/
  case AGTV_TSVECTOR:
    numstr = DatumGetCString(DirectFunctionCall1(
        tsvectorout, PointerGetDatum(scalar_val->val.tsvector)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_TSQUERY:
    numstr = DatumGetCString(DirectFunctionCall1(
        tsqueryout, PointerGetDatum(scalar_val->val.tsquery)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_BYTEA:
    numstr = DatumGetCString(
        DirectFunctionCall1(byteaout, PointerGetDatum(scalar_val->val.bytea)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_RANGE_INT:
  case AGTV_RANGE_NUM:
  ...
  case AGTV_RANGE_DATE:
    numstr = DatumGetCString(PostGraphDirectFunctionCall1Coll(
        range_out, DEFAULT_COLLATION_OID,
        PointerGetDatum(scalar_val->val.range)));
    appendStringInfoString(out, numstr);
    break;
  ...
  case AGTV_RANGE_DATE_MULTI:
    numstr = DatumGetCString(PostGraphDirectFunctionCall1Coll(
        multirange_out, DEFAULT_COLLATION_OID,
        PointerGetDatum(scalar_val->val.multirange)));
    appendStringInfoString(out, numstr);
    break;
  case AGTV_BOOL:
...
static void gtype_in_scalar(void *pstate, char *token,
  ...
  case GTYPE_TOKEN_FLOAT:
    ...
    v.type = AGTV_FLOAT;
    v.val.float_value =
        float8in_internal(token, NULL, "double precision", token);
    break;
  ...
  case GTYPE_TOKEN_TIMESTAMP:
    ...
    v.val.int_value = DatumGetInt64(
        DirectFunctionCall3(timestamp_in, CStringGetDatum(token),
                            ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1)));
    break;
  case GTYPE_TOKEN_TIMESTAMPTZ:
    v.type = AGTV_TIMESTAMPTZ;
    v.val.int_value = DatumGetInt64(
        DirectFunctionCall3(timestamptz_in, CStringGetDatum(token),
                            ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1)));
    break;
  case GTYPE_TOKEN_DATE:
    v.type = AGTV_DATE;
    v.val.date =
        DatumGetInt32(DirectFunctionCall1(date_in, CStringGetDatum(token)));
    break;
  case GTYPE_TOKEN_TIME:
    v.type = AGTV_TIME;
    v.val.int_value = DatumGetInt64(
        DirectFunctionCall3(time_in, CStringGetDatum(token),
                            ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1)));
    break;
  case GTYPE_TOKEN_TIMETZ:
    v.type = AGTV_TIMETZ;
    TimeTzADT *timetz = DatumGetTimeTzADTP(
        DirectFunctionCall3(timetz_in, CStringGetDatum(token),
                            ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1)));
  ...
  case GTYPE_TOKEN_INTERVAL: {
    ...
    interval = DatumGetIntervalP(
        DirectFunctionCall3(interval_in, CStringGetDatum(token),
                            ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1)));
  ...
  case GTYPE_TOKEN_MACADDR: {
    ...
    v.type = AGTV_MAC;
    mac = DatumGetMacaddrP(
        DirectFunctionCall1(macaddr_in, CStringGetDatum(token)));
  ...
  case GTYPE_TOKEN_MACADDR8: {
    ...
    v.type = AGTV_MAC8;
    mac = DatumGetMacaddrP(
        DirectFunctionCall1(macaddr8_in, CStringGetDatum(token)));
...
static void datum_to_gtype(Datum val, bool is_null, gtype_in_state *result,
  ...
  if (is_null) {
    ...
    case AGT_TYPE_NUMERIC:
      ...
      if (key_scalar) {
         ...
         */
        numeric_error =
            (strchr(outputstr, 'N') != NULL || strchr(outputstr, 'n') != NULL);
        if (!numeric_error) {
          Datum numd;

          agtv.type = AGTV_NUMERIC;
          numd = DirectFunctionCall3(numeric_in, CStringGetDatum(outputstr),
    ...
    case AGT_TYPE_INET: {
      ...
      memcpy(&agtv.val.inet.inet_data.ipaddr, &i->inet_data.ipaddr,
             16 * sizeof(char));
  ...
  if (!is_null && !scalar_gtype && tcategory >= AGT_TYPE_GTYPE &&
      tcategory <= AGT_TYPE_JSONCAST) {
    ...
    case AGTV_ARRAY:
      ...
      break;
    case AGTV_OBJECT:
      result->res = push_gtype_value(&result->parse_state,
                                     key_scalar ? WGT_KEY : WGT_VALUE, &agtv);
      break;
    default:
      elog(ERROR, "unexpected parent of nested structure");
    }
...
static void array_dim_to_gtype(gtype_in_state *result, int dim, int ndims,
  ...
  for (int i = 1; i <= dims[dim]; i++) {
    if (dim + 1 == ndims) {
      datum_to_gtype(vals[*valcount], nulls[*valcount], result, tcategory,
                     outfuncoid, false);
      (*valcount)++;
    } else {
      array_dim_to_gtype(result, dim + 1, ndims, dims, vals, nulls, valcount,
...
void array_to_gtype_internal(Datum array, gtype_in_state *result) {
  ...
  array_dim_to_gtype(result, 0, ndim, dim, elements, nulls, &count, tcategory,
                     outfuncoid);
  ...
  if (nargs % 2 != 0)
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             ...
             errhint("The arguments of gtype_build_map() must consist of "
...
Datum gtype_field_access(PG_FUNCTION_ARGS) {
  ...
  if (key_value->type == AGTV_INTEGER)
    AG_RETURN_GTYPE_P(
        gtype_array_element_impl(fcinfo, agt, key_value->val.int_value, false));
  else if (key_value->type == AGTV_STRING)
    AG_RETURN_GTYPE_P(
        gtype_object_field_impl(fcinfo, agt, key_value->val.string.val,
...
Datum gtype_object_field_impl(FunctionCallInfo fcinfo, gtype *gtype_in,
  ...
  const gtype_value new_key_value = {.type = AGTV_STRING,
...
Datum gtype_object_field(PG_FUNCTION_ARGS) {
  ...
  AG_RETURN_GTYPE_P(gtype_object_field_impl(fcinfo, agt, VARDATA_ANY(key),
...
Datum gtype_object_field_text(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_TEXT_P(gtype_object_field_impl(fcinfo, agt, VARDATA_ANY(key),
...
Datum gtype_access_slice(PG_FUNCTION_ARGS) {
  ...
  if (PG_ARGISNULL(1)) {
    ...
    lidx_value =
        get_ith_gtype_value_from_container(&(AG_GET_ARG_GTYPE_P(1))->root, 0);
  ...
  if (PG_ARGISNULL(2)) {
    ...
    uidx_value =
        get_ith_gtype_value_from_container(&(AG_GET_ARG_GTYPE_P(2))->root, 0);
  ...
  for (i = lower_index; i < upper_index; i++) {
    result.res =
        push_gtype_value(&result.parse_state, WGT_ELEM,
...
Datum gtype_in_operator(PG_FUNCTION_ARGS) {
  ...
  for (i = 0; i < array_size && !result; i++) {
    ...
    else if (IS_A_GTYPE_SCALAR(&agtv_item) && IS_A_GTYPE_SCALAR(&agtv_elem) &&
...
Datum gtype_string_match_starts_with(PG_FUNCTION_ARGS) {
  Datum x =
      convert_to_scalar(gtype_to_text_internal, AG_GET_ARG_GTYPE_P(0), "text");
  Datum y =
      convert_to_scalar(gtype_to_text_internal, AG_GET_ARG_GTYPE_P(1), "text");

  Datum d =
      DirectFunctionCall2Coll(text_starts_with, DEFAULT_COLLATION_OID, x, y);
...
Datum gtype_btree_cmp(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_INT16(compare_gtype_containers_orderability(&gtype_lhs->root,
                                                        &gtype_rhs->root));
}
...
Datum column_get_datum(TupleDesc tupdesc, HeapTuple tuple, int column,
  ...
  if (att->atttypid != typid)
    ereport(ERROR, (errcode(ERRCODE_UNDEFINED_TABLE),
                    errmsg("Invalid attribute typid. Expected %d, found %d",
                           typid, att->atttypid)));
...
Datum gtype_last(PG_FUNCTION_ARGS) {
  ...
  gtype_value *gtv =
      get_ith_gtype_value_from_container(&gt->root, AGT_ROOT_COUNT(gt) - 1);
...
Datum gtype_size(PG_FUNCTION_ARGS) {
  ...
  if ((AGT_ROOT_IS_SCALAR(agt) && !is_gtype_string(agt)) ||
      (!AGT_ROOT_IS_SCALAR(agt) && !AGT_ROOT_IS_ARRAY(agt)))
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
Datum gtype_reverse(PG_FUNCTION_ARGS) {
  ...
  if (!AGT_ROOT_IS_SCALAR(gt) && AGT_ROOT_IS_ARRAY(gt)) {
    ...
    gtype_value *agtv_value =
        push_gtype_value(&parse_state, WGT_BEGIN_ARRAY, NULL);
    ...
    gtype_value gtv = {.type = AGTV_STRING,
                       .val.string = {VARSIZE(d), text_to_cstring(d)}};
  ...
  while (true) {
    ...
    if (strcmp(str, var_name)) {
      ...
      if (GTYPE_CONTAINER_IS_SCALAR(&new_v->root)) {
        ...
        parsed_gtype_value =
            push_gtype_value(&parse_state, WGT_VALUE, new_gtype_value_v);
      } else {
...
Datum gtype_accum(PG_FUNCTION_ARGS) {
  PG_RETURN_DATUM(
      DirectFunctionCall2(float8_accum, PG_GETARG_DATUM(0),
                          convert_to_scalar(gtype_to_float8_internal,
...
Datum gtype_regr_accum(PG_FUNCTION_ARGS) {
  PG_RETURN_DATUM(
      DirectFunctionCall3(float8_regr_accum, PG_GETARG_DATUM(0),
                          convert_to_scalar(gtype_to_float8_internal,
                                            AG_GET_ARG_GTYPE_P(1), "float"),
                          convert_to_scalar(gtype_to_float8_internal,
...
#define REGRESSIONAGGFINAL(name)                                               \
  ...
  Datum gtype_##name(PG_FUNCTION_ARGS) {                                       \
    ...
    gtype_value gtv = {.type = AGTV_FLOAT,                                     \
                       .val.float_value = DatumGetFloat8(result)};             \
                                                                               \
    AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv));                             \
  }
...
Datum gtype_max_trans(PG_FUNCTION_ARGS) {
  ...
  AG_RETURN_GTYPE_P(
      (compare_gtype_containers_orderability(&lhs->root, &rhs->root) >= 0)
          ? lhs
          : rhs);
}
...
Datum gtype_percentile_aggtransfn(PG_FUNCTION_ARGS) {
  ...
  if (PG_ARGISNULL(0)) {
    ...
    percentile = DatumGetFloat8(
        DirectFunctionCall1(gtype_to_float8, PG_GETARG_DATUM(2)));
     ...
     */
    pgastate->sortstate = tuplesort_begin_datum(
        FLOAT8OID, Float8LessOperator, InvalidOid, false, work_mem, NULL, true);
...
Datum gtype_collect_aggtransfn(PG_FUNCTION_ARGS) {
  ...
  if (PG_ARGISNULL(0)) {
    ...
    castate->res =
        push_gtype_value(&castate->parse_state, WGT_BEGIN_ARRAY, NULL);
  } else {
...
Datum gtype_range_w_skip(PG_FUNCTION_ARGS) {
  int64 start = DatumGetInt64(
      convert_to_scalar(gtype_to_int8_internal, AG_GET_ARG_GTYPE_P(0), "int"));
  int64 end = DatumGetInt64(
      convert_to_scalar(gtype_to_int8_internal, AG_GET_ARG_GTYPE_P(1), "int"));
  int64 step = DatumGetInt64(
      convert_to_scalar(gtype_to_int8_internal, AG_GET_ARG_GTYPE_P(2), "int"));
  ...
  for (int64 i = start; (step > 0 && i <= end) || (step < 0 && i >= end);
       i += step) {
    gtype_value gtv;
...
Datum gtype_unnest(PG_FUNCTION_ARGS) {
  ...
  tmp_cxt = AllocSetContextCreate(
      CurrentMemoryContext, "age_unnest temporary cxt", ALLOCSET_DEFAULT_SIZES);
