...
Datum vertex_out(PG_FUNCTION_ARGS) {
  ...
  appendStringInfoString(
      str, DatumGetCString(DirectFunctionCall1(int8out, Int64GetDatum(id))));
...
void append_vertex_to_string(StringInfoData *buffer, vertex *v) {
  ...
  appendStringInfoString(
      buffer, DatumGetCString(DirectFunctionCall1(int8out, Int64GetDatum(id))));
...
Datum vertex_eq(PG_FUNCTION_ARGS) {
...
Datum vertex_ne(PG_FUNCTION_ARGS) {
  vertex *lhs = AG_GET_ARG_VERTEX(0);
  vertex *rhs = AG_GET_ARG_VERTEX(1);
...
Datum vertex_lt(PG_FUNCTION_ARGS) {
  vertex *lhs = AG_GET_ARG_VERTEX(0);
  vertex *rhs = AG_GET_ARG_VERTEX(1);
...
Datum vertex_le(PG_FUNCTION_ARGS) {
  vertex *lhs = AG_GET_ARG_VERTEX(0);
  vertex *rhs = AG_GET_ARG_VERTEX(1);
...
Datum vertex_gt(PG_FUNCTION_ARGS) {
  vertex *lhs = AG_GET_ARG_VERTEX(0);
  vertex *rhs = AG_GET_ARG_VERTEX(1);
...
Datum vertex_ge(PG_FUNCTION_ARGS) {
  vertex *lhs = AG_GET_ARG_VERTEX(0);
  vertex *rhs = AG_GET_ARG_VERTEX(1);
...
Datum vertex_btree_cmp(PG_FUNCTION_ARGS) {
  vertex *lhs = AG_GET_ARG_VERTEX(0);
  vertex *rhs = AG_GET_ARG_VERTEX(1);
 ...
 * Operators
 *
 * All operators on a vertex actually happen on the vertex's properties, thats
 * why the first step in all these functions is to extract the properties.
 */
// -> operator
PG_FUNCTION_INFO_V1(vertex_property_access);
Datum vertex_property_access(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
  gtype *agt = extract_vertex_properties(v);
...
Datum vertex_property_access_gtype(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
  gtype *agt = extract_vertex_properties(v);
...
Datum vertex_property_access_text(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
  gtype *agt = extract_vertex_properties(v);
  text *key = PG_GETARG_TEXT_PP(1);
...
Datum vertex_contains(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
...
Datum vertex_contained_by(PG_FUNCTION_ARGS) {
...
Datum vertex_exists(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
  gtype *agt = extract_vertex_properties(v);
  text *key = PG_GETARG_TEXT_PP(1);
   ...
   */
  gtype_value agtv = {.type = AGTV_STRING,
                      .val.string = {VARSIZE_ANY_EXHDR(key), VARDATA_ANY(key)}};

  gtype_value *val = find_gtype_value_from_container(
      &agt->root, GT_FOBJECT | GT_FARRAY, &agtv);
...
Datum vertex_exists_any(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
  gtype *agt = extract_vertex_properties(v);
  ...
  for (int i = 0; i < elem_count; i++) {
    ...
    gtype_value agtv = {.type = AGTV_STRING,
                        .val.string = {VARSIZE(key_datums[i]) - VARHDRSZ,
...
Datum vertex_exists_all(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
  gtype *agt = extract_vertex_properties(v);
  ArrayType *keys = PG_GETARG_ARRAYTYPE_P(1);
  Datum *key_datums;
...
Datum vertex_id(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);

  gtype_value gtv = {.type = AGTV_INTEGER,
                     .val = {.int_value = *((int64 *)(&v->children[0]))}};

  AG_RETURN_GTYPE_P(gtype_value_to_gtype(&gtv));
}

PG_FUNCTION_INFO_V1(vertex_properties);
Datum vertex_properties(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
...
Datum vertex_label(PG_FUNCTION_ARGS) {
  vertex *v = AG_GET_ARG_VERTEX(0);
...
Datum vertex_collect_transfn(PG_FUNCTION_ARGS) {
...
Datum vertex_collect_transfn_w_limit(PG_FUNCTION_ARGS) {
  Oid arg1_typeid = VERTEXOID;
  MemoryContext aggcontext;
  ArrayBuildState *state;
  Datum elem;
...
Datum vertex_collect_finalfn(PG_FUNCTION_ARGS) {
  Datum result;
  ArrayBuildState *state;
  int dims[1];
  int lbs[1];
...
char *extract_vertex_label(vertex *v) {
  ...
  label_cache_data *cache_data =
      search_label_graph_oid_cache(graph_oid, label_id);
