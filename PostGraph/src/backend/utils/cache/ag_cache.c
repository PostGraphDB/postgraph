...
static void initialize_graph_caches(void) {
  ...
  // ag_graph.namespace
  ag_cache_scan_key_init(&graph_namespace_scan_keys[0], Anum_ag_graph_namespace,
                         F_OIDEQ);
...
static void flush_graph_name_cache(void) {
  ...
  for (;;) {
    ...
    removed =
        hash_search(graph_name_cache_hash, &entry->name, HASH_REMOVE, NULL);
    if (!removed)
...
static void flush_graph_namespace_cache(void) {
  ...
  for (;;) {
    ...
    removed = hash_search(graph_namespace_cache_hash, &entry->namespace,
                          HASH_REMOVE, NULL);
    if (!removed) {
      ereport(ERROR, (errmsg_internal("graph (namespace) cache corrupted")));
    }
  }
}
...
graph_cache_data *search_graph_namespace_cache(Oid namespace) {
  ...
  entry = hash_search(graph_namespace_cache_hash, &namespace, HASH_FIND, NULL);
  if (entry)
    return &entry->data;
...
static graph_cache_data *search_graph_namespace_cache_miss(Oid namespace) {
   ...
   */
  ag_graph = table_open(ag_graph_relation_id(), AccessShareLock);
  scan_desc = systable_beginscan(ag_graph, ag_graph_namespace_index_id(), true,
                                 NULL, 1, scan_keys);
  ...
  // get a new entry
  entry =
      hash_search(graph_namespace_cache_hash, &namespace, HASH_ENTER, &found);
...
static void create_label_name_graph_cache(void) {
   ...
   * HASH_BLOBS flag is set because the key for this hash is fixed-size.
   */
  label_name_graph_cache_hash = hash_create("ag_label (name, graph) cache", 16,
                                            &hash_ctl, HASH_ELEM | HASH_BLOBS);
}
...
static void invalidate_label_name_graph_cache(Oid relid) {
  ...
  for (;;) {
    ...
    removed = hash_search(label_name_graph_cache_hash, &entry->key, HASH_REMOVE,
                          NULL);
    hash_seq_term(&hash_seq);

    if (!removed) {
      ereport(ERROR, (errmsg_internal("label (name, graph) cache corrupted")));
    }
...
static void invalidate_label_graph_oid_cache(Oid relid) {
  ...
  for (;;) {
    ...
    removed =
        hash_search(label_graph_oid_cache_hash, &entry->key, HASH_REMOVE, NULL);
    hash_seq_term(&hash_seq);

    if (!removed) {
      ereport(ERROR, (errmsg_internal("label (graph, id) cache corrupted")));
    }
...
static void flush_label_relation_cache(void) {
  ...
  for (;;) {
    ...
    removed = hash_search(label_relation_cache_hash, &entry->relation,
                          HASH_REMOVE, NULL);
    if (!removed) {
      ereport(ERROR, (errmsg_internal("label (relation) cache corrupted")));
    }
  }
}
...
label_cache_data *search_label_name_graph_cache(const char *name, Oid graph) {
  ...
  namestrcpy(&name_key, name);
  entry = label_name_graph_cache_hash_search(&name_key, graph, HASH_FIND, NULL);
  if (entry)
    return &entry->data;
...
static label_cache_data *search_label_name_graph_cache_miss(Name name,
  ...
  scan_desc = systable_beginscan(ag_label, ag_label_name_graph_index_id(), true,
                                 NULL, 2, scan_keys);
  ...
  // get a new entry
  entry = label_name_graph_cache_hash_search(name, graph, HASH_ENTER, &found);
...
static label_cache_data *search_label_relation_cache_miss(Oid relation) {
  ...
  memcpy(scan_keys, label_relation_scan_keys, sizeof(label_relation_scan_keys));
  ...
  // get a new entry
  entry = hash_search(label_relation_cache_hash, &relation, HASH_ENTER, &found);
