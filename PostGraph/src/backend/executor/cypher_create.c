...
static void begin_cypher_create(CustomScanState *node, EState *estate,
  ...
  foreach (lc, css->pattern) {
    ...
    foreach (lc2, path->target_nodes) {
      ...
      cypher_node->elemTupleSlot =
          table_slot_create(rel, &estate->es_tupleTable);
...
static void process_pattern(cypher_create_custom_scan_state *css) {
  ...
  foreach (lc2, css->pattern) {
     ...
     */
    create_vertex_1(css, lfirst(lc), lnext(path->target_nodes, lc),
                    path->target_nodes);
...
static void create_edge_1(cypher_create_custom_scan_state *css,
  ...
  if (CYPHER_TARGET_NODE_OUTPUT(node->flags)) {
    ...
    edge *e = create_edge(
        id, start_id, end_id, css->graph_oid,
...
static Datum create_vertex_1(cypher_create_custom_scan_state *css,
  ...
  /*
   * Vertices in a patEDGE_GET_DATUMh might already exists. If they do get the
   * id to pass to the edges before and after it. Otherwise, insert the new
   * vertex into it's table and then pass the id along.
   */
