...
static void InitCenters(Relation index, VectorArray samples,
                        VectorArray centers, float8 *lowerBound) {
  FmgrInfo *procinfo;
  Oid collation;
  int64 j;
  double distance;
  ...
  VectorArraySet(centers, 0,
                 VectorArrayGet(samples, RandomInt() % samples->length));
  centers->length++;
...
static void QuickCenters(Relation index, VectorArray samples,
  ...
  while (centers->length < centers->maxlen) {
    ...
    for (int j = 0; j < dimensions; j++) {
      *((float8 *)(&vec->root.children[1 + (j * sizeof(float8))])) =
          RandomDouble();
    }
...
static void ElkanKmeans(Relation index, VectorArray samples,
  ...
  Size totalSize = samplesSize + centersSize + newCentersSize +
                   centerCountsSize + closestCentersSize + lowerBoundSize +
                   upperBoundSize + sSize + halfcdistSize + newcdistSize;
  ...
  for (iteration = 0; iteration < 500; iteration++) {
    ...
    for (j = 0; j < numSamples; j++) {
      ...
      for (k = 0; k < dimensions; k++)
        *((float8 *)&newCenter->root.children[1 + (k * sizeof(float8))]) +=
