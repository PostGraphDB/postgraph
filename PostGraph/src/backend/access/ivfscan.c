 ...
 */
static int CompareLists(const pairingheap_node *a, const pairingheap_node *b,
                        void *arg) {
  if (((const IvfflatScanList *)a)->distance >
      ((const IvfflatScanList *)b)->distance)
    return 1;
  ...
  while (BlockNumberIsValid(nextblkno)) {
    ...
    for (offno = FirstOffsetNumber; offno <= maxoffno;
      ...
      if (listCount < so->probes) {
        ...
        if (listCount == so->probes)
          maxDistance =
              ((IvfflatScanList *)pairingheap_first(so->listQueue))->distance;
...
static void GetScanItems(IndexScanDesc scan, Datum value) {
  ...
  while (!pairingheap_is_empty(so->listQueue)) {
    searchPage =
        ((IvfflatScanList *)pairingheap_remove_first(so->listQueue))->startPage;
 ...
 */
static int GetDimensions(Relation index) {
  Buffer buf;
  Page page;
...
bool ivfflatgettuple(IndexScanDesc scan, ScanDirection dir) {
  ...
  if (tuplesort_gettupleslot(so->sortstate, true, false, so->slot, NULL)) {
    ItemPointer tid =
        (ItemPointer)DatumGetPointer(slot_getattr(so->slot, 2, &so->isnull));
    BlockNumber indexblkno =
        DatumGetInt32(slot_getattr(so->slot, 3, &so->isnull));
