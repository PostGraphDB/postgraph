...
Datum create_ivfflat_ip_ops_index(PG_FUNCTION_ARGS) {
  ...
  DefElem *dims =
      makeDefElem("dimensions", makeInteger(PG_GETARG_INT32(3)), -1);
...
VectorArray VectorArrayInit(int maxlen, int dimensions) {
  ...
  res->items =
      palloc_extended(gtype_size * 2, MCXT_ALLOC_ZERO | MCXT_ALLOC_HUGE);
...
void IvfflatAppendPage(Relation index, Buffer *buf, Page *page,
  ...
  Page newpage =
      GenericXLogRegisterBuffer(*state, newbuf, GENERIC_XLOG_FULL_IMAGE);
 ...
 */
void IvfflatUpdateList(Relation index, ListInfo listInfo,
  ...
  if (BlockNumberIsValid(insertPage) && insertPage != list->insertPage) {
    ...
    if (!BlockNumberIsValid(originalInsertPage) ||
        insertPage >= originalInsertPage) {
