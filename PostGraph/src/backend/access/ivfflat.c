...
void IvfflatInit(void) {
  ...
  add_int_reloption(
      ivfflat_relopt_kind, "dimensions", "Number of dimensions in the index",
      IVFFLAT_DEFAULT_ELEMENTS, -1, VECTOR_MAX_DIM, AccessExclusiveLock);
 ...
 */
#if PG_VERSION_NUM >= 120000
static char *ivfflatbuildphasename(int64 phasenum) {
  switch (phasenum) {
...
static void ivfflatcostestimate(PlannerInfo *root, IndexPath *path,
  ...
  if (costs.numIndexPages > path->indexinfo->rel->pages && ratio < 0.5) {
    ...
    costs.indexTotalCost -=
        costs.numIndexPages * (costs.spc_random_page_cost - spc_seq_page_cost);

    // Remove cost of extra pages
    costs.indexTotalCost -=
        (costs.numIndexPages - path->indexinfo->rel->pages) * spc_seq_page_cost;
  } else {
    // Change some page cost from random to sequential
    costs.indexTotalCost -= 0.5 * costs.numIndexPages *
