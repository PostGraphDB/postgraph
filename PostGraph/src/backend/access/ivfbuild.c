...
static void SampleRows(IvfflatBuildState *buildstate) {
  ...
  while (BlockSampler_HasMore(&buildstate->bs)) {
    ...
    table_index_build_range_scan(
        ...
        false, targblock, 1, SampleCallback, (void *)buildstate, NULL);
#else
    IndexBuildHeapRangeScan(buildstate->heap, buildstate->index,
                            buildstate->indexInfo, false, true, targblock, 1,
 ...
 * Add tuple to sort
 */
static void AddTupleToSort(Relation index, ItemPointer tid, Datum *values,
...
static inline void GetNextTuple(Tuplesortstate *sortstate, TupleDesc tupdesc,
  ...
  if (tuplesort_gettupleslot(sortstate, true, false, slot, NULL)) {
    ...
    *itup = index_form_tuple(tupdesc, &value, &isnull);
    (*itup)->t_tid =
        *((ItemPointer)DatumGetPointer(slot_getattr(slot, 2, &isnull)));
  } else {
    *list = -1;
  }
}
...
static void InsertTuples(Relation index, IvfflatBuildState *buildstate,
  ...
  for (int i = 0; i < buildstate->centers->length; i++) {
    ...
    IvfflatUpdateList(index, buildstate->listInfo[i], insertPage,
                      InvalidBlockNumber, startPage, forkNum);
  }
}
...
static void InitBuildState(IvfflatBuildState *buildstate, Relation heap,
  ...
  TupleDescInitEntry(buildstate->tupdesc, (AttrNumber)3, "vector",
  ...
  buildstate->normvec =
      gtype_value_to_gtype(InitVectorGType(buildstate->dimensions));
 ...
 */
static void IvfflatParallelScanAndSort(IvfflatSpool *ivfspool,
                                       IvfflatShared *ivfshared,
                                       Sharedsort *sharedsort,
                                       gtype *ivfcenters, int sortmem,
                                       bool progress) {
  ...
  memcpy(buildstate.centers->items, ivfcenters,
         VECTOR_SIZE(buildstate.centers->dim) * buildstate.centers->maxlen);
  buildstate.centers->length = buildstate.centers->maxlen;
  ivfspool->sortstate = tuplesort_begin_heap(
  ...
  scan = table_beginscan_parallel(
      ivfspool->heap, ParallelTableScanFromIvfflatShared(ivfshared));
  ...
  reltuples =
      IndexBuildHeapScan(ivfspool->heap, ivfspool->index, indexInfo, true,
                         BuildCallback, (void *)&buildstate, scan);
#endif
  ...
  // Log statistics
  if (progress)
    ereport(DEBUG1, (errmsg("leader processed " INT64_FORMAT " tuples",
                            (int64)reltuples)));
  else
    ereport(DEBUG1, (errmsg("worker processed " INT64_FORMAT " tuples",
                            (int64)reltuples)));
...
void IvfflatParallelBuildMain(dsm_segment *seg, shm_toc *toc) {
  ...
  IvfflatParallelScanAndSort(ivfspool, ivfshared, sharedsort, ivfcenters,
                             sortmem, false);
 ...
 * End parallel build
 */
static void IvfflatEndParallel(IvfflatLeader *ivfleader) {
 ...
 */
static Size ParallelEstimateShared(Relation heap, Snapshot snapshot) {
#if PG_VERSION_NUM >= 120000
  return add_size(BUFFERALIGN(sizeof(IvfflatShared)),
 ...
 */
static void IvfflatLeaderParticipateAsWorker(IvfflatBuildState *buildstate) {
 ...
 */
static void IvfflatBeginParallel(IvfflatBuildState *buildstate,
  ...
  table_parallelscan_initialize(buildstate->heap,
                                ...
                                snapshot);
#else
  heap_parallelscan_initialize(&ivfshared->heapdesc, buildstate->heap,
                               snapshot);
#endif
  ...
  ivfcenters = (gtype *)shm_toc_allocate(pcxt->toc, estcenters);
  ...
  // Log participants
  ereport(DEBUG1,
          (errmsg("using %d parallel workers", pcxt->nworkers_launched)));
...
static void AssignTuples(IvfflatBuildState *buildstate) {
  ...
  if (buildstate->heap != NULL)
    parallel_workers =
        plan_create_index_workers(RelationGetRelid(buildstate->heap),
                                  RelationGetRelid(buildstate->index));

  // Attempt to launch parallel worker scan when required
  if (parallel_workers > 0)
    IvfflatBeginParallel(buildstate, buildstate->indexInfo->ii_Concurrent,
                         parallel_workers);
  ...
  buildstate->sortstate = tuplesort_begin_heap(
      buildstate->tupdesc, 1, attNums, sortOperators, sortCollations,
      nullsFirstFlags, maintenance_work_mem, coordinate, false);
