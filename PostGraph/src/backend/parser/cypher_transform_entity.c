...
find_transform_entity_type(cypher_parsestate *cpstate, char *name) {
  ListCell *lc;

  foreach (lc, cpstate->entities) {
    transform_entity *entity = lfirst(lc);

    if (entity->type == ENT_VERTEX && entity->entity.node->name != NULL &&
        !strcmp(entity->entity.node->name, name))
      return ENT_VERTEX;
    else if (entity->type == ENT_EDGE && entity->entity.node->name != NULL &&
             !strcmp(entity->entity.rel->name, name))
      return ENT_EDGE;
    else if (entity->type == ENT_VLE_EDGE &&
             entity->entity.node->name != NULL &&
             !strcmp(entity->entity.rel->name, name))
      return ENT_VLE_EDGE;
  }
 ...
 * type.
 */
transform_entity *find_transform_entity(cypher_parsestate *cpstate, char *name,
                                        enum transform_entity_type type) {
  ListCell *lc;
