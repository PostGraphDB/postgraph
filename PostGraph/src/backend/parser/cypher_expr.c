...
static Node *transform_cypher_inet_const(cypher_parsestate *cpstate,
                                         cypher_inet_const *inet);
...
static Node *transform_coalesce_expr(cypher_parsestate *cpstate,
                                     CoalesceExpr *cexpr);
...
static Node *transform_column_ref_for_indirection(cypher_parsestate *cpstate,
                                                  ColumnRef *cr);
static Node *transformSQLValueFunction(cypher_parsestate *cpstate,
                                       SQLValueFunction *svf);
static List *make_qualified_function_name(cypher_parsestate *cpstate, List *lst,
                                          List *targs);
static void unify_hypothetical_args(ParseState *pstate, List *fargs,
...
static int check_agg_arguments(ParseState *pstate, List *directargs, List *args,
                               Expr *filter);
...
static void check_agglevels_and_constraints(ParseState *pstate, Node *expr) {
  ...
  *p_levelsup = min_varlevel;
  ...
  case EXPR_KIND_FROM_SUBSELECT:
     ...
     */
    if (isAgg)
      err = _("aggregate functions are not allowed in FROM clause of their own "
              "query level");
    else
      err = _("grouping operations are not allowed in FROM clause of their own "
              "query level");
  ...
  case EXPR_KIND_PARTITION_EXPRESSION:
    if (isAgg)
      err =
          _("aggregate functions are not allowed in partition key expressions");
    else
      err =
          _("grouping operations are not allowed in partition key expressions");

    break;
  case EXPR_KIND_GENERATED_COLUMN:

    if (isAgg)
      err = _("aggregate functions are not allowed in column generation "
              "expressions");
    else
      err = _("grouping operations are not allowed in column generation "
              "expressions");
  ...
  if (err)
    ereport(ERROR, (errcode(ERRCODE_GROUPING_ERROR), errmsg_internal("%s", err),
...
static int check_agg_arguments(ParseState *pstate, List *directargs, List *args,
  ...
  if (directargs) {
    ...
    if (context.min_varlevel >= 0 && context.min_varlevel < agglevel)
      ereport(ERROR, (errcode(ERRCODE_GROUPING_ERROR),
                      errmsg("outer-level aggregate cannot contain a "
                             "lower-level variable in its direct arguments"),
                      parser_errposition(
                          pstate, locate_var_of_level((Node *)directargs,
                                                      context.min_varlevel))));
    if (context.min_agglevel >= 0 && context.min_agglevel <= agglevel)
      ereport(ERROR, (errcode(ERRCODE_GROUPING_ERROR),
                      errmsg("aggregate function calls cannot be nested"),
                      parser_errposition(
                          pstate, locate_agg_of_level((Node *)directargs,
...
static bool check_agg_arguments_walker(Node *node,
  ...
  if (context->sublevels_up == 0) {
    if ((IsA(node, FuncExpr) && ((FuncExpr *)node)->funcretset) ||
      ...
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg("aggregate function calls cannot contain "
                             "set-returning function calls"),
                      errhint("You might be able to move the set-returning "
                              "function into a LATERAL FROM item."),
  ...
  if (IsA(node, Query)) {
    ...
    result = query_tree_walker((Query *)node, check_agg_arguments_walker,
                               (void *)context, 0);
    context->sublevels_up--;
    return result;
  }

  return expression_tree_walker(node, check_agg_arguments_walker,
                                (void *)context);
}
 ...
 * pstate level.
 */
void transform_aggregate_call(ParseState *pstate, Aggref *agg, List *args,
                              List *aggorder, bool agg_distinct) {
  List *argtypes = NIL;
  List *tlist = NIL;
  List *torder = NIL;
  List *tdistinct = NIL;
  ...
  if (AGGKIND_IS_ORDERED_SET(agg->aggkind)) {
    ...
    torder =
        transform_cypher_order_by(pstate, aggorder, &tlist, EXPR_KIND_ORDER_BY
                                  /* force SQL99 rules */);
        ...
        if (!OidIsValid(sortcl->sortop)) {
          ...
          ereport(ERROR,
                  (errcode(ERRCODE_UNDEFINED_FUNCTION),
                   ...
                   errdetail("Aggregates with DISTINCT must be able to sort "
                             "their inputs."),
...
Node *transformGroupingFunc(ParseState *pstate, GroupingFunc *p) {
  ...
  foreach (lc, args) {
    ...
    current_result =
        transformExpr(pstate, (Node *)lfirst(lc), pstate->p_expr_kind);
 ...
 */
Node *parse_func_or_column(ParseState *pstate, List *funcname, List *fargs,
  ...
  if (fn && fn->agg_filter != NULL)
    agg_filter =
        (Expr *)transform_cypher_expr(pstate, fn->agg_filter, EXPR_KIND_FILTER);
   ...
   */
  could_be_projection =
      (nargs == 1 && !proc_call && agg_order == NIL && agg_filter == NULL &&
       !agg_star && !agg_distinct && over == NULL && !func_variadic &&
  ...
  fdresult = func_get_detail(funcname, fargs, argnames, nargs, actual_arg_types,
                             !func_variadic, true, proc_call, &funcid, &rettype,
                             &retset, &nvargs, &vatype, &declared_arg_types,
                             &argdefaults);
  ...
  if (proc_call &&
      (fdresult == FUNCDETAIL_NORMAL || fdresult == FUNCDETAIL_AGGREGATE ||
       fdresult == FUNCDETAIL_WINDOWFUNC || fdresult == FUNCDETAIL_COERCION))
  ...
  if (fdresult == FUNCDETAIL_NORMAL || fdresult == FUNCDETAIL_PROCEDURE ||
    ...
    if (over)
      ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                      errmsg("OVER specified, but %s is not a window function "
  ...
  if (fdresult == FUNCDETAIL_NORMAL || fdresult == FUNCDETAIL_PROCEDURE) {
    ...
    if (AGGKIND_IS_ORDERED_SET(aggkind)) {
          ...
          ereport(ERROR,
                  (errcode(ERRCODE_UNDEFINED_FUNCTION),
                   errmsg("function %s does not exist",
                          func_signature_string(funcname, nargs, argnames,
                                                actual_arg_types)),
                   errhint_plural("There is an ordered-set aggregate %s, but "
                                  "it requires %d direct argument, not %d.",
                                  "There is an ordered-set aggregate %s, but "
                                  "it requires %d direct arguments, not %d.",
                                  catDirectArgs, NameListToString(funcname),
                  ...
                  (errcode(ERRCODE_UNDEFINED_FUNCTION),
                   errmsg("function %s does not exist",
                          func_signature_string(funcname, nargs, argnames,
                                                actual_arg_types)),
                   errhint(
                       "To use the hypothetical-set aggregate %s, the number "
                       "of hypothetical direct arguments (here %d) must match "
                       "the number of ordering columns (here %d).",
                       NameListToString(funcname), nvargs - numAggregatedArgs,
                       numAggregatedArgs),
                   parser_errposition(pstate, location)));
          } else {
                  ...
                  (errcode(ERRCODE_UNDEFINED_FUNCTION),
                   errmsg("function %s does not exist",
                          func_signature_string(funcname, nargs, argnames,
                                                actual_arg_types)),
                   errhint_plural("There is an ordered-set aggregate %s, but "
                                  "it requires at least %d direct argument.",
                                  "There is an ordered-set aggregate %s, but "
                                  "it requires at least %d direct arguments.",
                                  catDirectArgs, NameListToString(funcname),
                                  catDirectArgs),
                   parser_errposition(pstate, location)));
          }
      ...
      if (aggkind == AGGKIND_HYPOTHETICAL)
        /*            ereport(ERROR,
                            (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                             errmsg("unify_hypothetical_args not
           implemented")));*/
      ...
      if (agg_within_group)
        ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                        errmsg("%s is not an ordered-set aggregate, so it "
                               "cannot have WITHIN GROUP",
                               NameListToString(funcname)),
                        parser_errposition(pstate, location)));
    }
     ...
     */
    return coerce_type(pstate, linitial(fargs), actual_arg_types[0], rettype,
                       -1, COERCION_EXPLICIT, COERCE_EXPLICIT_CALL, location);
      ...
      ereport(ERROR,
              (errcode(ERRCODE_UNDEFINED_FUNCTION),
               errmsg("function %s does not exist",
                      func_signature_string(funcname, nargs, argnames,
                                            actual_arg_types)),
               errhint("No aggregate function matches the given name and "
                       "argument types. "
  ...
  rettype = enforce_generic_type_consistency(
      actual_arg_types, declared_arg_types, nargsplusdefs, rettype, false);
  ...
  if (fdresult == FUNCDETAIL_NORMAL || fdresult == FUNCDETAIL_PROCEDURE) {
    ...
    if (pstate->p_last_srf != last_srf)
      ereport(ERROR,
              (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
               errmsg("window function calls cannot contain set-returning "
                      "function calls"),
...
static Node *transform_a_const(cypher_parsestate *cpstate, A_Const *ac) {
  ...
  case T_Null:
    ...
    break;
  default:
    ereport(ERROR, (errmsg_internal("unrecognized node type: %d", nodeTag(v))));
    return NULL;
  }
...
static Node *transformWholeRowRef(ParseState *pstate,
  ...
  if (nsitem->p_names == nsitem->p_rte->eref) {
    ...
    result =
        makeWholeRowVar(nsitem->p_rte, nsitem->p_rtindex, sublevels_up, true);
     ...
     */
    expandRTE(nsitem->p_rte, nsitem->p_rtindex, sublevels_up, location, false,
              NULL, &fields);
    rowexpr = makeNode(RowExpr);
    rowexpr->args =
        list_truncate(fields, list_length(nsitem->p_names->colnames));
...
static Node *transform_column_ref(cypher_parsestate *cpstate, ColumnRef *cref) {
  ...
  case 1: {
    ...
    if (te != NULL && te->expr != NULL) {
      ...
      ParseNamespaceItem *nsitem = refnameNamespaceItem(
          pstate, NULL, colname, cref->location, &levels_up);
  ...
  case 2: {
    ...
    pnsi = refnameNamespaceItem(pstate, nspname, relname, cref->location,
                                &levels_up);
 ...
 */
static bool is_a_valid_operator(List *lst) {
  ...
  if (!strcmp(str->val.str, "?") || !strcmp(str->val.str, "?|") ||
      !strcmp(str->val.str, "?&"))
    return true;
...
static Node *transform_a_expr_op(cypher_parsestate *cpstate, A_Expr *a) {
  ...
  if (a->lexpr != NULL && is_a_valid_operator(a->name) &&
    ...
    // locate the referenced RTE
    pnsi =
        refnameNamespaceItem(pstate, NULL, relname, cr->location, &levels_up);
    if (pnsi)
...
static Node *transform_bool_expr(cypher_parsestate *cpstate, BoolExpr *expr) {
  ...
  default:
    ereport(ERROR,
            (errmsg_internal("unrecognized boolop: %d", (int)expr->boolop)));
    return NULL;
  }
...
static Node *transform_cypher_param(cypher_parsestate *cpstate,
  ...
  const_str = makeConst(GTYPEOID, -1, InvalidOid, -1, string_to_gtype(p->name),
                        false, false);

  return (Node *)make_op(pstate, list_make1(makeString("->")),
                         (Node *)cpstate->params, (Node *)const_str,
                         pstate->p_last_srf, -1);
}
...
static Node *transform_cypher_map(cypher_parsestate *cpstate, cypher_map *cm) {
  ...
  while (le != NULL) {
    ...
    // typtypmod, typcollation, typlen, and typbyval of gtype are hard-coded.
    Const *newkey = makeConst(TEXTOID, -1, InvalidOid, -1,
  ...
  fexpr = makeFuncExpr(func_oid, GTYPEOID, newkeyvals, InvalidOid, InvalidOid,
...
static Node *transform_cypher_list(cypher_parsestate *cpstate,
  ...
  FuncExpr *expr = makeFuncExpr(oid, GTYPEOID, args, InvalidOid, InvalidOid,
                                COERCE_EXPLICIT_CALL);
...
static Node *transform_column_ref_for_indirection(cypher_parsestate *cpstate,
  ...
  // This column ref is referencing something that was created in a previous
  // query and is a variable.
  if (!pnsi)
...
static Node *transform_a_indirection(cypher_parsestate *cpstate,
  ...
  /*
   * we can mark the last srf here, because if any part of this function finds
   * another srf, its a problem, no need to track this throughout
   *
   * XXX: There might be a use case that allows us to use SRFs with
   * Indirection... leave it as a potential todo for now.
   */
  Node *last_srf = pstate->p_last_srf;
  ...
  if (IsA(a_ind->arg, ColumnRef))
    cur =
        transform_column_ref_for_indirection(cpstate, (ColumnRef *)a_ind->arg);
  else
  ...
  foreach (lc, a_ind->indirection) {
    ...
    if (is_a_slice(node)) {
      ...
      // lower bound
      if (!indices->lidx)
        args = lappend(args, makeConst(GTYPEOID, -1, InvalidOid, -1,
                                       (Datum)NULL, true, false));
      else
        args = lappend(args,
                       transform_cypher_expr_recurse(cpstate, indices->lidx));
      ...
      if (!indices->uidx)
        ...
        args = lappend(args,
                       transform_cypher_expr_recurse(cpstate, indices->uidx));

      Oid oid = get_ag_func_oid("gtype_access_slice", 3, GTYPEOID, GTYPEOID,
                                GTYPEOID);
      FuncExpr *func_expr = makeFuncExpr(oid, GTYPEOID, args, InvalidOid,
      ...
      cur = (Node *)make_op(pstate, list_make1(makeString("->")), cur, node,
                            pstate->p_last_srf, -1);
      ...
      Const *const_str =
          makeConst(GTYPEOID, -1, InvalidOid, -1,
                    string_to_gtype(strVal(string)), false, false);

      cur = (Node *)make_op(pstate, list_make1(makeString("->")), cur,
                            (Node *)const_str, pstate->p_last_srf, -1);
...
static void unify_hypothetical_args(ParseState *pstate, List *fargs,
  ...
  for (hargpos = numNonHypotheticalArgs; hargpos < numDirectArgs; hargpos++) {
    ...
    if (declared_arg_types[hargpos] != declared_arg_types[aargpos])
      elog(ERROR, "hypothetical-set aggregate has inconsistent declared "
                  "argument types");
     ...
     */
    commontype = select_common_type(
        pstate, list_make2(lfirst(aarg), lfirst(harg)), "WITHIN GROUP", NULL);
    commontypmod = select_common_typmod(
        pstate, list_make2(lfirst(aarg), lfirst(harg)), commontype);

    /*
     * Perform the coercions.  We don't need to worry about NamedArgExprs
     * here because they aren't supported with aggregates.
     */
    lfirst(harg) = coerce_type(
        pstate, (Node *)lfirst(harg), actual_arg_types[hargpos], commontype,
        commontypmod, COERCION_IMPLICIT, COERCE_IMPLICIT_CAST, -1);
    actual_arg_types[hargpos] = commontype;
    lfirst(aarg) = coerce_type(
        pstate, (Node *)lfirst(aarg), actual_arg_types[aargpos], commontype,
        commontypmod, COERCION_IMPLICIT, COERCE_IMPLICIT_CAST, -1);
...
static List *make_qualified_function_name(cypher_parsestate *cpstate, List *lst,
  ...
  if (strcmp(ag_name, "now") == 0 ||
      strcmp(ag_name, "transaction_timestamp") == 0 ||
      strcmp(ag_name, "statement_timestamp") == 0 ||
      strcmp(ag_name, "clock_timestamp") == 0 ||
      ...
      strcmp(name, "lag") == 0 || strcmp(name, "lead") == 0 ||
      strcmp(name, "first_value") == 0 || strcmp(name, "last_value") == 0 ||
      strcmp(name, "ntile") == 0 || strcmp(name, "nth_value") == 0)
...
static Node *transform_func_call(cypher_parsestate *cpstate, FuncCall *fn) {
  ...
  foreach (lc, fn->args) {
    args = lappend(args,
                   transform_cypher_expr_recurse(cpstate, (Node *)lfirst(lc)));
  }

  // if the function name is not qualified assume the function is in our
  // catalog.
  List *fname;
  ...
  Node *retval = parse_func_or_column(pstate, fname, args, last_srf, fn, false,
                                      fn->location);

  if (list_length(fn->funcname) == 1) {
    char *name = strVal(linitial(fname));
    if (strcmp(name, "now") == 0 ||
        strcmp(name, "transaction_timestamp") == 0 ||
        strcmp(name, "statement_timestamp") == 0 ||
        strcmp(name, "clock_timestamp") == 0 ||
      ...
      typenameTypeIdAndMod(cpstate, makeTypeName("gtype"), &targetType,
                           &targetTypmod);

      inputType = exprType(retval);

      result = coerce_to_target_type(cpstate, retval, inputType, targetType,
      ...
      if (result == NULL)
        ereport(ERROR,
                (errcode(ERRCODE_CANNOT_COERCE),
                 errmsg("cannot cast type %s to %s", format_type_be(inputType),
...
static Node *transform_coalesce_expr(cypher_parsestate *cpstate,
  ...
  newcexpr->coalescetype =
      select_common_type(pstate, newargs, "COALESCE", NULL);
  ...
  foreach (lc, newargs) {
    ...
    Node *newe =
        coerce_to_common_type(pstate, e, newcexpr->coalescetype, "COALESCE");
...
static Node *transform_case_expr(cypher_parsestate *cpstate, CaseExpr *cexpr) {
  ...
  foreach (lc, cexpr->args) {
    ...
    if (placeholder)
      warg = (Node *)makeSimpleA_Expr(AEXPR_OP, "=", (Node *)placeholder, warg,
                                      w->location);

    neww->expr = (Expr *)transform_cypher_expr_recurse(cpstate, warg);

    neww->expr =
        (Expr *)coerce_to_boolean(pstate, (Node *)neww->expr, "CASE/WHEN");
  ...
  newcexpr->defresult =
      (Expr *)transform_cypher_expr_recurse(cpstate, defresult);
  ...
  foreach (lc, newcexpr->args) {
    ...
    w->result = (Expr *)coerce_to_common_type(pstate, (Node *)w->result, ptype,
                                              "CASE/WHEN");
  }
...
static Node *make_row_comparison_op(ParseState *pstate, List *opname,
    ...
    ereport(
        ...
        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
         errmsg(
             ...
             strVal(llast(opname))),
         errhint("Row comparison operators must be associated with btree "
                 "operator families."),
         parser_errposition(pstate, location)));
  }
  ...
  for (i = 0; i < nopers; i++) {
    ...
    if (OidIsValid(opfamily))
      ...
      ereport(ERROR,
              (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
               errmsg("could not determine interpretation of row comparison "
                      "operator %s",
                      strVal(llast(opname))),
...
static Node *transform_sub_link(cypher_parsestate *cpstate, SubLink *sublink) {
  ...
  err = NULL;
  switch (pstate->p_expr_kind) {
  case EXPR_KIND_OTHER: /* Accept sublink here; caller must throw error if
                           wanted */
  case EXPR_KIND_JOIN_ON:
  case EXPR_KIND_JOIN_USING:
  case EXPR_KIND_FROM_SUBSELECT:
  ...
  if (err)
    ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg_internal("%s", err),
  ...
  Query *query =
      cypher_parse_sub_analyze(sublink->subselect, cpstate, NULL, false, true);
  ...
  if (sublink->subLinkType == EXISTS_SUBLINK) {
    ...
    sublink->testexpr = make_row_comparison_op(
        pstate, sublink->operName, left_list, right_list, sublink->location);
  }
...
static Node *transformSQLValueFunction(cypher_parsestate *cpstate,
  ...
  result =
      coerce_to_target_type(cpstate, expr, inputType, targetType, targetTypmod,
