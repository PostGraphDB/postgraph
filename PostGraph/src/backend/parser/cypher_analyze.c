...
static bool convert_cypher_walker(Node *node, ParseState *pstate) {
  ...
  if (IsA(node, RangeTblFunction)) {
    ...
    return expression_tree_walker((Node *)funcexpr->args, convert_cypher_walker,
                                  pstate);
  }
  ...
  if (IsA(node, Query)) {
     ...
     */
    int flags = QTW_EXAMINE_RTES_BEFORE | QTW_IGNORE_RT_SUBQUERIES |
                QTW_IGNORE_JOINALIASES;
...
static bool is_rte_cypher(RangeTblEntry *rte) {
  // The planner expects RangeTblFunction nodes in rte->functions list. We
  // cannot replace one of them to a SELECT subquery.
...
static void convert_cypher_to_subquery(RangeTblEntry *rte, ParseState *pstate) {
   ...
   * query, it must take a string constant as an argument so that the query can
  ...
  int query_loc =
      get_query_location(((Const *)arg)->location, pstate->p_sourcetext);

  // Check to see if the cypher function had any parameters passed to it, if so
  // make sure Postgres parsed the second argument to a Param node.
  Param *params = NULL;
  ...
  if (is_ag_node(llast(stmt), cypher_create) ||
    ...
    if (!(rtfunc->funccolcount == 1 &&
          linitial_oid(rtfunc->funccoltypes) == GTYPEOID))
      ereport(ERROR,
              (errcode(ERRCODE_DATATYPE_MISMATCH),
               errmsg("column definition list for CREATE clause must contain a "
                      "single gtype attribute"),
    ...
    query =
        analyze_cypher_and_coerce(stmt, rtfunc, pstate, query_str, query_loc,
                                  NameStr(*graph_name), graph_oid, params);
  }
...
static Query *analyze_cypher(List *stmt, ParseState *parent_pstate,
  ...
  // install error context callback to adjust an error position since locations
  // in stmt are 0 based
  errpos_ecb_state ecb_state;
...
static Query *analyze_cypher_and_coerce(List *stmt, RangeTblFunction *rtfunc,
  ...
  Query *subquery = analyze_cypher(stmt, pstate, query_str, query_loc,
                                   graph_name, graph_oid, (Param *)params);
  ...
  foreach (lc, query->targetList) {
    ...
    if (current_type != target_type) {
      ...
      /*
       * The coercion context of this coercion is COERCION_EXPLICIT because the
       * target type is explicitly metioned in the column definition list and we
       * need to do this by looking up all possible coercion.
       */
      Node *new_expr = coerce_to_target_type(
          pstate, expr, current_type, target_type, target_typmod,
