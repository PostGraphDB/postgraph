...
Datum create_unique_properties_constraint(PG_FUNCTION_ARGS) {
  ...
  ProcessUtility(wrapper, "(generated ALTER TABLE ADD CONSTRAINT command)",
                 false, PROCESS_UTILITY_SUBCOMMAND, NULL, NULL, None_Receiver,
                 NULL);
...
Datum create_property_index(PG_FUNCTION_ARGS) {
  ...
  idx_elem->name = NULL;
  idx_elem->expr = makeSimpleA_Expr(
      AEXPR_OP, "->", c,
      makeStringConstCast(make_property_alias_for_index(property), -1,
                          makeTypeName("gtype")),
      -1);
...
static List *create_edge_table_elements(char *graph_name, char *label_name,
  ...
  id->constraints = list_make2(
      build_pk_constraint(),
      build_id_default(graph_name, label_name, schema_name, seq_name));

  // "start_id" graphid NOT NULL
  start_id =
      makeColumnDef(AG_EDGE_COLNAME_START_ID, GRAPHIDOID, -1, InvalidOid);
  ...
  // "properties" gtype NOT NULL DEFAULT CATALOG_SCHEMA."gtype_build_map"()
  props = makeColumnDef(AG_EDGE_COLNAME_PROPERTIES, GTYPEOID, -1, InvalidOid);
  props->constraints =
      list_make2(build_not_null_constraint(), build_properties_default());
...
static List *create_vertex_table_elements(char *graph_name, char *label_name,
  ...
  // "properties" gtype NOT NULL DEFAULT CATALOG_SCHEMA."gtype_build_map"()
  props = makeColumnDef(AG_VERTEX_COLNAME_PROPERTIES, GTYPEOID, -1, InvalidOid);
  props->constraints =
      list_make2(build_not_null_constraint(), build_properties_default());
...
static FuncCall *build_id_default_func_expr(char *graph_name, char *label_name,
  ...
  label_id_func_name =
      list_make2(makeString(CATALOG_SCHEMA), makeString("_label_id"));
  ...
  nextval_func =
      makeFuncCall(nextval_func_name, nextval_func_args, COERCE_SQL_SYNTAX, -1);
   ...
   */
  graphid_func_name =
      list_make2(makeString(CATALOG_SCHEMA), makeString("_graphid"));
  graphid_func_args = list_make2(label_id_func, nextval_func);
  graphid_func =
      makeFuncCall(graphid_func_name, graphid_func_args, COERCE_SQL_SYNTAX, -1);
...
static Constraint *build_id_default(char *graph_name, char *label_name,
  ...
  graphid_func =
      build_id_default_func_expr(graph_name, label_name, schema_name, seq_name);
...
static Constraint *build_properties_default(void) {
  ...
  func_name =
      list_make2(makeString(CATALOG_SCHEMA), makeString("gtype_build_map"));
...
static void remove_relation(List *qname) {
  ...
  rel_oid =
      RangeVarGetRelidExtended(rel, AccessExclusiveLock, RVR_MISSING_OK,
                               range_var_callback_for_remove_relation, NULL);
...
static void range_var_callback_for_remove_relation(const RangeVar *rel,
  ...
  if (!pg_class_ownercheck(rel_oid, GetUserId()) &&
    ...
    aclcheck_error(ACLCHECK_NOT_OWNER,
                   get_relkind_objtype(get_rel_relkind(rel_oid)), rel->relname);
  }
